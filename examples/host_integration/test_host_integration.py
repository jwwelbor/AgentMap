...
class HostIntegrationTester:
            ...
            "host_application": {
                ...
                "services": {
                    "database_service": {
                        "enabled": True,
                        "configuration": {
                            "database_path": str(self.temp_dir / "test.db")
                        },
                    },
                    "email_service": {
                        "enabled": True,
    ...
    def test_service_instantiation(self) -> bool:
        ...
        try:
            ...
            db_service = self.container.get_host_service_instance(
                "test_database_service"
            )
            ...
            # Test email service
            email_service = self.container.get_host_service_instance(
                "test_email_service"
            )
            ...
            notification_service = self.container.get_host_service_instance(
                "test_notification_service"
            )
            if not notification_service or not isinstance(
    ...
    def test_agent_configuration(self) -> bool:
        ...
        try:
            ...
            notification_agent = NotificationAgent(
                "test_notification_agent", logger=logger
            )
            configured_count = self.container.configure_host_protocols(
                notification_agent
            )
            ...
            if configured_count < 3:  # Should have all 3 services
                raise AssertionError(
                    f"Multi-service agent only configured {configured_count}/3 services"
                )
    ...
    def test_agent_functionality(self) -> bool:
        ...
        try:
            ...
            if result.get("status") != "success":
                raise AssertionError(
                    f"Notification agent failed: {result.get('error')}"
                )
            ...
            result = multi_agent.run(
                {"operation": "process_user_request", "user_id": 1}
            )
            if result.get("status") != "success":
                raise AssertionError(
                    f"Multi-service agent failed: {result.get('error')}"
                )
            ...
            print(
                f"   ✅ Multi-service agent processed user: {multi_result['user']['name']}"
            )
