...
class TestBlobStorageService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        blob_config = {
            "blob": {
                ...
                "providers": {
                    "azure": {"connection_string": "test_connection_string"},
                    "s3": {
                        "access_key": "test_access_key",
                        "secret_key": "test_secret_key",
                        "region": "us-east-1",
                    },
                    "gs": {"credentials_path": "/path/to/credentials.json"},
                    "file": {"base_directory": "data/blob/files"},
                },
            }
        }

        self.mock_storage_config_service = (
            MockServiceFactory.create_mock_storage_config_service(blob_config)
        )

        # Add blob-specific methods to the mock
        self.mock_storage_config_service.get_blob_config.return_value = blob_config[
            "blob"
        ]
        ...
        self.mock_storage_config_service.get_blob_provider_config.side_effect = (
            get_blob_provider_config
        )

        def is_blob_storage_enabled():
            return blob_config["blob"].get("enabled", False)

        self.mock_storage_config_service.is_blob_storage_enabled.side_effect = (
        ...
        self.mock_availability_cache.get_availability.return_value = (
            None  # Default to cache miss
        )
        self.mock_availability_cache.set_availability.return_value = True
        self.mock_availability_cache.get_cache_stats.return_value = {
            "total_entries": 0,
            "performance": {"cache_hits": 0, "cache_misses": 0, "cache_sets": 0},
        }
    ...
    def test_health_check_all_providers(self):
        ...
        self.service._available_providers.update(
            {"azure": True, "s3": True, "gs": True, "file": True, "local": True}
        )
    ...
    def test_health_check_with_provider_errors(self):
        ...
        self.service._config = {"providers": {"file": {}, "local": {}}}
    ...
    def test_dependency_availability_static_methods(
        self, mock_gcs, mock_s3, mock_azure
    ):
    ...
    def test_graceful_degradation_with_import_errors(
        self, mock_gcs, mock_s3, mock_azure
    ):
    ...
    def test_configuration_loading_edge_cases(self):
        ...
        for config_data in config_variations:
            mock_config = MockServiceFactory.create_mock_storage_config_service(
                config_data
            )

            # Configure the blob config method to return appropriate data
            if "storage" in config_data and "blob" in config_data["storage"]:
                mock_config.get_blob_config.return_value = config_data["storage"][
                    "blob"
                ]
    ...
    def test_large_blob_operations(self):
        ...
        with patch(
            "agentmap.services.storage.blob_storage_service.get_connector_for_uri"
