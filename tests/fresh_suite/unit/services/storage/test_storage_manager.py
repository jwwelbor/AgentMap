...
class MockStorageServiceFactory:
    ...
    def create_service(self, provider_name: str, config_data: Dict[str, Any]):
        ...
        mock_config = Mock()
        mock_config.get_value.side_effect = lambda key, default=None: config_data.get(
            key, default
        )
        ...
        return self.service_class(
            provider_name, mock_config, mock_logging, mock_file_path
        )
...
class TestStorageServiceManager(unittest.TestCase):
    ...
    def test_get_service_creation_failure(self):
        ...
        self.assertIn(
            "Failed to initialize failing_provider client", str(context.exception)
        )
    ...
    def test_get_service_factory_creation_failure(self):
        ...
        self.assertIn(
            "Failed to initialize failing_factory client", str(context.exception)
        )
    ...
    def test_get_service_info_with_health_check_exception(self):
        ...
        with patch.object(
            service, "health_check", side_effect=Exception("Health check failed")
        ):
            # Get service info
            info = self.manager.get_service_info("test_provider")
    ...
    def test_register_same_provider_twice(self):
        ...
        self.assertEqual(
            self.manager._service_classes["duplicate"], FailingStorageService
        )
    ...
    def test_empty_provider_name(self):
        ...
        self.assertIn(
            "Provider name must be a non-empty string", str(context.exception)
        )
