...
class BlobStorageTestFixtures:
    ...
    LOCAL_CONFIG = {
        "base_directory": "/tmp/blob_storage"
    }
    ...
    @classmethod
    def get_test_uri_for_provider(cls, provider: str, filename: str = "test.blob") -> str:
...
class MockBlobStorageServiceFactory:
    ...
    def create_successful_service() -> Mock:
        ...
        def mock_read_blob(uri: str) -> bytes:
            ...
            return _storage.get(uri, BlobStorageTestFixtures.TEST_STRING_DATA.encode('utf-8'))
        
        mock_service.read_blob.side_effect = mock_read_blob
        
        # Mock successful write operations
        def mock_write_blob(uri: str, data: Union[str, bytes, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        mock_service.list_blobs.return_value = ["mock://container/blob1", "mock://container/blob2"]
    ...
    def create_failing_service() -> Mock:
        ...
        mock_service.write_blob.side_effect = StorageOperationError("Mock write failure")
        mock_service.blob_exists.side_effect = StorageOperationError("Mock exists check failure")
        mock_service.list_blobs.side_effect = StorageOperationError("Mock list failure")
        mock_service.delete_blob.side_effect = StorageOperationError("Mock delete failure")
    ...
    def create_intermittent_service(failure_rate: float = 0.5) -> Mock:
        ...
        if failure_rate == 0.3:
            ...
            failure_calls = set(range(1, total_calls + 1, total_calls // expected_failures)[:expected_failures])
        ...
        def maybe_fail_write(uri: str, data: Union[str, bytes, Dict[str, Any]]) -> Dict[str, Any]:
            call_count['count'] += 1
            if call_count['count'] in failure_calls:
...
class BlobAgentTestHelpers:
    ...
    def create_test_blob_reader(
        name: str = "test_reader",
        blob_service: Optional[Mock] = None,
        **kwargs
    ) -> BlobReaderAgent:
        ...
        if 'execution_tracking_service' not in kwargs:
            kwargs['execution_tracking_service'] = MockServiceFactory.create_mock_execution_tracking_service()
        if 'state_adapter_service' not in kwargs:
            kwargs['state_adapter_service'] = MockServiceFactory.create_mock_state_adapter_service()
        
        agent = BlobReaderAgent(
            name=name,
            prompt="Test blob reader",
            **kwargs
        )
    ...
    def create_test_blob_writer(
        ...
        **kwargs
        ...
        agent = BlobWriterAgent(
            name=name,
            prompt="Test blob writer",
            **kwargs
        )
...
class PerformanceTestHelpers:
    ...
    def validate_performance(
        ...
        if operation_time > max_time:
            print(f"WARNING: {operation_name} took {operation_time:.2f}s for {data_size} bytes "
                  f"(max: {max_time:.2f}s)")
            return False
        return True
...
class MockCloudProviderHelpers:
    ...
    def mock_azure_behavior(mock_service: Mock, scenario: str = "success"):
        ...
        if scenario == "success":
            mock_service.read_blob.return_value = BlobStorageTestFixtures.TEST_STRING_DATA.encode()
            mock_service.write_blob.return_value = {"success": True, "etag": "mock-etag"}
            mock_service.blob_exists.return_value = True
            ...
            mock_service.read_blob.side_effect = StorageConnectionError("Authentication failed")
    ...
    def mock_s3_behavior(mock_service: Mock, scenario: str = "success"):
        ...
        if scenario == "success":
            mock_service.read_blob.return_value = BlobStorageTestFixtures.TEST_STRING_DATA.encode()
            mock_service.write_blob.return_value = {"success": True, "version_id": "mock-version"}
            mock_service.blob_exists.return_value = True
        elif scenario == "not_found":
    ...
    def mock_gcs_behavior(mock_service: Mock, scenario: str = "success"):
        ...
        if scenario == "success":
            mock_service.read_blob.return_value = BlobStorageTestFixtures.TEST_STRING_DATA.encode()
            mock_service.write_blob.return_value = {"success": True, "generation": "mock-generation"}
            mock_service.blob_exists.return_value = True
        elif scenario == "not_found":
            mock_service.read_blob.side_effect = FileNotFoundError("Object not found")
            mock_service.blob_exists.return_value = False
            ...
            from agentmap.exceptions import StorageOperationError
            mock_service.write_blob.side_effect = StorageOperationError("Quota exceeded")
...
class BlobStorageTestRunner:
    ...
    def run_provider_compatibility_tests(self, providers: List[str]) -> Dict[str, bool]:
        ...
        for provider in providers:
            try:
                ...
                test_uri = self.env.get_blob_uri(f"compat_test_{provider}.blob", provider)
    ...
    def run_error_resilience_tests(self, error_scenarios: List[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    def run_performance_tests(self, performance_scenarios: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
        ...
        for scenario in performance_scenarios:
            try:
                ...
                write_throughput = PerformanceTestHelpers.calculate_throughput(data_size, write_time)
                read_throughput = PerformanceTestHelpers.calculate_throughput(data_size, read_time)
