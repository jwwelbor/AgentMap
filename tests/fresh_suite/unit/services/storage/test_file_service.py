...
class TestFileStorageService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Apply the patch
        self.storage_config_patch = patch.object(
            StorageConfig, "from_dict", side_effect=mock_storage_config_from_dict
        )
    ...
    def test_health_check_with_inaccessible_directory(self):
        ...
        bad_config = MockServiceFactory.create_mock_storage_config_service(
            {"file": {"enabled": True, "base_directory": invalid_path}}
        )
    ...
    def test_path_validation_security(self):
        ...
        if platform.system() != "Windows":
            # Unix-like systems
            dangerous_paths.extend(["/etc/passwd", "/etc/shadow", "/root/.bashrc"])
        else:
        ...
        for dangerous_path in dangerous_paths:
            ...
            try:
                ...
                self.assertIsNone(
                    result, f"Expected None for dangerous path: {dangerous_path}"
                )
            ...
            try:
                result = self.service.write(
                    "test_collection", "content", dangerous_path
                )
                # If no exception was raised, should return error result
                if hasattr(result, "success"):
                    self.assertFalse(
                        result.success,
                        f"Expected failure for dangerous path: {dangerous_path}",
                    )
    ...
    def test_write_modes_text_files(self):
        ...
        result = self.service.write(
            collection, initial_content, document_id, WriteMode.WRITE
        )
        self.assertTrue(result.success)
        self.assertTrue(result.created_new)
        ...
        result = self.service.write(
            collection, append_content, document_id, WriteMode.APPEND
        )
        self.assertTrue(result.success)
        ...
        result = self.service.write(
            collection, update_content, document_id, WriteMode.UPDATE
        )
        self.assertTrue(result.success)
    ...
    def test_write_and_read_binary_file(self):
        ...
        # Read the file back
        retrieved_content = self.service.read(
            collection, document_id, binary_mode=True, format="raw"
        )
        self.assertEqual(retrieved_content, content)
    ...
    def test_binary_file_detection_and_handling(self):
        ...
        structured_result = self.service.read(
            collection, "image.png", format="structured"
        )
    ...
    def test_binary_disabled_configuration(self):
        ...
        result = no_binary_service.write(
            "test", binary_content, "test.bin", binary_mode=True
        )
    ...
    def test_copy_file(self):
        ...
        result = self.service.copy_file(
            source_collection, source_id, target_collection, target_id
        )
        ...
        original_content = self.service.read(
            source_collection, source_id, format="text"
        )
        self.assertEqual(original_content, content)

    def test_copy_nonexistent_file(self):
        """Test copying non-existent file."""
        result = self.service.copy_file("nonexistent", "file.txt", "target", "copy.txt")
    ...
    def test_langchain_loader_integration(self, mock_text_loader):
        ...
        # To get structured data with metadata, explicitly request structured format
        structured_result = self.service.read(
            collection, document_id, format="structured"
        )
        self.assertIsInstance(structured_result, dict)
        self.assertIn("content", structured_result)
        self.assertIn("metadata", structured_result)
    ...
    def test_langchain_loader_fallback(self, mock_text_loader):
        ...
        document_id = (
            "test.unknown"  # Unknown extension to trigger document loader path
        )
