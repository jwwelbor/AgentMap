...
class TestLLMServiceEnhanced(unittest.TestCase):
    ...
    def test_can_make_successful_llm_call_with_text_response(self):
        ...
        with patch.object(self.service, '_get_or_create_client') as mock_client, \
    ...
    def test_can_use_simple_generate_interface(self):
        ...
        with patch.object(self.service, 'call_llm') as mock_call_llm:
            ...
            expected_messages = [{"role": "user", "content": "What is the answer to everything?"}]
    ...
    def test_falls_back_gracefully_when_routing_fails(self):
        ...
        # Make routing fail
        self.mock_routing_service.route_request.side_effect = Exception("Routing service down")
    ...
    def test_handles_large_message_payloads(self):
        ...
        large_messages = [
            ...
            {"role": "user", "content": f"Please analyze this document: {large_content}"}
        ]
