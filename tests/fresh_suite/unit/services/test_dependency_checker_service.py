...
class TestDependencyCheckerService(unittest.TestCase):
    ...
    def test_service_initialization(self):
        ...
        self.assertEqual(
            self.service.features_registry, self.mock_features_registry_service
        )
    ...
    def test_check_dependency_with_version(self, mock_importlib):
        ...
        with patch("packaging.version.parse") as mock_version_parse:
            ...
            mock_parsed_current.__lt__ = MagicMock(
                return_value=False
            )  # 2.0.0 is not < 1.5.0

            mock_version_parse.side_effect = lambda v: (
                mock_parsed_current if v == "2.0.0" else mock_parsed_required
            )
    ...
    def test_validate_imports_all_valid(self):
        ...
        with patch.object(
            self.service._dependency_validator, "check_dependency", return_value=True
        ):
            # Execute test
            success, invalid = self.service.validate_imports(
    ...
    def test_validate_imports_some_invalid(self):
        ...
        with patch.object(
            self.service._dependency_validator,
            "check_dependency",
            side_effect=mock_check_dependency,
        ):
            # Execute test
            success, invalid = self.service.validate_imports(
                ["valid_module", "missing_module", "another_valid"]
            )
    ...
    def test_check_llm_dependencies_feature_disabled(self):
        ...
        self.mock_features_registry_service.is_feature_enabled.assert_called_once_with(
            "llm"
        )
    ...
    def test_check_llm_dependencies_specific_provider_success(self):
        ...
        with patch.object(
            self.service, "_validate_llm_provider", return_value=(True, [])
        ):
            # Execute test
            ...
            self.mock_features_registry_service.set_provider_validated.assert_called_once_with(
                "llm", "openai", True
            )
    ...
    def test_check_llm_dependencies_specific_provider_failure(self):
        ...
        with patch.object(
            self.service, "_validate_llm_provider", return_value=(False, missing_deps)
        ):
            # Execute test
            result, missing = self.service.check_llm_dependencies(provider="openai")
            ...
            self.mock_features_registry_service.record_missing_dependencies.assert_called_once_with(
                "llm.openai", missing_deps
            )
    ...
    def test_check_llm_dependencies_any_provider_available(self):
        ...
        def mock_validate_provider(provider, force=False):
            ...
            return (
                ...
                else (False, [f"missing_{provider}"])
            )

        with patch.object(
            self.service, "_validate_llm_provider", side_effect=mock_validate_provider
        ):
            # Execute test
            result, missing = self.service.check_llm_dependencies()
    ...
    def test_check_storage_dependencies_specific_type_success(self):
        ...
        with patch.object(
            self.service, "_validate_storage_type", return_value=(True, [])
        ):
            # Execute test
            result, missing = self.service.check_storage_dependencies(
                storage_type="csv"
    ...
    def test_can_use_provider_enabled_and_validated(self):
        ...
        self.mock_features_registry_service.is_provider_validated.assert_called_once_with(
            "llm", "openai"
        )
    ...
    def test_discover_and_validate_providers_llm(self):
        ...
        with patch.object(
            ...
            self.assertEqual(
                self.mock_features_registry_service.set_provider_validated.call_count, 3
            )
    ...
    def test_discover_and_validate_providers_storage(self):
        ...
        def mock_validate_storage(storage_type, force=False):
            ...
            return (
                ...
                else (False, [f"missing_{storage_type}"])
            )

        with patch.object(
            self.service, "_validate_storage_type", side_effect=mock_validate_storage
        ):
            # Execute test
            ...
            self.assertTrue(
                self.mock_features_registry_service.set_provider_validated.call_count
                >= 2
            )
    ...
    def test_get_dependency_status_summary(self):
        ...
        self.mock_features_registry_service.is_feature_enabled.side_effect = (
            lambda feature: feature in ["llm", "storage"]
        )
        self.mock_features_registry_service.get_missing_dependencies.return_value = []

        with (
            patch.object(
                self.service, "get_available_llm_providers", return_value=["openai"]
            ),
            patch.object(
                self.service, "get_available_storage_types", return_value=["csv"]
            ),
        ):
    ...
    def test_check_dependency_exception_handling(self):
        ...
        with patch(
            "agentmap.services.dependency_validators.importlib"
        ) as mock_importlib:
    ...
    def test_validate_imports_exception_handling(self):
        ...
        with patch.object(
            self.service._dependency_validator,
            "check_dependency",
            side_effect=problematic_check_dependency,
        ):
            # Execute test
            success, invalid = self.service.validate_imports(
                ["good_module", "problem_module"]
            )
    ...
    def test_features_registry_integration_error_handling(self):
        ...
        self.mock_features_registry_service.set_provider_validated.side_effect = (
    ...
    def test_circular_dependency_patterns(self):
        ...
        with patch.object(
            self.service._dependency_validator, "validate_imports"
        ) as mock_validate:
