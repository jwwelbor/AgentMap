...
class TestPromptManagerService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "pathlib.Path.mkdir"
        ):  # Prevent actual directory creation during init
    ...
    def test_service_initialization_with_custom_config(self):
        ...
        self.assertEqual(
            service.registry_path, Path("custom_prompts/custom_registry.yaml")
        )
    ...
    def test_load_registry_from_configured_path(
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("yaml.safe_load")
    def test_load_registry_yaml_parse_error(
        self, mock_yaml_load, mock_file_open, mock_exists
    ):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("yaml.safe_load")
    def test_load_registry_with_system_fallback(
        self, mock_yaml_load, mock_file_open, mock_exists
    ):
        ...
        mock_exists.side_effect = [
            False,
            True,
        ]  # First False, then True for system registry
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("yaml.safe_load")
    def test_resolve_prompt_yaml_reference_key_not_found(
    ...
    def test_resolve_prompt_yaml_reference_invalid_format(self):
        ...
        self.assertEqual(
            result, "[Invalid YAML reference (missing #key): missing_hash_key]"
        )
    ...
    def test_format_prompt_with_langchain(self):
        ...
        with patch("langchain_core.prompts.PromptTemplate") as mock_template_class:
            ...
            mock_template_class.assert_called_once_with(
                template=template, input_variables=["name", "count"]
            )
    ...
    def test_format_prompt_global_function(self):
        ...
        with patch(
            "agentmap.services.prompt_manager_service.get_prompt_manager"
        ) as mock_get_manager:
            mock_manager = Mock()
    ...
    def test_get_formatted_prompt_helper_function(self):
        ...
        with (
            ...
            patch(
                "agentmap.services.prompt_manager_service.resolve_prompt"
            ) as mock_resolve,
        ):
    ...
    def test_resolve_prompt_yaml_non_scalar_value(self):
        ...
        with (
            patch.object(self.service, "_find_resource") as mock_find,
            patch("builtins.open", new_callable=mock_open),
    ...
    def test_full_prompt_workflow_integration(self, mock_mkdir):
        ...
        # Create service
        service = PromptManagerService(
            app_config_service=mock_config, logging_service=self.mock_logging_service
        )
        ...
        result1 = service.format_prompt(
            "prompt:user_greeting", {"username": "TestUser", "system_name": "AgentMap"}
        )
        self.assertEqual(result1, "Welcome TestUser to AgentMap!")

        # 2. Error handling scenario
        result2 = service.format_prompt(
            "prompt:error_template",
            {"component": "PromptManager", "error_details": "Template not found"},
        )
        self.assertEqual(result2, "Error in PromptManager: Template not found")

        # 3. Success scenario with metrics
        result3 = service.format_prompt(
            "prompt:success_message", {"item_count": 150, "duration": 2500}
        )
