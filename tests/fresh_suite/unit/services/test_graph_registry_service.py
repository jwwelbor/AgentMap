...
class TestGraphRegistryService(unittest.TestCase):
    ...
    def test_register_multiple_graphs_same_csv(self):
        ...
        self.assertEqual(
            self._normalize_path(hash_entry["graph1"]["bundle_path"]),
            "/tmp/bundle1.pkl",
        )
        self.assertEqual(
            self._normalize_path(hash_entry["graph2"]["bundle_path"]),
            "/tmp/bundle2.pkl",
        )
    ...
    def test_register_invalid_parameters_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            self.service.register(
                self.test_csv_hash, "", self.test_bundle_path, self.test_csv_path
            )
    ...
    def _register_multiple_test_graphs(self):
        ...
        with PathOperationsMocker() as path_mock:
            ...
            with patch("pathlib.Path.stat") as mock_stat:
                ...
                self.service.register(
                    self.test_csv_hash, "graph1", bundle1_path, self.test_csv_path
                )
                self.service.register(
                    self.test_csv_hash, "graph2", bundle2_path, self.test_csv_path
                )
    ...
    def test_full_composite_key_workflow(self):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("pathlib.Path.stat") as mock_stat:
                ...
                self.service.register(
                    csv_hash, "workflow1", Path("/tmp/wf1.pkl"), Path("/tmp/test.csv")
                )
                self.service.register(
                    csv_hash, "workflow2", Path("/tmp/wf2.pkl"), Path("/tmp/test.csv")
                )
