...
class TestGraphRegistryService(unittest.TestCase):
    ...
    # =============================================================================
    # 3. Legacy Structure Migration Tests
    # =============================================================================
    
    def test_migrate_legacy_entry_on_first_access(self):
        """Test that legacy direct entries are migrated on first find_bundle() call."""
        # Simulate legacy structure directly in cache
        legacy_entry = {
        ...
        with patch.object(Path, 'exists', return_value=True):
            ...
            result = self.service.find_bundle(self.test_csv_hash, "legacy_graph")
        
        # Verify migration occurred
        hash_entry = self.service._registry_cache[self.test_csv_hash]
        self.assertNotIn("bundle_path", hash_entry)  # No longer direct entry
        self.assertIn("legacy_graph", hash_entry)  # Now nested structure
        
        # Verify result still found
        self.assertEqual(result, self.test_bundle_path)
    
    def test_migrate_legacy_entry_on_register(self):
        """Test that legacy entries are migrated when registering new graphs."""
        # Create legacy structure
        legacy_entry = {
            "graph_name": "legacy_graph",
            "csv_hash": self.test_csv_hash,
            "bundle_path": "/tmp/legacy.pkl",
            "csv_path": str(self.test_csv_path),
            "created_at": datetime.utcnow().isoformat(),
            "bundle_size": 512
        }
        ...
        # Register new graph for same CSV
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pathlib.Path.stat') as mock_stat:
                stat_result = Mock()
                stat_result.st_size = 1024
        ...
        # Verify migration and new registration
        hash_entry = self.service._registry_cache[self.test_csv_hash]
        self.assertIn("legacy_graph", hash_entry)
        self.assertIn("new_graph", hash_entry)
        
        # Verify both entries have proper structure
        self.assertEqual(hash_entry["legacy_graph"]["bundle_path"], "/tmp/legacy.pkl")
        self.assertEqual(hash_entry["new_graph"]["bundle_path"], str(self.test_bundle_path))
