...
Tests cover:
- _sanitize_request_id with CUSTOM_ID_RE (Anthropic/OpenAI constraint)
- _sanitize_request_id with GEMINI_KEY_RE (Gemini constraint)
- build_request_id_map: happy path, collision detection, already-safe ids
"""
...
def _make_spec(request_id: str) -> LLMRequest:
    return LLMRequest(
        request_id=request_id, messages=[{"role": "user", "content": "hi"}]
    )


# ---------------------------------------------------------------------------
# AC-T1: _sanitize_request_id
# ---------------------------------------------------------------------------


class TestSanitizeSpecId:
    """AC-T1: accepts raw request_id + regex constraint; returns sanitized id."""

    def test_already_safe_id_returned_unchanged(self):
        """TC-007 edge: request_id already safe → returned unchanged."""
        result = _sanitize_request_id("my_safe_id", CUSTOM_ID_RE)
        assert result == "my_safe_id"

    def test_safe_id_with_hyphens_returned_unchanged(self):
        result = _sanitize_request_id("safe-id-123", CUSTOM_ID_RE)
        assert result == "safe-id-123"

    def test_unsafe_id_returns_deterministic_hash(self):
        """TC-007: request_id with special chars → deterministic safe string."""
        result = _sanitize_request_id("my/spec/with:special!chars", CUSTOM_ID_RE)
        # Must match regex
    ...
    def test_unsafe_id_is_deterministic(self):
        ...
        result1 = _sanitize_request_id("my/spec/with:special!chars", CUSTOM_ID_RE)
        result2 = _sanitize_request_id("my/spec/with:special!chars", CUSTOM_ID_RE)
        assert result1 == result2

    def test_different_inputs_produce_different_hashes(self):
        """TC-007: 'my/spec/with:special!chars' does not collide with 'my_spec_with_special_chars'."""
        r1 = _sanitize_request_id("my/spec/with:special!chars", CUSTOM_ID_RE)
        r2 = _sanitize_request_id("my_spec_with_special_chars", CUSTOM_ID_RE)
        # r2 is already safe so returned unchanged; they must differ
        assert r1 != r2

    def test_request_id_over_64_chars_is_truncated(self):
        """TC-007 edge: request_id > 64 chars → truncated to 64."""
        long_id = "a" * 100
        result = _sanitize_request_id(long_id, CUSTOM_ID_RE)
    ...
    def test_gemini_regex_constraint_applied(self):
        ...
        long_safe = "a" * 128
        result = _sanitize_request_id(long_safe, GEMINI_KEY_RE)
    ...
    def test_gemini_regex_unsafe_id_truncated_to_128(self):
        ...
        result = _sanitize_request_id(unsafe_id, GEMINI_KEY_RE)
        assert len(result) <= 128
        assert GEMINI_KEY_RE.match(result)

    def test_default_regex_is_custom_id_re(self):
        """_sanitize_request_id without regex arg uses CUSTOM_ID_RE."""
        unsafe = "has spaces here"
        result = _sanitize_request_id(unsafe)
        assert CUSTOM_ID_RE.match(result)


# ---------------------------------------------------------------------------
# AC-T2: build_request_id_map — collision detection
# ---------------------------------------------------------------------------


class TestBuildSpecIdMap:
    """AC-T2 / AC-T1: build_request_id_map sanitization and collision detection."""

    def test_happy_path_all_safe_ids(self):
        """All safe request_ids returned in 1-to-1 map."""
        specs = [_make_spec("spec1"), _make_spec("spec2")]
        result = build_request_id_map(specs, CUSTOM_ID_RE)
        assert result == {"spec1": "spec1", "spec2": "spec2"}

    def test_unsafe_ids_sanitized_in_map(self):
        """Unsafe request_ids are sanitized; map keys are original request_ids."""
        spec = _make_spec("my/spec/with:special!chars")
        result = build_request_id_map([spec], CUSTOM_ID_RE)
    ...
    def test_collision_raises_llm_service_error(self):
        """AC-T2: two request_ids that sanitize to the same provider_id raise LLMServiceError.
        ...
        with pytest.raises(LLMServiceError, match="collision"):
            build_request_id_map(specs, tiny_re)
    ...
    def test_no_collision_distinct_hashes(self):
        ...
        result = build_request_id_map([spec_a, spec_b], CUSTOM_ID_RE)
        assert len(result) == 2
        assert result["path/a"] != result["path/b"]

    def test_empty_specs_returns_empty_map(self):
        result = build_request_id_map([], CUSTOM_ID_RE)
        assert result == {}

    def test_gemini_regex_accepted(self):
        """build_request_id_map accepts Gemini regex."""
        specs = [_make_spec("my/gemini/spec")]
        result = build_request_id_map(specs, GEMINI_KEY_RE)
