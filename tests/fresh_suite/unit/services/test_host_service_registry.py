...
class TestHostServiceRegistry(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = HostServiceRegistry(logging_service=self.mock_logging_service)
    ...
    def test_service_initialization(self):
        ...
        self.assertTrue(
            any("Initialized" in call[1] for call in logger_calls if call[0] == "debug")
        )
    ...
    def test_register_service_provider_basic(self):
        ...
        self.assertTrue(
            any(
                "Registered service provider: test_service" in call[1]
    ...
    def test_register_service_provider_with_protocols(self):
        ...
        self.service.register_service_provider(
            "test_service", self.test_provider, protocols=self.single_protocol
        )
        ...
        self.assertTrue(
            any(
                "Registered protocol TestProtocol -> test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_register_service_provider_with_metadata(self):
        ...
        # Act
        self.service.register_service_provider(
            "test_service", self.test_provider, metadata=self.test_metadata
        )
    ...
    def test_register_service_provider_with_multiple_protocols(self):
        ...
        # Act
        self.service.register_service_provider(
            "multi_service", self.test_provider, protocols=self.multiple_protocols
        )
    ...
    def test_register_service_provider_overwrite_existing(self):
        ...
        self.assertTrue(
            any(
                "already registered, overwriting" in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
        )
    ...
    def test_register_service_provider_empty_name_warning(self):
        ...
        self.assertTrue(
            any(
                "Empty service name provided" in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
        )
    ...
    def test_register_service_provider_empty_provider_warning(self):
        ...
        self.assertTrue(
            any(
                "Empty provider provided for service 'test_service'" in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
        )
    ...
    def test_register_service_provider_invalid_protocol(self):
        ...
        self.service.register_service_provider(
            "test_service", self.test_provider, protocols=invalid_protocols
        )
        ...
        self.assertTrue(
            any(
                "Invalid protocol provided" in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
        )
    ...
    def test_register_protocol_implementation_invalid_protocol(self):
        ...
        # Act
        self.service.register_protocol_implementation(
            str, "test_service"
        )  # Not a Protocol
    ...
    def test_register_protocol_implementation_unregistered_service(self):
        ...
        # Act
        self.service.register_protocol_implementation(
            TestProtocol, "nonexistent_service"
        )
        ...
        self.assertTrue(
            any(
                "Service 'nonexistent_service' not registered" in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
        )
    ...
    def test_get_service_provider_existing_service(self):
        ...
        self.assertTrue(
            any(
                "Retrieved service provider: test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_service_provider_nonexistent_service(self):
        ...
        self.assertTrue(
            any(
                "Service 'nonexistent_service' not found" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_protocol_implementation_existing(self):
        ...
        self.assertTrue(
            any(
                "Protocol TestProtocol implemented by: test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_protocol_implementation_nonexistent(self):
        ...
        self.assertTrue(
            any(
                "No implementation found for protocol: TestProtocol" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_discover_services_by_protocol_single_implementation(self):
        ...
        self.assertTrue(
            any(
                "Found 1 services implementing TestProtocol" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_discover_services_by_protocol_multiple_implementations(self):
        ...
        # Arrange
        self.service.register_service_provider(
            "service1", self.test_provider, protocols=self.single_protocol
        )
        self.service.register_service_provider(
            "service2", TestServiceProvider("Service2"), protocols=self.single_protocol
        )
        ...
        self.assertTrue(
            any(
                "Found 2 services implementing TestProtocol" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_discover_services_by_protocol_no_implementations(self):
        ...
        self.assertTrue(
            any(
                "No services found implementing TestProtocol" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_list_registered_services_with_services(self):
        ...
        for service_name in services:
            self.service.register_service_provider(
                service_name, TestServiceProvider(service_name)
            )
        ...
        self.assertTrue(
            any(
                "3 services registered" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_list_registered_services_empty_registry(self):
        ...
        self.assertTrue(
            any(
                "0 services registered" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_service_metadata_existing_service(self):
        ...
        self.assertTrue(
            any(
                "Retrieved metadata for service: test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_service_metadata_nonexistent_service(self):
        ...
        self.assertTrue(
            any(
                "No metadata found for service: nonexistent_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_update_service_metadata_existing_service(self):
        ...
        self.assertTrue(
            any(
                "Updated metadata for service: test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_update_service_metadata_nonexistent_service(self):
        ...
        # Act
        result = self.service.update_service_metadata(
            "nonexistent_service", {"test": "data"}
        )
        ...
        self.assertTrue(
            any(
                "Cannot update metadata for unregistered service: nonexistent_service"
                in call[1]
                for call in logger_calls
                if call[0] == "warning"
            )
    ...
    def test_get_service_protocols_with_protocols(self):
        ...
        # Arrange
        self.service.register_service_provider(
            "test_service", self.test_provider, protocols=self.multiple_protocols
        )
        ...
        self.assertTrue(
            any(
                "Service 'test_service' implements 3 protocols" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_get_service_protocols_no_protocols(self):
        ...
        self.assertTrue(
            any(
                "No protocols cached for service: test_service" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_unregister_service_existing_service(self):
        ...
        self.assertTrue(
            any(
                "Unregistered service: test_service" in call[1]
                for call in logger_calls
                if call[0] == "info"
            )
        )
        self.assertTrue(
            any(
                "Removed 3 protocol mappings" in call[1]
                for call in logger_calls
                if call[0] == "debug"
            )
        )
    ...
    def test_clear_registry(self):
        ...
        self.assertTrue(
            any(
                "Cleared registry: 2 services, 2 protocols" in call[1]
                for call in logger_calls
                if call[0] == "info"
            )
        )
    ...
    def test_error_handling_during_registration(self):
        ...
        with unittest.mock.patch.object(
            ...
            self.assertTrue(
                any(
                    "Failed to register service 'test_service'" in call[1]
                    for call in logger_calls
                    if call[0] == "error"
                )
            )
    ...
    def test_protocol_registration_consistency(self):
        ...
        self.service.register_service_provider(
            "service1", self.test_provider, protocols=[TestProtocol]
        )

        # Register different service with same protocol
        self.service.register_service_provider(
            "service2", TestServiceProvider("Service2"), protocols=[TestProtocol]
        )
    ...
    def test_cleanup_partial_registration(self):
        ...
        with unittest.mock.patch.object(
            self.service, "_is_valid_protocol", side_effect=failing_validation
        ):
