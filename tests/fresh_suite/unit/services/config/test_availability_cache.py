    ...
    def test_load_save_cache_roundtrip(self):
        ...
        cache_data = {
            ...
            "availability": {
                ...
                "vector": {
                    "enabled": False,
                    "last_error": "No vector store configured",
                },
            },
        }
...
class TestThreadSafetyConcurrentAccess(unittest.TestCase):
            ...
            try:
                for i in range(10):
                    cache_data = {
                        "worker_id": worker_id,
                        "iteration": i,
                        "config_hash": self.cache._get_full_config_hash(
                            self.test_config
                        ),
                        "generated_at": datetime.now(timezone.utc).isoformat(),
                    }
    ...
    def test_concurrent_cache_validation(self):
        ...
        for worker_id, iteration, is_valid in validation_results:
            self.assertTrue(
                is_valid,
                f"Validation failed for worker {worker_id} iteration {iteration}",
            )
...
class TestPerformanceRegression(unittest.TestCase):
    ...
    def test_cache_hit_performance(self):
        ...
        self.assertLess(
            avg_hit_time,
            1.0,
            f"Average cache hit time {avg_hit_time:.3f}ms exceeds 1ms target",
        )
        self.assertLess(
            max_hit_time,
            5.0,
            f"Max cache hit time {max_hit_time:.3f}ms exceeds 5ms threshold",
        )
    ...
    @unittest.skipIf(
        ...
        "psutil not available - install with 'pip install psutil' for on-demand memory testing",
    )
...
class TestAvailabilityCacheManager(unittest.TestCase):
    ...
    def test_get_availability_unknown_storage_type(self):
        ...
        async def run_test():
            result = await self.manager.get_or_generate_availability(
                "unknown_storage", self.test_config
            )
    ...
    def test_get_availability_cache_hit(self):
        ...
        async def run_test():
            ...
            result1 = await self.manager.get_or_generate_availability(
                "csv_storage", self.test_config
            )
            self.assertEqual(strategy.call_count, 1)

            # Second call should hit cache
            result2 = await self.manager.get_or_generate_availability(
                "csv_storage", self.test_config
            )
    ...
    def test_get_availability_concurrent_generation(self):
        ...
        strategy = MockValidationStrategy(
            "csv_storage", validation_delay=0.01
        )  # Reduced delay
        self.manager.register_validator("csv_storage", strategy)
        ...
        async def run_test():
            ...
            tasks = [
                self.manager.get_or_generate_availability(
                    "csv_storage", self.test_config
                )
