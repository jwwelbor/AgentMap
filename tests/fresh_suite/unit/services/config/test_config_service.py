...
class TestConfigService(unittest.TestCase):
    ...
    def test_get_value_from_config_nested(self):
        ...
        config_data = {"services": {"agent": {"config": {"timeout": 30, "retries": 3}}}}

        result = self.config_service.get_value_from_config(
            config_data, "services.agent.config.timeout"
        )
        self.assertEqual(result, 30)

        result = self.config_service.get_value_from_config(
            config_data, "services.agent.config.retries"
        )
    ...
    def test_get_value_from_config_default(self):
        ...
        result = self.config_service.get_value_from_config(
            config_data, "missing.section", "default_value"
        )
        self.assertEqual(result, "default_value")

        # Test missing path with no default returns None
        result = self.config_service.get_value_from_config(
            config_data, "missing.section"
        )
    ...
    def test_get_value_from_config_partial_path(self):
        ...
        result = self.config_service.get_value_from_config(
            config_data, "database.missing_key", "default"
        )
    ...
    def test_infrastructure_only_methods(self):
        ...
        actual_methods = set(
            method
            for method in dir(self.config_service)
