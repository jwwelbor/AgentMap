...
class TestCacheEntry(unittest.TestCase):
    ...
    def test_cache_entry_is_expired(self):
        ...
        entry = CacheEntry(
            decision=decision, timestamp=current_time - 400
        )  # 400 seconds ago
...
class TestRoutingCache(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cache = RoutingCache(
            logging_service=self.mock_logging_service, max_size=100, default_ttl=300
        )
    ...
    def test_cache_get_expired_entry(self):
        ...
        # Create a decision
        decision = RoutingDecision(
            provider="openai", model="gpt-4", complexity=TaskComplexity.HIGH
        )
    ...
    def test_cache_get_updates_access_order(self):
        ...
        decision1 = RoutingDecision(
            provider="openai", model="gpt-3.5", complexity=TaskComplexity.LOW
        )
        decision2 = RoutingDecision(
            provider="anthropic", model="claude", complexity=TaskComplexity.HIGH
        )

        self.cache.put("general", TaskComplexity.LOW, "prompt1", ["openai"], decision1)
        self.cache.put(
            "analysis", TaskComplexity.HIGH, "prompt2", ["anthropic"], decision2
        )
        ...
        key1 = self.cache._generate_cache_key(
            "general",
            TaskComplexity.LOW,
            self.cache._hash_prompt("prompt1"),
            ["openai"],
            [],
            True,
        )
    ...
    def test_cache_put_update_existing_entry(self):
        ...
        decision2 = RoutingDecision(
            provider="openai", model="gpt-4", complexity=TaskComplexity.LOW
        )
    ...
    def test_cache_put_triggers_eviction(self):
        ...
        decision1 = RoutingDecision(
            provider="p1", model="m1", complexity=TaskComplexity.LOW
        )
        decision2 = RoutingDecision(
            provider="p2", model="m2", complexity=TaskComplexity.LOW
        )
        decision3 = RoutingDecision(
            provider="p3", model="m3", complexity=TaskComplexity.LOW
        )
    ...
    def test_lru_eviction_order(self):
        ...
        decisions = [
            RoutingDecision(
                provider=f"p{i}", model=f"m{i}", complexity=TaskComplexity.LOW
            )
            for i in range(4)
        ]
        ...
        for i in range(3):
            self.cache.put(
                f"t{i}", TaskComplexity.LOW, f"prompt{i}", [f"p{i}"], decisions[i]
            )
    ...
    def test_remove_entry_cleanup(self):
        ...
        # Add entry manually
        self.cache._cache[test_key] = CacheEntry(
    ...
    def test_cache_hit_count_tracking(self):
        ...
        cache_key = self.cache._generate_cache_key(
            "test",
            TaskComplexity.LOW,
            self.cache._hash_prompt("prompt"),
            ["test"],
            [],
            True,
        )
    ...
    def test_cache_with_complex_routing_context(self):
        ...
        decision = RoutingDecision(
            provider="complex", model="model", complexity=TaskComplexity.HIGH
        )
