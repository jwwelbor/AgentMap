...
class TestModernizedVectorWriterAgent(unittest.TestCase):
    ...
    def test_execute_operation_calls_vector_service(self):
        ...
        inputs = {"docs": docs, "should_persist": False}
    ...
    def test_execute_operation_no_documents(self):
        ...
        expected_result = {"status": "error", "error": "No documents provided"}
        self.assertEqual(result, expected_result)
    ...
    def test_document_validation_success(self):
        ...
        inputs = {"docs": [{"text": "Document 1"}, {"text": "Document 2"}]}
    ...
    def test_logging_operation_start(self):
        ...
        collection = "test_collection"
        inputs = {"docs": [{"text": "Doc 1"}, {"text": "Doc 2"}, {"text": "Doc 3"}]}
        ...
        storage_logged = any(
            "Starting vector storage with 3 document(s)" in msg
            for msg in logged_messages
        )
        self.assertTrue(
            storage_logged,
            f"Expected vector storage operation logged, got: {logged_messages}",
        )
    ...
    def test_logging_single_document(self):
        ...
        logged_messages = [call[1] for call in debug_calls]
        storage_logged = any(
            "Starting vector storage with 1 document(s)" in msg
            for msg in logged_messages
        )
        self.assertTrue(
            storage_logged, f"Expected single document logged, got: {logged_messages}"
        )
    ...
    def test_logging_no_documents(self):
        ...
        logged_messages = [call[1] for call in debug_calls]
        storage_logged = any(
            "Starting vector storage with 0 document(s)" in msg
            for msg in logged_messages
        )
        self.assertTrue(
            storage_logged, f"Expected zero documents logged, got: {logged_messages}"
        )
    ...
    def test_service_configuration_via_protocol(self):
        ...
        self.assertEqual(agent.vector_service, self.mock_vector_service)
        self.assertEqual(
            agent._client, self.mock_vector_service
    ...
    def test_full_operation_workflow(self):
        ...
        inputs = {"docs": docs, "should_persist": True}
        ...
        self.mock_vector_service.write.assert_called_once_with(
            collection=collection, data=docs, mode=WriteMode.APPEND, should_persist=True
        )
