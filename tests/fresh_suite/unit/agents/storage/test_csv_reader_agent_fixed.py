...
class TestModernizedCSVReaderAgent(unittest.TestCase):
    ...
    def test_missing_collection_validation(self):
        ...
        with patch.object(agent, 'get_collection', side_effect=ValueError("Missing required 'collection' parameter")):
            inputs = {}
    ...
    def test_csv_extension_validation_warning(self):
        ...
        with patch.object(agent, 'get_collection', return_value='data.txt'):
            ...
            warning_logged = any("does not end with .csv" in call[1] for call in warning_calls)
            self.assertTrue(warning_logged, "Expected CSV extension warning to be logged")
    ...
    def test_generic_error_handling_delegates_to_base_agent(self):
        ...
        with patch.object(BaseStorageAgent, '_handle_operation_error') as mock_base_error:
            ...
            mock_base_error.assert_called_once_with(
                test_error,
                collection,
                inputs
            )
    ...
    def test_logging_integration(self):
        ...
        read_logged = any("Starting CSV read operation" in msg for msg in logged_messages)
        self.assertTrue(read_logged, f"Expected CSV read operation logged, got: {logged_messages}")
    ...
    def test_service_configuration_via_protocol(self):
        ...
        self.assertEqual(agent.csv_service, self.mock_csv_service)
        self.assertEqual(agent._client, self.mock_csv_service)  # Should also set as main client
