...
class TestFailureAgent(unittest.TestCase):
    ...
    def test_agent_initialization_with_minimal_dependencies(self):
        ...
        minimal_agent = FailureAgent(
            name="minimal_failure", prompt="Minimal failure prompt"
        )
    ...
    def test_process_with_multiple_inputs(self):
        ...
        inputs = {"operation": "backup_data", "target": "database", "format": "sql"}

        result = self.agent.process(inputs)

        # Should include all input keys (not values) in failure message
        self.assertIn(
            "test_failure executed (will set last_action_success=False)", result
        )
        ...
        test_cases = [
            {
                ...
                "prompt": None,
                "expected_parts": [
                    "test_agent executed (will set last_action_success=False)"
                ],
            },
            {
    ...
    def test_post_process_sets_failure_flag(self):
        ...
        result_state, result_output = self.agent._post_process(
            test_state, test_inputs, original_output
        )
    ...
    def test_get_child_service_info(self):
        ...
        self.assertEqual(
            behavior["post_process_behavior"], "sets_last_action_success_false"
        )
    ...
    def test_run_method_integration_with_failure_simulation(self):
        ...
        self.assertIn(
            "test_failure executed (will set last_action_success=False)",
            failure_message,
        )
