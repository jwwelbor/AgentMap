...
class TestInputAgent(unittest.TestCase):
    ...
    def test_agent_initialization_with_minimal_dependencies(self):
        ...
        minimal_agent = InputAgent(name="minimal_input", prompt="Enter something: ")
    ...
    def test_process_with_custom_prompt(self, mock_input):
        ...
        self.assertTrue(
            any("prompting for user input" in call[1] for call in info_calls)
        )
    ...
    def test_process_with_different_input_scenarios(self):
        ...
        test_cases = [
            ...
            {"name": "empty_input", "user_input": "", "expected": ""},
            {"name": "numeric_input", "user_input": "42", "expected": "42"},
            {
        ...
        for case in test_cases:
            with self.subTest(case=case["name"]):
                with patch(
                    "builtins.input", return_value=case["user_input"]
                ) as mock_input:
    ...
    def test_logging_integration(self, mock_input):
        ...
        prompt_logged = any(
            "test_input" in msg and "prompting for user input" in msg
            for msg in log_messages
        )
        self.assertTrue(
            prompt_logged, f"Expected prompting logged, got: {log_messages}"
        )
