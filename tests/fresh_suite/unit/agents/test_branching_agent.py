...
class TestBranchingAgent(unittest.TestCase):
    ...
    def test_agent_initialization_custom_configuration(self):
        ...
        self.assertEqual(agent.success_values, ["passed", "ok", "good"])  # Normalized to lowercase
    ...
    def test_determine_success_with_default_field_string_values(self):
        ...
        success_strings = ["true", "True", "TRUE", "yes", "Yes", "success", "succeed", "1", "t", "y"]
        ...
        failure_strings = ["false", "False", "FALSE", "no", "No", "failure", "fail", "0", "f", "n"]
    ...
    def test_automatic_success_field_from_input_fields(self):
        ...
        agent1 = self.create_branching_agent(
            input_fields=["status"]
        )
        ...
        agent3 = self.create_branching_agent(
            input_fields=[]
        )
    ...
    def test_custom_success_field_configuration(self):
        ...
        success = agent._determine_success({"primary_field": False, "secondary_field": True})
    ...
    def test_custom_fallback_fields_configuration(self):
        ...
        agent = self.create_branching_agent(
            success_field="primary",
            fallback_fields=["secondary", "tertiary"]
        )
    ...
    def test_determine_success_detailed_return_values(self):
        ...
        success, field_used, value_found = agent._determine_success_detailed({"success": True})
        self.assertTrue(success)
        self.assertEqual(field_used, "success")
        self.assertTrue(value_found)
        
        # Test with fallback field
        success, field_used, value_found = agent._determine_success_detailed({"succeed": "no"})
        self.assertFalse(success)
        self.assertEqual(field_used, "succeed")
        self.assertEqual(value_found, "no")
        
        # Test with no relevant field
        success, field_used, value_found = agent._determine_success_detailed({"other": "data"})
    ...
    def test_evaluate_value_method(self):
        ...
        agent = self.create_branching_agent(
            success_values=["GOOD", "OK"],
            failure_values=["BAD", "ERROR"]
        )
    ...
    def test_logging_integration_with_configuration(self):
        ...
        config_logged = any("BranchingAgent configured" in call[1] for call in debug_calls)
        self.assertTrue(config_logged, f"Expected configuration logged, got: {debug_calls}")
        
        # Should log decision details
        decision_logged = any("Decision based on field" in call[1] for call in info_calls)
