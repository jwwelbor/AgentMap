...
class TestCrossStorageWorkflows(BaseIntegrationTest):
    ...
    def setup_services(self):
        ...
        try:
            ...
            self.logging_service.get_class_logger(self).warning(
                f"Vector service not available: {e}"
            )
        ...
        return {
            ...
            "product_data": [
                ...
                },
                {
                    ...
                    "category": "Clothing",
                    "price": 299,
                    "description": "Stylish jacket for modern fashion",
                },
                {
                    "product_id": "PROD003",
                    "name": "Smart Speaker",
                ...
                },
                {
                    ...
                    "category": "Food",
                    "price": 25,
                    "description": "Healthy organic snack variety pack",
                },
            ],
        }
        ...
        for customer in csv_data:
            enriched_customer = {
                **customer,  # Preserve original data
                "spending_tier": self._calculate_spending_tier(
...
"""
        ...
        self.assertTrue(
            file_result.success, "Stage 3: File report generation should succeed"
        )

        # Verification: Validate end-to-end pipeline integrity
        final_report = self.file_service.read(
            f"customer_analytics_report_{pipeline_id}.txt", pipeline_id
        )
    ...
    def test_transaction_processing_workflow(self):
        ...
        for customer_id in customer_aggregates:
            customer_aggregates[customer_id]["categories"] = list(
                customer_aggregates[customer_id]["categories"]
            )
        ...
        analytics_report = {
            "summary": {
                ...
                "average_transaction": memory_data["total_amount"]
                / memory_data["total_transactions"],
            ...
            "top_customers": self._get_top_customers(
                memory_data["customer_aggregates"]
            ),
    ...
    def _get_top_customers(
        self, customer_aggregates: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def test_memory_cache_with_json_persistence(self):
        ...
        frequently_accessed_data = {
            ...
            "session_data": {
                "SESS001": {
                    "user_id": "USER001",
                    "login_time": "2025-06-01T10:00:00Z",
                    "last_activity": "2025-06-01T12:00:00Z",
                },
                "SESS002": {
                    "user_id": "USER002",
                    "login_time": "2025-06-01T11:00:00Z",
                    "last_activity": "2025-06-01T12:30:00Z",
                },
            },
            "cache_metadata": {
        ...
        memory_result = self.memory_service.write(
            collection="user_data_cache", data=json_data, document_id=cache_workflow_id
        )
        ...
        start_time = time.time()
        persistent_data = self.json_service.read(
            "persistent_user_data", cache_workflow_id
        )
        persistent_read_time = time.time() - start_time

        # Verify data consistency
        self.assertEqual(
            cached_data, persistent_data, "Cache and persistent data should match"
        )

        # Memory access should be faster (though timing can vary in tests)
        self.assertLessEqual(
            cache_read_time,
            persistent_read_time + 0.1,
        ...
        self.assertTrue(
            json_update_result.success, "Persistent storage sync should succeed"
        )
        ...
        final_persistent = self.json_service.read(
            "persistent_user_data", cache_workflow_id
        )
    ...
    def test_memory_cache_invalidation_and_refresh(self):
        ...
        self.json_service.write(
            "persistent_cache_data", initial_data, cache_invalidation_id
        )
        ...
        initial_persistent = self.json_service.read(
            "persistent_cache_data", cache_invalidation_id
        )
        self.assertEqual(
            initial_cached["version"],
            initial_persistent["version"],
            "Initial data should be synced",
        )
        ...
        self.json_service.write(
            "persistent_cache_data", updated_data, cache_invalidation_id
        )
        ...
        self.assertEqual(
            persistent_data["version"], 2, "Persistent storage should be version 2"
        )
        self.assertNotEqual(
            cached_data["version"],
        ...
        current_persistent = self.json_service.read(
            "persistent_cache_data", cache_invalidation_id
        )

        self.assertEqual(
            refreshed_cache["version"],
            current_persistent["version"],
            "Cache should be current after refresh",
        )
        self.assertEqual(
            len(refreshed_cache["data"]["items"]),
            5,
    ...
    def test_multi_tier_caching_workflow(self):
        ...
        large_dataset = {
            ...
            "sample_records": self.sample_datasets[
                "customer_data"
...
"""
        ...
        hot_cache_data = {
            ...
            "cache_hits": 0,
            "frequently_accessed_customers": large_dataset["sample_records"][
                :3
            ],  # Top 3
        }

        memory_result = self.memory_service.write(
            collection="hot_cache", data=hot_cache_data, document_id=multi_tier_id
        )
        ...
        archive_data = self.file_service.read(
            f"archive_{multi_tier_id}.txt", multi_tier_id
        )
        # FIX: File service returns a complex structure, extract content
    ...
    def test_data_migration_workflow(self):
        ...
        legacy_data = [
            {
                "id": "1",
                "name": "Legacy Record 1",
                "status": "active",
                "created": "2025-01-01",
            },
            {
                "id": "2",
                "name": "Legacy Record 2",
                "status": "inactive",
                "created": "2025-01-15",
            },
            {
                "id": "3",
                "name": "Legacy Record 3",
                "status": "active",
                "created": "2025-02-01",
            },
        ]
        ...
        csv_result = self.csv_service.write(
            collection="legacy_data", data=legacy_data, document_id=migration_id
        )
        ...
        if not csv_legacy:
            self.fail(
                f"CSV reading failed. DataFrame result: {csv_legacy_df}, type: {type(csv_legacy_df)}"
            )
        self.assertEqual(
            len(csv_legacy),
            3,
            f"Should have 3 legacy records, got {len(csv_legacy)}: {csv_legacy}",
        )
        ...
        json_result = self.json_service.write(
            collection="migrated_data", data=migrated_data, document_id=migration_id
        )
...
"""
        ...
        if "records" not in migrated_json:
            self.fail(
                f"JSON data missing 'records' key. Keys: {list(migrated_json.keys())}"
            )
        if not migrated_json["records"]:
            self.fail(f"JSON records list is empty. Full data: {migrated_json}")

        self.assertEqual(
            len(migrated_json["records"]), 3, "All records should be migrated"
        )

        # Migration report should exist
        report_content = self.file_service.read(
            f"migration_report_{migration_id}.txt", migration_id
        )
        # FIX: File service returns a complex structure, extract content
    ...
    def test_data_aggregation_and_reporting_workflow(self):
        ...
        self.assertTrue(
            transaction_result.success, "Transaction data storage should succeed"
        )
        ...
        for customer_id in customer_summary:
            customer_summary[customer_id]["categories"] = list(
                customer_summary[customer_id]["categories"]
            )

        for category in category_summary:
            category_summary[category]["unique_customers"] = list(
                category_summary[category]["unique_customers"]
            )
            category_summary[category]["unique_customer_count"] = len(
                category_summary[category]["unique_customers"]
            )
        ...
        aggregated_data = {
            ...
            "totals": {
                ...
                "total_revenue": sum(
                    txn["amount_numeric"] for txn in enriched_transactions
                ),
        ...
        memory_analytics = self.memory_service.read(
            "aggregated_analytics", aggregation_id
        )
...
"""
        ...
        for customer_id, data in memory_analytics["customer_summary"].items():
            ...
            report_sections.append(
                f"- {data['name']} ({customer_id}): ${data['total_spent']:,.2f} in {data['transaction_count']} transactions (avg: ${avg_transaction:.2f})"
            )
        ...
        for category, data in memory_analytics["category_summary"].items():
            ...
            report_sections.append(
                f"- {category}: ${data['total_revenue']:,.2f} from {data['unique_customer_count']} customers (avg per customer: ${avg_per_customer:.2f})"
            )
...
"""
        ...
        self.assertTrue(
            file_report_result.success, "Comprehensive report should be created"
        )

        # Verification: Validate complete workflow
        final_report = self.file_service.read(
            f"comprehensive_report_{aggregation_id}.txt", aggregation_id
        )
        # FIX: File service returns a complex structure, extract content
        if isinstance(final_report, list) and len(final_report) > 0:
            final_report_content = final_report[0].get("content", str(final_report))
        ...
        self.assertIn(
            "Alice Johnson", final_report_content
        )  # Should contain customer names
    ...
    def test_storage_cleanup_workflow(self):
        ...
        test_collections = {
            ...
            "csv": (
                "cleanup_csv_test",
                [{"id": cleanup_id, "temp_data": "csv_cleanup_test"}],
            ),
        ...
        for storage_type, (collection, data) in test_collections.items():
            ...
            self.assertTrue(
                result.success, f"Setup for {storage_type} cleanup test should succeed"
            )
        ...
        for storage_type, (collection, _) in test_collections.items():
            ...
            self.assertTrue(
                exists, f"Data should exist in {storage_type} before cleanup"
            )
        ...
        for storage_type, (collection, _) in test_collections.items():
            try:
                ...
                cleanup_results[storage_type] = {"success": False, "error": str(e)}

        # Verify cleanup results
        for storage_type, result_info in cleanup_results.items():
            self.assertTrue(
                result_info["success"], f"Cleanup should succeed for {storage_type}"
            )
...
"""
        ...
        final_report = self.file_service.read(
            f"cleanup_report_{cleanup_id}.txt", cleanup_id
        )
        # FIX: File service returns a complex structure, extract content
        if isinstance(final_report, list) and len(final_report) > 0:
            final_report_content = final_report[0].get("content", str(final_report))
    ...
    def test_resource_management_across_services(self):
        ...
        large_datasets = {
            ...
            "file_content": "\\n".join(
                [
                    f"Line {i}: File content for resource management test"
                    for i in range(100)
                ]
            ),
        }
        ...
        self.assertTrue(
            memory_result.success, "Memory resource test setup should succeed"
        )
        ...
        for service_name in ["memory", "json", "csv", "file"]:
            ...
            self.assertTrue(
                service_info[service_name]["available"],
...
"""
        ...
        self.assertTrue(
            report_result.success, "Resource management report should be created"
        )

        # Verify resource report
        final_report = self.file_service.read(
            f"resource_report_{resource_mgmt_id}.txt", resource_mgmt_id
        )
