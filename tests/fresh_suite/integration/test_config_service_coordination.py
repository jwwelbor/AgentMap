...
class TestConfigServiceCoordination(BaseIntegrationTest):
    ...
    def test_config_service_pure_infrastructure(self):
        ...
        simple_value = self.config_service.get_value_from_config(
            loaded_config, "simple.value"
        )
        self.assertEqual(simple_value, "test")

        nested_value = self.config_service.get_value_from_config(
            loaded_config, "nested.section.key"
        )
        self.assertEqual(nested_value, "nested_value")

        # Test default handling in infrastructure layer
        missing_value = self.config_service.get_value_from_config(
            loaded_config, "missing.path", "default"
        )
    ...
    def test_app_config_service_domain_logic(self):
        ...
        self.assertTrue(
            hasattr(self.app_config_service, "get_routing_config"),
            "AppConfigService should have domain-specific routing config method",
        )
        self.assertTrue(
            hasattr(self.app_config_service, "get_llm_config"),
        ...
        self.assertIsInstance(
            routing_config, dict, "Routing config should be dictionary"
        )
        ...
        self.assertIsInstance(
            validation_result, bool, "Config validation should return boolean"
        )
    ...
    def test_app_config_service_defaults_integration(self):
        ...
        partial_config = {
            ...
            "execution": {"max_retries": 5},  # Override default
        }
        ...
        self.assertEqual(
            routing_config["enabled"], False, "Should use configured value over default"
        )

        # Should have custom value
        self.assertEqual(
            routing_config["custom_setting"],
            "custom_value",
        ...
        self.assertIsInstance(
            execution_config, dict, "Execution config should be dictionary"
        )
    ...
    def test_config_loading_workflow_coordination(self):
        ...
        comprehensive_config = {
            ...
            "paths": {"custom_agents": "test_agents", "functions": "test_functions"},
        }
    ...
    def test_environment_specific_configuration_coordination(self):
        ...
        base_config = {
            "llm": {
                "anthropic": {"model": "claude-3-haiku-20240307", "temperature": 0.7}
            },
    ...
    def test_configuration_coordination_with_missing_sections(self):
        ...
        minimal_config = {
            "llm": {"anthropic": {"api_key": "test_key"}}
        ...
        missing_section = self.config_service.get_value_from_config(
            loaded_config, "routing.enabled"
        )
        self.assertIsNone(
            missing_section,
    ...
    def test_architectural_boundary_enforcement(self):
        ...
        config_service_methods = [
            method for method in dir(self.config_service) if not method.startswith("_")
        ]
    ...
    def test_domain_logic_separation(self):
        ...
        app_config_methods = [
            method
            for method in dir(self.app_config_service)
        ...
        self.assertTrue(
            hasattr(self.app_config_service, "get_value"),
            "AppConfigService should delegate infrastructure methods",
        )
        self.assertTrue(
            hasattr(self.app_config_service, "get_section"),
            "AppConfigService should delegate infrastructure methods",
        )
    ...
    def test_service_dependency_isolation(self):
        ...
        self.assertTrue(
            has_config_service_dep, "AppConfigService should depend on ConfigService"
        )
    ...
    def test_full_configuration_pipeline_coordination(self):
        ...
        try:
            pipeline_storage_config = StorageConfigService(
                self.config_service, storage_path
            )
            ...
            self.assertIn(
                "StorageConfigurationNotAvailableException", str(type(e).__name__)
            )
        ...
        if routing_matrix:  # May be empty in test environment
            ...
            available_providers = (
                self.llm_routing_config_service.get_available_providers()
            )
            self.assertIsInstance(
                available_providers, list, "Should get list of available providers"
            )

            # Should have at least one provider (from defaults or test config)
            if available_providers:
                self.assertIn(
                    "anthropic", available_providers, "Should have anthropic provider"
                )

                # Test that we can get models for complexity levels
                medium_model = self.llm_routing_config_service.get_model_for_complexity(
                    "anthropic", "medium"
                )
                self.assertIsNotNone(
                    medium_model, "Should get model for anthropic medium complexity"
                )

            # Test that we can get provider preferences
            provider_preferences = (
                self.llm_routing_config_service.get_provider_preference("general")
            )
            self.assertIsInstance(
                provider_preferences, list, "Should get provider preference list"
            )
