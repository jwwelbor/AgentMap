...
class TestConfigurationExamples(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_config_dir = (
            Path(__file__).parent.parent.parent.parent.parent / "temp_config_test"
        )
        self.examples_dir = (
    ...
    def _test_configuration_file(
        self, config_path: Path, test_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.assertIsInstance(
                host_config, dict, f"Host config should be dict for {test_name}"
            )
            ...
            self.assertIsInstance(
                enabled, bool, f"Host enabled should be bool for {test_name}"
            )
            ...
            if services:
                ...
                service_config = app_config_service.get_host_service_config(
                    first_service
                )
                self.assertIsInstance(
                    service_config,
                    dict,
                    f"Service config should be dict for {test_name}",
                )
            ...
            self.assertIsInstance(
                validation_result,
                dict,
                f"Validation result should be dict for {test_name}",
            )
            result["details"]["validation_passed"] = validation_result.get(
                "valid", False
            )
            result["details"]["validation_warnings"] = len(
                validation_result.get("warnings", [])
            )
            result["details"]["validation_errors"] = len(
    ...
    def test_example1_basic_host_config(self):
        ...
        self.assertTrue(
            details.get("host_application_enabled", False),
        ...
        self.assertTrue(
            details.get("has_env_vars", False), "Should contain environment variables"
        )
    ...
    def test_example2_comprehensive_host_config(self):
        ...
        result = self._test_configuration_file(
            config_path, "Comprehensive Host Configuration"
        )

        if not result["passed"]:
            self.fail(
                f"Comprehensive host configuration test failed: {result['errors']}"
            )

        # Verify multiple protocol folders
        details = result["details"]
        self.assertGreaterEqual(
            details.get("protocol_folders_count", 0),
            2,
    ...
    def test_example3_detailed_service_config(self):
        ...
        result = self._test_configuration_file(
            config_path, "Detailed Service Configuration"
        )
        ...
        details = result["details"]
        self.assertTrue(
            details.get("service_config_loaded", False),
    ...
    def test_example4_environment_variables(self):
        ...
        if not result["passed"]:
            self.fail(
                f"Environment variables configuration test failed: {result['errors']}"
            )

        # Verify environment variables are present
        details = result["details"]
        self.assertTrue(
            details.get("has_env_vars", False),
    ...
    def test_example6_debug_logging(self):
        ...
        result = self._test_configuration_file(
            config_path, "Debug Logging Configuration"
        )
    ...
    def test_all_examples_comprehensive_report(self):
        ...
        test_files = [
            ...
            (
                "example2_comprehensive_host_config.yaml",
        ...
        if working_config_path.exists():
            result = self._test_configuration_file(
                working_config_path, "Working Example"
            )
            results.append(result)
...
"""
        ...
        if failed_tests > 0:
            self.fail(
                f"{failed_tests} configuration examples failed verification. See report above."
            )
