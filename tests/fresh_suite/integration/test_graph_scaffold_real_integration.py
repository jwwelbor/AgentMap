...
2. Real PromptManagerService integration  
...
class TestGraphScaffoldServiceIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Use REAL PromptManagerService for true integration testing
        ...
        # Initialize service with real PromptManager
        self.service = GraphScaffoldService(
            app_config_service=self.mock_app_config_service,
            logging_service=self.mock_logging_service,
            prompt_manager=self.prompt_manager_service,  # REAL service!
    ...
    def test_scaffold_agent_with_real_template(self):
        ...
        self.assertIn("class TestAgentAgent(BaseAgent", content)
        
        # Verify real template structure
        self.assertIn("# Auto-generated agent class for TestAgent", content)
        self.assertIn("# Generated by AgentMap GraphScaffoldService", content)
        self.assertIn("from agentmap.agents.base_agent import BaseAgent", content)
        
        # Verify service dependencies were injected
        self.assertIn("LLMServiceUser", content)
        self.assertIn("CSVServiceUser", content)
        self.assertIn("from agentmap.services import LLMServiceUser", content)
        ...
        template_vars = [var for var in unsubstituted_vars 
                        if not var.startswith(('e}', '"', "'", 'f"'))]
        self.assertEqual(len(template_vars), 0, 
                       f"Unsubstituted template variables: {template_vars}")
    ...
    def test_scaffold_from_real_csv_with_templates(self):
        ...
        # Verify processor agent has service dependencies
        processor_content = processor_agent.read_text()
        self.assertIn("class ProcessorAgent(BaseAgent", processor_content)
        self.assertIn("LLMServiceUser", processor_content)
        self.assertIn("JSONServiceUser", processor_content)
        self.assertIn("MemoryServiceUser", processor_content)
        self.assertIn("with llm, json, memory capabilities", processor_content)
    ...
    def test_scaffold_with_edge_functions_real_templates(self):
        ...
        self.assertIn("JSONServiceUser", agent_content)
        ...
        # Verify real function template structure
        self.assertIn("def validate_input(state:", validate_content)
        self.assertIn("# Auto-generated edge function", validate_content)
        self.assertIn("ProcessNode", validate_content)  # Success route
        
        self.assertIn("def handle_validation_error(state:", error_content)
        self.assertIn("# Auto-generated edge function", error_content)
    ...
    def test_real_template_error_handling(self):
        ...
        self.assertIn("Unknown services", str(cm.exception))
    ...
    def test_real_template_service_integration(self):
        ...
        expected_protocols = [
            "LLMServiceUser", "CSVServiceUser", "JSONServiceUser", 
            "FileServiceUser", "VectorServiceUser", "MemoryServiceUser"
        ]
        ...
        expected_imports = [
            ...
            "from agentmap.services import CSVServiceUser",
            "from agentmap.services import VectorServiceUser"
        ]
        ...
        # Verify usage examples for key services
        self.assertIn("call_llm", content)  # LLM usage
        self.assertIn("csv_service.read", content)  # CSV usage
        self.assertIn("vector_service.read", content)  # Vector usage
