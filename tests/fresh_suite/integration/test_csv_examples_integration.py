...
class TestCSVExamplesIntegration(BaseIntegrationTest):
    ...
    def create_csv_from_documentation(self, csv_content: str, filename: str = None) -> Path:
        """
    ...
    def execute_workflow_end_to_end(self, csv_path: Path, graph_name: str, 
                                  initial_state: Dict[str, Any] = None) -> ExecutionResult:
        """
    ...
    def assert_workflow_success(self, result: ExecutionResult, 
                              expected_graph_name: str = None) -> None:
        """
        ...
        if expected_graph_name:
            self.assertEqual(result.graph_name, expected_graph_name, 
                           "Graph name should match expected")
        
        # Verify duration field exists (handle both possible field names)
        duration_exists = (hasattr(result, 'total_duration') or 
    ...
    def test_example_directory_accessibility(self):
        ...
        self.assertGreater(len(accessible_core), 0, 
                         f"At least one core example should be accessible: {core_examples}")
        
        print(f"✅ Example directory accessibility verified: {len(accessible_files)} files accessible")
    ...
    def test_physical_csv_examples_parametrized(self):
        ...
        for csv_filename in csv_files:
            with self.subTest(csv_file=csv_filename):
                try:
                    ...
                    result = self.execute_workflow_end_to_end(csv_path, main_graph_name, initial_state)
                    
                    # Verify workflow result structure
                    self.assert_workflow_success(result, main_graph_name)
                    print(f"✅ Execution: {csv_filename} workflow executed successfully")
        ...
        self.assertGreater(len(successful_core), 0, 
    ...
    def test_linear_graph_workflow(self):
        ...
        try:
            ...
            result = self.execute_workflow_end_to_end(csv_path, "LinearGraph", initial_state)
            
            # Verify linear workflow results
            self.assert_workflow_success(result, "LinearGraph")
            self.assertIsNotNone(result.final_state, "Linear workflow should have final state")
    ...
    def test_branching_graph_workflows(self):
        ...
        for csv_filename in branching_files:
            try:
                ...
                result = self.execute_workflow_end_to_end(csv_path, main_graph, initial_state)
        ...
        self.assertGreater(len(tested_files), 0, "At least one branching example should be testable")
    ...
    def test_llm_workflow_examples(self):
        ...
        for csv_filename in llm_files:
            try:
                ...
                with patch('agentmap.services.llm_service.LLMService') as mock_llm_class:
                    ...
                    self.assertIsInstance(result, ExecutionResult, "Should return ExecutionResult")
                    self.assertEqual(result.graph_name, main_graph, "Graph name should match")
