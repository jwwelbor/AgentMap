...
class TestAuthenticationEndpoints(BaseAPIIntegrationTest):
    ...
    def create_api_key_auth_service(
        self, api_key: str, permissions: list = None
    ) -> AuthService:
    ...
    def test_permission_based_access_control(self):
        ...
        readonly_auth_service = self.create_api_key_auth_service(
            self.valid_api_key, permissions=["read"]  # Only read permission
        )
        ...
        self.assertTrue(
            readonly_auth_service.validate_permissions(auth_context, ["read"])
        )
        self.assertFalse(
            readonly_auth_service.validate_permissions(auth_context, ["write"])
        )
    ...
    def test_admin_permission_full_access(self):
        ...
        self.assertTrue(
            admin_auth_service.validate_permissions(auth_context, ["write"])
        )
        self.assertTrue(
            admin_auth_service.validate_permissions(auth_context, ["execute"])
        )
        self.assertTrue(
            admin_auth_service.validate_permissions(auth_context, ["admin"])
        )
    ...
    def test_auth_service_functionality(self):
        ...
        self.assertTrue(
            enabled_service.validate_permissions(admin_context, ["read", "write"])
        )
        ...
        self.assertTrue(
            enabled_service.validate_permissions(readonly_context, ["read"])
        )
        self.assertFalse(
            enabled_service.validate_permissions(readonly_context, ["write"])
        )
    ...
    def test_auth_service_unavailable(self):
        ...
        mock_container = Mock()
        mock_container.auth_service.side_effect = RuntimeError(
