...
class BaseIntegrationTest(unittest.TestCase):
            ...
            "kv": {
                ...
                "collections": {
                    "test_kv": {
                        ...
                        "settings": {
                            "file_path": str(
                                Path(self.temp_dir) / "storage" / "kv" / "kv_data.json"
                            )
                        },
                    }
                },
            },
            "json": {"default_directory": "json", "collections": {}},
            "file": {"default_directory": "file", "collections": {}},
        }
    ...
    def create_test_csv_file(
        self, content: str, filename: str = "test_graph.csv"
    ) -> Path:
        """
...
"""
    ...
    def assert_service_created(self, service: Any, expected_class_name: str) -> None:
        ...
        self.assertGreater(
            len(service_info.public_methods),
            0,
    ...
    def assert_file_exists(self, file_path: Path, description: str = "File") -> None:
        ...
        """
        self.assertTrue(
            file_path.exists(), f"{description} should exist at: {file_path}"
        )
        self.assertTrue(
            file_path.is_file(), f"{description} should be a file: {file_path}"
        )
    ...
    def assert_directory_exists(
        ...
        """
        self.assertTrue(dir_path.exists(), f"{description} should exist at: {dir_path}")
        self.assertTrue(
            dir_path.is_dir(), f"{description} should be a directory: {dir_path}"
        )

    def assert_graphs_equivalent(
        self, graph1, graph2, message_prefix: str = "Graphs"
    ) -> None:
        """
        ...
        """
        self.assertEqual(
            graph1.name, graph2.name, f"{message_prefix} names should match"
        )
        self.assertEqual(
            graph1.entry_point,
            graph2.entry_point,
            f"{message_prefix} entry points should match",
        )
        self.assertEqual(
            len(graph1.nodes),
            len(graph2.nodes),
            f"{message_prefix} node counts should match",
        )
        ...
        for node_name in graph1.nodes:
            self.assertIn(
                node_name,
                graph2.nodes,
                f"Node '{node_name}' should exist in both graphs",
            )
            ...
            self.assertEqual(
                node1.name, node2.name, f"Node {node_name} names should match"
            )
            self.assertEqual(
                node1.agent_type,
                node2.agent_type,
                f"Node {node_name} agent types should match",
            )
            self.assertEqual(
                node1.prompt, node2.prompt, f"Node {node_name} prompts should match"
            )
            self.assertEqual(
                node1.inputs, node2.inputs, f"Node {node_name} inputs should match"
            )
            self.assertEqual(
                node1.output, node2.output, f"Node {node_name} outputs should match"
            )
            self.assertEqual(
                node1.edges, node2.edges, f"Node {node_name} edges should match"
            )
    ...
    def assert_service_has_logging(self, service: Any, service_name: str) -> None:
        ...
        has_logger = (
            (hasattr(service, "logger") and service.logger is not None)
            or (hasattr(service, "_logger") and service._logger is not None)
            or (
                hasattr(service, "_bootstrap_logger")
                and service._bootstrap_logger is not None
            )
        )
        ...
        logger_instance = (
            ...
            or getattr(service, "_logger", None)
            or getattr(service, "_bootstrap_logger", None)
        )
        self.assertIsNotNone(
            logger_instance, f"{service_name} logger instance should not be None"
        )
        ...
        for method_name in basic_logging_methods:
            self.assertTrue(
                hasattr(logger_instance, method_name),
