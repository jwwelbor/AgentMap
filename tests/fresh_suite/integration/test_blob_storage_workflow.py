...
"""
    ...
    def test_application_container_blob_agent_registration(self):
        ...
        if blob_service is not None:
            ...
            from agentmap.agents.builtins.storage.blob.blob_reader_agent import (
                BlobReaderAgent,
            )
            from agentmap.agents.builtins.storage.blob.blob_writer_agent import (
                BlobWriterAgent,
            )
            ...
            self.skipTest(
                "Blob storage service not available - skipping agent registration test"
            )
        ...
        for size_name, test_data in test_cases:
            ...
            try:
                ...
                self.assertLess(
                    write_time,
                    10.0,
                    f"Write time for {size_name} data too slow: {write_time}s",
                )
                self.assertLess(
                    read_time,
                    10.0,
                    f"Read time for {size_name} data too slow: {read_time}s",
                )
    ...
    def test_blob_storage_configuration_flexibility(self):
        ...
        config_variations = [
            ...
            {
                "blob": {
                    "providers": {"s3": {"access_key": "test", "secret_key": "test"}}
                }
            },
        ]
...
class TestBlobStorageGracefulDegradation(unittest.TestCase):
    ...
    @patch(
        ...
        side_effect=ImportError("azure not found"),
    )
    @patch(
    ...
    @patch(
        ...
        side_effect=ImportError("google.cloud.storage not found"),
    )
    def test_graceful_degradation_all_cloud_missing(
        self,
        mock_gcs,
        mock_s3,
        mock_azure,
        mock_gcs_availability,
        mock_s3_availability,
        mock_azure_availability,
    ):
