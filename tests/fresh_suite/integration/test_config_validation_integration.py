...
class TestConfigValidationIntegration(BaseIntegrationTest):
    ...
    def test_cross_service_configuration_validation(self):
        ...
        self.assertEqual(loaded_config, valid_config, "ConfigService should load valid config correctly")
        ...
        self.assertTrue(app_validation_result, "AppConfigService validation should pass for valid config")
        ...
        try:
            storage_config = StorageConfigService(self.config_service, storage_config_path)
            
            # Validate storage config
            storage_validation = storage_config.validate_storage_config()
            self.assertIsInstance(storage_validation, dict, "Storage validation should return dict")
            self.assertIn('warnings', storage_validation, "Storage validation should include warnings")
            self.assertIn('errors', storage_validation, "Storage validation should include errors")
            
            # Valid config should have minimal errors
            self.assertEqual(len(storage_validation['errors']), 0, 
        ...
        routing_validation_errors = self.llm_routing_config_service.validate_AppConfigService()
        self.assertIsInstance(routing_validation_errors, list, "Routing validation should return list")
        self.assertEqual(len(routing_validation_errors), 0,
    ...
    def test_configuration_dependency_resolution(self):
        ...
        interdependent_config = {
            ...
            "routing": {
                ...
                "task_types": {
                    "general": {
                        "provider_preference": ["anthropic", "openai"],  # References LLM providers
                        "default_complexity": "medium"
                    },
                    "specialized": {
                        "provider_preference": ["anthropic"],  # References LLM providers
        ...
        self.assertIsInstance(routing_providers, list, "Should get list of available providers")
        ...
        general_providers = self.llm_routing_config_service.get_provider_preference("general")
        self.assertIsInstance(general_providers, list, "Should get provider preference list")
    ...
    def test_validation_error_dependency_propagation(self):
        ...
        loaded_config = self.config_service.load_config(config_path)
        self.assertEqual(loaded_config, error_config, "ConfigService should load config with errors")
        ...
        self.assertIsInstance(app_validation, bool, "AppConfigService validation should complete")
        ...
        if routing_validation_errors:
            ...
            self.assertTrue(len(routing_validation_errors) > 0, 
    ...
    def test_routing_validation_fail_fast_coordination(self):
        ...
        self.assertIsInstance(routing_config, dict, "AppConfigService should provide dict routing config")
        
        # User's invalid values should be preserved (AppConfigService doesn't do type validation)
        self.assertEqual(routing_config.get('routing_matrix'), "invalid_matrix_format",
                        "AppConfigService should preserve user config values even if invalid")
        self.assertEqual(routing_config.get('task_types'), [],
                        "AppConfigService should preserve user config values even if invalid")
        ...
        try:
            routing_validation_errors = self.llm_routing_config_service.validate_AppConfigService()
            self.assertIsInstance(routing_validation_errors, list, "Should return validation errors list")
            # If validation completes, there should be errors detected
            if routing_validation_errors:
                self.assertGreater(len(routing_validation_errors), 0, "Should detect routing configuration errors")
    ...
    def test_graceful_degradation_coordination(self):
        ...
        app_validation = app_config.validate_config()
        self.assertTrue(app_validation, "AppConfigService should validate partial config")
        
        # Should provide defaults for missing routing config
        routing_config = app_config.get_routing_config()
        self.assertIsInstance(routing_config, dict, "Should provide default routing config")
        self.assertIn('enabled', routing_config, "Should have default routing enabled setting")
    ...
    def test_configuration_error_propagation_chain(self):
        ...
        self.assertIsInstance(execution_config, dict, "Should return execution config dict")
        ...
        storage_error_config = {
            "storage_config_path": "/nonexistent/path/storage.yaml"
        }
    ...
    def test_comprehensive_validation_summary(self):
        ...
        self.assertTrue(app_summary["validation_passed"], "App config validation should pass")
        self.assertIsInstance(app_summary["config_summary"], dict, "Should have config summary")
        
        # Verify routing config service summary
        routing_summary = validation_summary["llm_routing_config_service"]
        self.assertIsInstance(routing_summary["validation_errors"], list, "Should have validation errors list")
        self.assertIsInstance(routing_summary["available_providers"], list, "Should have providers list")
        self.assertIsInstance(routing_summary["available_task_types"], list, "Should have task types list")
