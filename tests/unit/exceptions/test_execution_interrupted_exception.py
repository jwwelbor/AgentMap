...
class TestExecutionInterruptedException(unittest.TestCase):
    ...
    def test_exception_data_preservation(self):
        ...
        interaction_request = self._create_test_interaction_request(thread_id, "complex_node")
        ...
        self.assertEqual(checkpoint["agent_context"]["previous_nodes"], ["node1", "node2"])
        
        self.assertEqual(checkpoint["execution_tracker"]["node_count"], 5)
        self.assertEqual(checkpoint["execution_tracker"]["execution_path"], ["start", "process", "complex_node"])
        
        self.assertEqual(checkpoint["state"]["data_buffer"], ["item1", "item2", "item3"])
    ...
    def test_exception_with_minimal_data(self):
        ...
        self.assertEqual(exception.interaction_request.interaction_type, InteractionType.APPROVAL)
        self.assertEqual(exception.checkpoint_data, {})

    def test_exception_message_format(self):
        """Test exception message formatting with different thread IDs."""
        test_cases = [
            ("simple_thread", "Execution interrupted for human interaction in thread: simple_thread"),
            ("thread_with_numbers_123", "Execution interrupted for human interaction in thread: thread_with_numbers_123"),
            ("thread-with-dashes", "Execution interrupted for human interaction in thread: thread-with-dashes"),
            ("thread_with_underscores_and_numbers_456", "Execution interrupted for human interaction in thread: thread_with_underscores_and_numbers_456")
        ]
    ...
    def test_exception_attribute_access(self):
        ...
        self.assertEqual(exception.interaction_request.thread_id, thread_id)
        self.assertEqual(exception.interaction_request.interaction_type, InteractionType.TEXT_INPUT)
