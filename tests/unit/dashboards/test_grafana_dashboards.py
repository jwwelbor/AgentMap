...
MODERN_PANEL_TYPES = {
    "timeseries", "stat", "gauge", "table", "heatmap",
    "traces", "nodeGraph", "row", "text",
}
...
class TestAC1_JsonStructuralValidity:
    ...
    def test_llm_ops_has_templating(self, llm_ops_dashboard: dict) -> None:
        ...
        required = {
            "datasource", "provider", "model", "interval",
            "cost_per_input_token", "cost_per_output_token",
        }
        assert required.issubset(var_names), (
    ...
    def test_workflow_has_templating(self, workflow_dashboard: dict) -> None:
        ...
        var_names = {v["name"] for v in tlist}
        required = {
            "datasource_prometheus", "datasource_tempo",
            "workflow", "agent_type",
        }
        assert required.issubset(var_names), (
            f"Missing template variables: {required - var_names}"
...
class TestAC2_PrometheusMetricNames:
    ...
    def test_llm_ops_metric_names_valid(self, llm_ops_dashboard: dict) -> None:
        ...
        for expr in exprs:
            for match in metric_pattern.finditer(expr):
                name = match.group(1)
                assert name in VALID_AGENTMAP_METRICS, (
        ...
        for expr in exprs:
            for base in counter_bases:
                ...
                if base in expr:
                    ...
                    assert not pattern.search(expr), (
    ...
    def test_updowncounter_no_total_suffix(self, llm_ops_dashboard: dict) -> None:
        ...
        exprs = _extract_promql_exprs(llm_ops_dashboard)
        for expr in exprs:
            assert "agentmap_llm_circuit_breaker_total" not in expr, (
    ...
    def test_no_dot_notation_metrics(self, llm_ops_dashboard: dict) -> None:
        ...
        for expr in exprs:
            ...
            assert "agentmap.llm" not in expr, (
...
class TestAC4_TraceQLCorrectness:
    ...
    def test_workflow_traces_query_correct_span(
        self, workflow_dashboard: dict
    ) -> None:
        """Trace search filters on agentmap.workflow.run span name."""
        exprs = _extract_traceql_exprs(workflow_dashboard)
        found_workflow = any(
            "agentmap.workflow.run" in expr for expr in exprs
        )
        assert found_workflow, (
            "No TraceQL query references agentmap.workflow.run"
        )
    ...
    def test_traceql_attributes_valid(self, workflow_dashboard: dict) -> None:
        ...
        for expr in exprs:
            for match in attr_pattern.finditer(expr):
                attr = match.group(1)
                assert attr in VALID_SPAN_ATTRIBUTES, (
...
class TestAC5_DatasourcePortability:
    ...
    def test_llm_ops_no_hardcoded_uids(self, llm_ops_dashboard: dict) -> None:
        ...
        for uid in uids:
            assert uid.startswith("${"), (
                f"Hardcoded datasource UID found: '{uid}'"
            )
    ...
    def test_llm_ops_inputs_declare_prometheus(
        self, llm_ops_dashboard: dict
    ) -> None:
    ...
    def test_workflow_inputs_declare_both(
        self, workflow_dashboard: dict
    ) -> None:
...
class TestAC6_CostEstimation:
    ...
    def test_cost_panels_use_variables(self, llm_ops_dashboard: dict) -> None:
        ...
        cost_panels = [
            p for p in panels
            if p.get("title", "").lower().startswith(("estimated cost", "total estimated"))
        ]
        ...
        for panel in cost_panels:
            ...
            for target in targets:
                ...
                if expr:
                    assert "$cost_per_input_token" in expr or "$cost_per_output_token" in expr, (
...
class TestAC8_ProvisioningConfig:
    ...
    def test_dashboards_has_providers(self) -> None:
        ...
        found_path = any(
            p.get("options", {}).get("path") for p in providers
        )
...
class TestAC9_DocumentationCompleteness:
    ...
    def test_readme_has_ui_import(self) -> None:
        ...
        assert re.search(r"(step|1\.|1\))", content), (
...
class TestAC10_MetricDimensionFiltering:
    ...
    def test_metrics_with_provider_have_filter(
        self, llm_ops_dashboard: dict
    ) -> None:
        ...
        for expr in exprs:
            ...
            for metric in METRICS_WITH_PROVIDER_MODEL:
                if metric in expr:
                    ...
                    assert 'provider=~"$provider"' in expr or 'provider=~' in expr, (
                        f"Metric '{metric}' missing provider filter in: {expr}"
                    )

    def test_metrics_without_provider_no_filter(
        self, llm_ops_dashboard: dict
    ) -> None:
...
class TestAC11_ModernPanelTypes:
    ...
    def test_llm_ops_no_deprecated(self, llm_ops_dashboard: dict) -> None:
        ...
        for panel in panels:
            ...
            assert ptype not in DEPRECATED_PANEL_TYPES, (
                f"Deprecated panel type '{ptype}' in panel '{panel.get('title')}'"
            )
            if ptype != "row":
                assert ptype in MODERN_PANEL_TYPES, (
...
class TestPanelCounts:
    ...
    def test_llm_ops_panel_count(self, llm_ops_dashboard: dict) -> None:
        ...
        assert len(panels) == 22, (
            f"Expected 22 panels, got {len(panels)}"
        )
    ...
    def test_workflow_panel_count(self, workflow_dashboard: dict) -> None:
        ...
        assert len(panels) == 15, (
            f"Expected 15 panels, got {len(panels)}"
        )
...
class TestRequiredPanels:
    ...
    def test_llm_ops_required_panels_present(
        self, llm_ops_dashboard: dict
    ) -> None:
    ...
    def test_workflow_required_panels_present(
        self, workflow_dashboard: dict
    ) -> None:
