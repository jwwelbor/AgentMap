...
class TestServiceRequirementParser(unittest.TestCase):
    ...
    def test_parse_services_with_json_context_llm_service(self):
        ...
        self.assertEqual(
            result.imports, ["from agentmap.services.protocols import LLMCapableAgent"]
        )
        ...
        self.assertEqual(
            llm_attr.documentation, "LLM service for calling language models"
        )
    ...
    def test_parse_services_with_storage_service_unified_approach(self):
        ...
        self.assertEqual(
            storage_attr.documentation,
            "Generic storage service (supports all storage types)",
        )
    ...
    def test_parse_services_with_specific_storage_types(self):
        ...
        self.assertIn(
            "from agentmap.services.protocols import CSVCapableAgent", result.imports
        )
        self.assertIn(
            "from agentmap.services.protocols import JSONCapableAgent", result.imports
        )
    ...
    def test_extract_services_list_handles_various_formats(self):
        ...
        # Test dict
        self.assertEqual(
            self.parser._extract_services_list({"services": ["llm"]}), ["llm"]
        )

        # Test JSON string
        self.assertEqual(
            self.parser._extract_services_list('{"services": ["llm", "storage"]}'),
            ["llm", "storage"],
        )
