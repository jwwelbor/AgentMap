...
class TestBundleUpdateService(unittest.TestCase):
    ...
    def _create_test_declaration(
        self, agent_type: str, class_path: str, source: str = "test_source"
    ) -> AgentDeclaration:
        """Create a test agent declaration."""
        return AgentDeclaration(
            agent_type=agent_type, class_path=class_path, source=source
        )
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.bundle_update_service.file_path_service, self.mock_file_path_service
        )
    ...
    def test_update_bundle_with_missing_declarations_resolved(self):
        ...
        self.assertEqual(
            bundle.agent_mappings["missing_agent"], "agentmap.agents.MissingAgent"
        )
        self.assertEqual(
            bundle.agent_mappings["another_missing"], "agentmap.agents.AnotherAgent"
        )
    ...
    def test_update_bundle_with_changed_class_paths(self):
        ...
        self.assertEqual(result, bundle)
        self.assertEqual(
            bundle.agent_mappings["existing_agent"], "new.path.ExistingAgent"
        )
        self.assertEqual(
            bundle.agent_mappings["unchanged_agent"], "correct.path.UnchangedAgent"
        )
    ...
    def test_update_service_requirements(self):
        ...
        # Arrange
        bundle = self._create_test_bundle(
            missing_declarations={"new_agent"}, required_services={"existing_service"}
        )
    ...
    def test_persistence_flag_true_with_changes(self):
        ...
        # Arrange
        bundle = self._create_test_bundle(missing_declarations={"new_agent"})
    ...
    def test_no_persistence_when_no_changes(self):
        ...
        self.mock_declaration_registry.get_agent_declaration.return_value = (
            existing_decl
        )
        self.mock_declaration_registry.resolve_agent_requirements.return_value = {
            "services": set(),
    ...
    def test_last_updated_timestamp_set(self):
        ...
        # Act
        with patch(
            "agentmap.services.graph.bundle_update_service.datetime"
        ) as mock_datetime:
            mock_now = Mock()
    ...
    def test_get_update_summary_preview(self):
        ...
        updated_decl = self._create_test_declaration("existing_agent", "new.path.Agent")
    ...
    def test_complex_update_scenario(self):
        ...
        self.assertEqual(
            bundle.agent_mappings["unchanged_agent"], "agentmap.agents.UnchangedAgent"
        )

        # Updated agent should have new path
        self.assertIn("updated_agent", bundle.agent_mappings)
        self.assertEqual(
            bundle.agent_mappings["updated_agent"], "new.path.UpdatedAgent"
        )
