...
class CachePerformanceBenchmark:
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        return {
            ...
            "p95_ms": (
                statistics.quantiles(self.times, n=20)[18] * 1000
                if len(self.times) >= 20
                else max(self.times) * 1000
            ),
            "p99_ms": (
                statistics.quantiles(self.times, n=100)[98] * 1000
...
class UnifiedCachePerformanceTests(unittest.TestCase):
    ...
    def test_cache_miss_impact_measurement(self):
        ...
        self.assertLess(
            stats["mean_ms"], 500.0, "Cache miss with work should complete within 200ms"
        )
    ...
    def test_unified_vs_separate_cache_overhead(self):
        ...
        for category in categories:
            cache_file = (
                Path(self.temp_dir) / f"cache_{category.replace('.', '_')}.json"
            )
            separate_caches[category] = AvailabilityCacheService(
                cache_file, self.logger
            )
        ...
        print(
            f"Separate Caches - Mean: {separate_stats['mean_ms']:.2f}ms, P95: {separate_stats['p95_ms']:.2f}ms"
        )
        ...
        separate_total_size = sum(
            ...
            for cache_file in [
                Path(self.temp_dir) / f"cache_{cat.replace('.', '_')}.json"
        ...
        print(
            f"Storage efficiency: {((separate_total_size - unified_size) / separate_total_size * 100):.1f}% reduction"
        )
    ...
    def test_service_integration_performance(self):
        ...
        dependency_checker = DependencyCheckerService(
            self.logging_service, self.features_registry_service, cache_service
        )
        ...
        for storage_type in storage_types:
            ...
            # Result should be cached now
            cached_result = cache_service.get_availability(
        ...
        is_ci = (
            ...
            or os.getenv("CONTINUOUS_INTEGRATION") is not None
            or "pytest" in sys.modules  # Running under pytest often indicates CI
        )
        ...
        print(
            f"Environment detection: CI={os.getenv('CI')}, GITHUB_ACTIONS={os.getenv('GITHUB_ACTIONS')}"
        )
        print(
            f"Performance result: P95={stats['p95_ms']:.1f}ms (threshold: {max_time_ms}ms)"
        )
        self.assertLess(
            stats["p95_ms"],
            max_time_ms,
                ...
                try:
                    ...
                    if result is None:
                        thread_errors.append(
                            f"Thread {thread_id}: Unexpected None result"
                        )
    ...
    def test_cache_invalidation_performance(self):
        ...
        self.assertLess(key_stats["mean_ms"], 50.0, "Key invalidation should be fast")
        self.assertLess(
            category_stats["mean_ms"],
            100.0,
    ...
    def test_startup_performance_comparison(self):
        ...
        dependency_checker_cold = DependencyCheckerService(
            self.logging_service, self.features_registry_service, cold_cache
        )
        ...
        dependency_checker_warm = DependencyCheckerService(
            self.logging_service, self.features_registry_service, warm_cache
        )
        ...
        self.assertGreater(
            speedup_ratio, 2.0, "Warm start should be at least 2x faster"
        )
    ...
    def test_memory_usage_patterns(self):
        ...
        try:
            ...
            self.skipTest(
                "psutil not available - install with 'pip install psutil' for memory usage testing"
            )
        ...
        print(
            f"File size efficiency: {((total_separate_size - unified_file_size) / total_separate_size * 100):.1f}% reduction"
        )
    ...
    def test_cross_service_cache_benefits(self):
        ...
        for provider in providers:
            ...
            if not routing_decision["cache_hit"]:
                self.fail(
                    f"Unexpected cache miss for provider {provider}. Cached result: {cached_result}"
                )
        ...
        print(
            f"Cache reuse speedup: {(cross_stats['mean_ms'] / routing_stats['mean_ms']):.1f}x"
        )

        # Cache reuse should be much faster
        speedup = cross_stats["mean_ms"] / routing_stats["mean_ms"]
        self.assertGreater(
            speedup, 5.0, "Cross-service cache reuse should provide significant speedup"
        )
    ...
    def test_file_io_patterns(self):
        ...
        self.assertLess(
            write_stats["mean_ms"], 50.0, "Write operations should be reasonably fast"
        )
        self.assertLess(
            read_stats["mean_ms"], 10.0, "Read operations should be very fast"
        )
    ...
    def test_performance_regression_benchmarks(self):
        ...
        for scenario_name, config in scenarios.items():
            ...
            if "hit" in scenario_name:
                ...
                cache_service.set_availability(
                    config["category"], config["key"], test_data
                )

                # Benchmark hits
                for _ in range(config["iterations"]):
                    benchmark.start()
                    result = cache_service.get_availability(
                ...
                for i in range(config["iterations"]):
                    ...
                    benchmark.start()
                    success = cache_service.set_availability(
                        config["category"], f"{config['key']}_{i}", test_data
                    )
                    benchmark.stop()
                    self.assertTrue(success)
                ...
                for i in range(config["iterations"]):
                    benchmark.start()
                    cache_service.invalidate_cache(
                        config["category"], f"{config['key']}_{i}"
                    )
                    benchmark.stop()
        ...
        self.assertLess(
            hit_baseline["p95_ms"], 10.0, "Cache hit P95 baseline should be under 10ms"
        )
