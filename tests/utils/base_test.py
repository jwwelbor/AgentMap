...
class BaseUnitTest:
    ...
    def assert_mock_called_with_args(self, mock_method, expected_args: tuple, expected_kwargs: dict = None):
    ...
    def assert_service_interaction(self, service_mock: Mock, method_name: str, call_count: int = 1):
        """Assert that a service method was called the expected number of times."""
        method_mock = getattr(service_mock, method_name)
        assert method_mock.call_count == call_count, f"{method_name} was called {method_mock.call_count} times, expected {call_count}"
...
class AgentTestMixin:
    ...
    def create_test_agent(
        ...
        **kwargs
        ...
        # For integration tests, use real services
            ...
            raise ValueError("Test class must inherit from BaseUnitTest or BaseIntegrationTest")
    ...
    def run_agent_and_assert_success(self, agent, input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def assert_agent_state(self, agent, expected_state: Dict[str, Any]):
        ...
        for key, expected_value in expected_state.items():
            ...
            assert actual_value == expected_value, f"Agent.{key} = {actual_value}, expected {expected_value}"
...
class ServiceTestMixin:
    ...
    def assert_service_configured(self, service, expected_config_keys: list):
        ...
        for key in expected_config_keys:
            assert hasattr(service, key) or hasattr(service, f"_{key}"), f"Service missing {key}"
    ...
    def assert_service_method_exists(self, service, method_name: str):
        ...
        assert callable(getattr(service, method_name)), f"Service.{method_name} is not callable"
    
    def mock_service_method_response(self, service_mock: Mock, method_name: str, response: Any):
