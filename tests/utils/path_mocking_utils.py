...
class PathExistsMocker:
    ...
    def __init__(self, path_existence_map: Dict[Union[str, Path], bool]):
        ...
        self.path_map = {str(path): exists for path, exists in path_existence_map.items()}
...
class PathStatMocker:
    ...
    def __init__(self, path_timestamp_map: Dict[Union[str, Path], float]):
        ...
        """
        # Normalize all paths to strings for consistent comparison
        self.path_map = {str(path): timestamp for path, timestamp in path_timestamp_map.items()}
...
class PathOperationsMocker:
    ...
    def set_exists(self, path: Union[str, Path], exists: bool) -> 'PathOperationsMocker':
        """Set existence status for a path. Returns self for chaining."""
        self.existence_map[str(path)] = exists
        return self
    
    def set_stat(self, path: Union[str, Path], timestamp: float) -> 'PathOperationsMocker':
        """Set stat timestamp for a path. Returns self for chaining."""
        self.timestamp_map[str(path)] = timestamp
        return self
    
    def set_file_newer_than(self, newer_path: Union[str, Path], older_path: Union[str, Path], 
                           newer_time: float = 1642000000, older_time: float = 1641000000) -> 'PathOperationsMocker':
    ...
    def set_compilation_scenario(self, output_path: Union[str, Path], csv_path: Union[str, Path],
                                is_current: bool = True) -> 'PathOperationsMocker':
...
@contextmanager
def mock_file_comparison(newer_file: Union[str, Path], older_file: Union[str, Path],
                        newer_time: float = 1642000000, older_time: float = 1641000000):
...
@contextmanager
def mock_compilation_currency(output_path: Union[str, Path], csv_path: Union[str, Path], 
...
class MockServiceConfigHelper:
    ...
    def configure_app_config_service(mock_service, config_dict: Dict[str, str]):
        ...
        if 'csv_path' in config_dict:
            ...
            mock_service.get_csv_path.return_value = csv_path
...
def enhance_mock_app_config_service(mock_service, config_dict: Dict[str, str]):
    ...
    """
    return MockServiceConfigHelper.configure_app_config_service(mock_service, config_dict)
