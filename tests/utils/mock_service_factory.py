...
class MockServiceFactory:
    ...
    def create_mock_logging_service(logger_name: str = "test") -> Mock:
        ...
        def create_mock_logger(name: str) -> Mock:
            ...
            mock_logger.debug.side_effect = lambda msg, *args, **kwargs: track_log_call(
                "debug", msg, *args, **kwargs
            )
            mock_logger.info.side_effect = lambda msg, *args, **kwargs: track_log_call(
                "info", msg, *args, **kwargs
            )
            mock_logger.warning.side_effect = (
                lambda msg, *args, **kwargs: track_log_call(
                    "warning", msg, *args, **kwargs
                )
            )
            # Create a new Mock for error method that preserves call_args_list
            mock_logger.error = Mock(
                side_effect=lambda msg, *args, **kwargs: track_log_call(
                    "error", msg, *args, **kwargs
                )
            )
            mock_logger.trace.side_effect = lambda msg, *args, **kwargs: track_log_call(
    ...
    def create_mock_app_config_service(
        ...
        mock_service.get_compiled_graphs_path.return_value = Path(
            defaults["compiled_graphs_path"]
        )
        mock_service.get_cache_path.return_value = Path(
            defaults.get("cache_path", "agentmap_data/cache")
        )
    ...
    def create_mock_file_path_service() -> Mock:
        ...
        mock_service.sanitize_filename.side_effect = (
            lambda f: f
        )  # Return filename as-is
        mock_service.ensure_directory.side_effect = lambda p: Path(
            p
        )  # Return Path object
        mock_service.resolve_storage_path.side_effect = (
            lambda base, storage_type, collection=None, filename=None: (
                Path(base) / storage_type / (collection or "") / (filename or "")
            )
        )
        mock_service.get_dangerous_system_paths.return_value = [
            "/bin",
            "/etc",
            "C:\\Windows",
        ]
    ...
    @staticmethod
    def create_mock_storage_config_service(
        config_overrides: Optional[Dict[str, Any]] = None,
    ) -> Mock:
        """
        ...
        defaults = {
            ...
            "kv": {"enabled": False, "provider": "memory", "collections": {}},
            "blob": {
                "enabled": True,
        ...
        def get_memory_config() -> Dict[str, Any]:
            ...
            return {
                ...
                "max_documents_per_collection": memory_config.get(
        ...
        mock_service.get_firebase_config.return_value = defaults["providers"][
            "firebase"
        ]
        mock_service.get_mongodb_config.return_value = defaults["providers"]["mongodb"]
        mock_service.get_supabase_config.return_value = defaults["providers"][
            "supabase"
        ]
        ...
        def is_csv_storage_enabled() -> bool:
            ...
            return csv_config.get("enabled", True) and bool(
                csv_config.get("collections")
            )

        def is_json_storage_enabled() -> bool:
            """Check if JSON storage is configured and enabled."""
            json_config = defaults["json"]
            return json_config.get("enabled", True) and bool(
                json_config.get("collections")
            )
        ...
        mock_service.get_json_collection_file_path.side_effect = (
            get_json_collection_file_path
        )
        ...
        def get_collection_config(
            storage_type: str, collection_name: str
        ) -> Dict[str, Any]:
    ...
    def create_mock_node_registry_service() -> Mock:
        ...
        def prepare_for_assembly(
            graph_def: Dict[str, Any], graph_name: str
        ) -> Dict[str, Any]:
        ...
        def verify_pre_compilation_injection(
            ...
            return {
                ...
                "stats": {
                    ...
                    "injected_nodes": len(nodes_storage),
                },
            }
        ...
        mock_service.verify_pre_compilation_injection.side_effect = (
    ...
    def create_mock_execution_tracker(
        graph_name: str = "test_graph", tracking_config: Optional[Dict[str, Any]] = None
    ) -> Mock:
        """
        ...
        def record_node_start(node_name: str, inputs: Dict[str, Any] = None) -> None:
            if current_execution:
                execution = next(
                    e for e in executions_storage if e["id"] == current_execution
                )
        ...
        def record_node_result(
            node_name: str, success: bool, result: Any = None, error: str = None
        ) -> None:
            if current_execution:
                execution = next(
                    e for e in executions_storage if e["id"] == current_execution
    ...
    def create_mock_graph_bundle(
        graph: Any = None, node_registry: Any = None, version_hash: str = "test123"
    ) -> Mock:
        """
    ...
    def create_mock_graph_execution_service() -> Mock:
        ...
        def execute_from_definition(
            graph_def: Dict[str, Any], state: Dict[str, Any]
        ) -> Mock:
    ...
    def create_mock_graph_assembly_service() -> Mock:
        ...
        def assemble_graph(
            ...
            def invoke(state: Dict[str, Any]) -> Dict[str, Any]:
                ...
                result_state["nodes_executed"] = (
                    list(graph_def.keys()) if graph_def else []
                )
                return result_state
    ...
    def create_mock_csv_graph_parser_service() -> Mock:
        ...
        def create_mock_node_spec(
            node_name: str, graph_name: str, line_number: int = 1
        ) -> Mock:
        ...
        def create_mock_graph_spec(csv_path: Path) -> Mock:
            ...
            mock_spec.graphs = {"test_graph": [mock_node1, mock_node2]}
        ...
        def create_mock_validation_result(
            csv_path: Path, is_valid: bool = True
        ) -> Mock:
            ...
            mock_result.errors = (
                [] if is_valid else [Mock(message="Test validation error")]
            )
            ...
            mock_result.total_issues = (
                len(mock_result.errors)
                + len(mock_result.warnings)
                + len(mock_result.info)
            )
    ...
    def create_mock_graph_factory_service() -> Mock:
        ...
        def create_graph_from_nodes(
            ...
            mock_graph.entry_point = (
                list(nodes_dict.keys())[0]
                if nodes_dict and auto_detect_entry_point
                else None
            )
            return mock_graph

        def create_graph_from_definition(
            graph_def: Dict[str, Any], graph_name: Optional[str] = None
        ) -> Mock:
        ...
        mock_service.create_graph_from_definition.side_effect = (
            create_graph_from_definition
        )
        mock_service.resolve_graph_name_from_definition.side_effect = (
            resolve_graph_name_from_definition
        )
        mock_service.resolve_graph_name_from_path.side_effect = (
            resolve_graph_name_from_path
        )
    ...
    def create_mock_availability_cache_service() -> Mock:
        ...
        def invalidate(
            ...
            if category:
                ...
                prefix = f"{category}."
                keys_to_remove = [
                    k for k in availability_storage.keys() if k.startswith(prefix)
                ]
    ...
    def create_mock_system_storage_manager() -> Mock:
        ...
        def get_file_storage(namespace: Optional[str] = None) -> Mock:
            ...
            mock_file_service.read.return_value = Mock(
                success=True, data=b"", error=None
            )
    ...
    def create_mock_custom_agent_declaration_manager() -> Mock:
        ...
        declarations_storage = {"version": "1.0", "namespace": "custom", "agents": {}}
...
class ServiceMockBuilder:
    ...
    def with_method_return(
        self, method_name: str, return_value: Any
    ) -> "ServiceMockBuilder":
    ...
    def build(self) -> Mock:
        ...
        if hasattr(MockServiceFactory, factory_method_name):
            ...
            if factory_method_name in [
                "create_mock_app_config_service",
                "create_mock_execution_tracker",
            ]:
