...
def with_path_mocking(workflow_path_key: str = "execution_csv_path"):
    ...
    def decorator(test_func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if csv_path is None:
                raise ValueError(f"Test instance must have attribute '{workflow_path_key}'")
...
def with_robust_mocking(**mock_configs: Dict[str, Any]):
    ...
    def decorator(test_func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            # Apply all patches
            with patches[0] if len(patches) == 1 else patch.multiple(**{
                k: v for k, v in mock_configs.items()
            }):
                return test_func(self, *args, **kwargs)
...
class CITestValidator:
    ...
    def validate_test_setup(test_instance, required_attributes: list[str]) -> None:
        ...
        for attr_name in required_attributes:
            if not hasattr(test_instance, attr_name):
                raise AssertionError(f"Test setup incomplete: missing attribute '{attr_name}'")
            
            attr_value = getattr(test_instance, attr_name)
            if attr_value is None:
                raise AssertionError(f"Test setup incomplete: attribute '{attr_name}' is None")
    ...
    def validate_temp_directory(temp_dir: Path) -> None:
        ...
        try:
            ...
            raise AssertionError(f"Temporary directory is not writable: {temp_dir} - {e}")
...
def validate_ci_environment(test_instance) -> Dict[str, Any]:
    ...
    # Validate CSV files
    csv_attributes = [attr for attr in dir(test_instance) 
...
def ci_robust_test(workflow_path_key: str = "execution_csv_path", 
