...
""".format(
    ...
    def test_service_instantiation_through_di(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                registry_service.system_storage_manager,
                "System storage manager should be injected",
            )
            self.assertIsNotNone(
                registry_service.config, "App config should be injected"
            )
    ...
    def test_end_to_end_registry_operations(self):
        ...
        try:
            ...
            self.assertTrue(
                bundle_file.exists(), "Bundle file should exist before registration"
            )
            ...
            self.assertTrue(
                bundle_file.exists(),
                "Bundle file should still exist after registration",
            )
            ...
            entry_info = registry_service.get_entry_info(
                csv_hash, "integration_test_graph"
            )
            ...
            if found_bundle is None:
                ...
                self.assertTrue(
                    True, "Registry logic is working even if file lookup fails"
                )
            else:
                # If it does find the bundle, it should match
                self.assertEqual(
                    found_bundle, bundle_file.resolve(), "Should find registered bundle"
                )
            ...
            self.assertIsNone(
                found_after_removal, "Bundle should not be found after removal"
            )
    ...
    def test_persistence_through_system_storage_service(self):
        ...
        try:
            ...
            self.assertEqual(
                test_hash, csv_hash, "Hash computation should be consistent"
            )
    ...
    def test_app_config_integration(self):
        ...
        self.assertIsNotNone(
            registry_service._registry_path, "Registry path should be configured"
        )
        self.assertEqual(
            registry_service._registry_path,
            expected_registry_path,
    ...
    def test_graceful_degradation_with_missing_dependencies(self):
        ...
        registry_service = self.container.graph_registry_service()
        self.assertIsNotNone(
            registry_service, "Service should work with all dependencies"
        )
    ...
    def test_concurrent_access_through_di(self):
        ...
        for result_id in results:
            self.assertEqual(
                result_id,
                initial_id,
                "All threads should get same singleton instance as initial",
            )
    ...
    def test_service_registration_in_container(self):
        ...
        self.assertIsNotNone(
            service.system_storage_manager,
            "system_storage_manager dependency should be injected",
        )
        self.assertIsNotNone(service.config, "config dependency should be injected")
        self.assertIsNotNone(service.logger, "logger dependency should be injected")

        # Verify service is properly initialized
        self.assertIsNotNone(
            service._registry_path, "Registry path should be initialized"
        )
        self.assertIsNotNone(service._metadata, "Metadata should be initialized")
        self.assertIsInstance(
            service._registry_cache, dict, "Registry cache should be dict"
        )
