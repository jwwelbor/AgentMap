...
class MockGraphRunnerService:
    ...
    def __init__(
        self,
        agent_service_injection_service: Optional[AgentServiceInjectionService] = None,
    ):
    ...
    def _configure_agent_services(self, agent: Any) -> None:
        ...
        if (
            hasattr(self, "agent_service_injection_service")
            and self.agent_service_injection_service
        ):
    ...
    def _configure_agent_services_fallback(self, agent: Any) -> None:
        ...
        if isinstance(agent, LLMCapableAgent) and self.llm_service:
            ...
            self.logger.debug(
                f"[GraphRunnerService] Configured LLM service for {agent.name} (fallback)"
            )
        ...
        if isinstance(agent, StorageCapableAgent) and self.storage_service_manager:
            ...
            self.logger.debug(
                f"[GraphRunnerService] Configured storage service for {agent.name} (fallback)"
            )
...
class TestGraphRunnerServiceIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.storage_service_manager.get_service.side_effect = (
    ...
    def test_delegation_to_agent_service_injection_service(self):
        ...
        self.assertIsNotNone(
            agent.llm_service, "LLM service should be configured via delegation"
        )
        self.assertIsNotNone(
            agent.storage_service, "Storage service should be configured via delegation"
        )
        self.assertIsNotNone(
            agent.prompt_manager_service,
            "Prompt service should be configured via delegation",
        )
    ...
    def test_fallback_when_injection_service_unavailable(self):
        ...
        self.assertIsNotNone(
            agent.llm_service, "LLM service should be configured in fallback"
        )
        self.assertIsNotNone(
            agent.storage_service, "Storage service should be configured in fallback"
        )

        # Verify services are the correct ones
        self.assertEqual(agent.llm_service, self.graph_runner_fallback.llm_service)
        self.assertEqual(
            agent.storage_service, self.graph_runner_fallback.storage_service_manager
        )
    ...
    def test_logging_consistency(self):
        ...
        summary_logged = any(
            "services for logging_test_agent" in str(call) for call in info_calls
        )
    ...
    def test_error_handling_consistency(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            self.graph_runner_with_delegation._configure_agent_services(
                failing_agent_delegation
            )
    ...
    def test_service_injection_service_status(self):
        ...
        status_before = (
            self.agent_service_injection_service.get_service_injection_status(agent)
        )
        ...
        self.assertGreater(
            summary["total_services_configured"],
            0,
            "Should configure services when available",
        )
        self.assertEqual(
            summary["configuration_status"],
            "success",
            "Configuration should succeed with available services",
        )
...
class TestServiceInjectionMigration(unittest.TestCase):
    ...
    def test_functional_equivalence(self):
        ...
        self.assertEqual(
            set(new_types),
            expected_types,
            "New service should return correct service types",
        )
    ...
    def test_logging_migration(self):
        ...
        agent_name_logged = any(
            "logging_migration_agent" in str(call) for call in debug_calls
        )
        self.assertTrue(
            agent_name_logged, "Agent name should be included in log messages"
        )
