...
test_graph,End,EndAgent,Final node,ProcessAgent"""
    ...
    def test_workflow_execution_consistent_responses(self):
        ...
        with (
            patch("agentmap.runtime.ensure_initialized"),
            patch(
                "agentmap.runtime.workflow_ops.RuntimeManager"
            ) as mock_runtime_manager,
            patch(
    ...
    def test_diagnose_system_consistent_format(self):
        ...
        with (
            ...
            # Mock services
            mock_container.features_registry_service.return_value = (
                mock_features_service
            )
            mock_container.dependency_checker_service.return_value = (
            ...
            mock_features_service.is_feature_enabled.side_effect = (
...
class TestAdapterErrorConsistency:
    ...
    def test_graph_not_found_consistency(self):
        ...
        for func_name, args, kwargs in test_cases:
            with (
                patch("agentmap.runtime.init_ops.ensure_initialized"),
                patch(
                    "agentmap.runtime.workflow_ops.RuntimeManager"
                ) as mock_runtime_manager,
                patch(
                ...
                mock_container = Mock()
                mock_service = Mock()
                mock_service.get_or_create_bundle.side_effect = FileNotFoundError(
                    "File not found"
                )
    ...
    def test_invalid_inputs_consistency(self):
        ...
        for func_name, args, kwargs in test_cases:
            with (
                ...
                mock_container = Mock()
                mock_service = Mock()
                mock_service.get_or_create_bundle.side_effect = ValueError(
                    "Invalid input"
                )
                mock_service.validate_csv_for_bundling.side_effect = ValueError(
                    "Invalid input"
                )
                mock_container.graph_bundle_service.return_value = mock_service
                mock_container.validation_service.return_value = mock_service
    ...
    def test_runtime_error_consistency(self):
        ...
        for func_name, args, kwargs in test_cases:
            with (
                patch("agentmap.runtime.ensure_initialized"),
                patch(
                    "agentmap.runtime.runtime_manager.RuntimeManager"
                ) as mock_runtime_manager,
            ):

                # Setup to raise unexpected exception that doesn't get mapped to specific errors
                mock_container = Mock()
                mock_container.features_registry_service.side_effect = Exception(
                    "Unexpected error"
                )
    ...
    def test_graph_resolution_error_mapping(self):
        ...
        for func_name, args, kwargs in test_cases:
            with (
                ...
                mock_resolve_path.side_effect = GraphNotFound(
                    "test_graph", "Failed to resolve graph path: Unexpected error"
                )
        ...
        for func_name in test_functions:
            ...
            with (
                ...
                if func_name == "list_graphs":
                    ...
                    mock_dependency_checker.check_llm_dependencies.return_value = (
                        True,
                        [],
                    )
                    mock_dependency_checker.check_storage_dependencies.return_value = (
                        True,
                        [],
                    )
                    mock_dependency_checker.get_dependency_status_summary.return_value = {
                    ...
                    mock_container.validation_cache_service.return_value = (
                        mock_validation_cache_service
                    )

                    if func_name == "refresh_cache":
                        mock_dependency_checker.invalidate_environment_cache.return_value = (
                            None
                        )
                        mock_dependency_checker.discover_and_validate_providers.side_effect = [
                            {"openai": True},
                ...
                assert isinstance(
                    result["outputs"], dict
                ), f"{func_name} outputs should be a dict"
                assert isinstance(
                    result["metadata"], dict
                ), f"{func_name} metadata should be a dict"
