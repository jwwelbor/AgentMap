...
def test_lambda_function(function_name: str, region: str = "us-east-1") -> bool:
    ...
    # Test event payload
    test_event = {"graph": "Demo", "state": {"hello": "world"}, "action": "run"}
...
def test_api_gateway(api_url: str) -> bool:
    ...
    test_payload = {"graph": "Demo", "state": {"hello": "world"}, "action": "run"}
...
def check_agentmap_import() -> bool:
    ...
    try:
        ...
        print(
            f"✓ AgentMap imported successfully (version: {getattr(agentmap, '__version__', 'unknown')})"
        )
        return True
