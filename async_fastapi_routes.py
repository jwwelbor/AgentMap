...
class AsyncExecutionRoutes:
    ...
    async def execute_workflow_async(
        ...
        try:
            ...
            if request.resume_from_checkpoint:
                ...
                self.logger.info(
                    f"Resuming from checkpoint: {request.resume_from_checkpoint}"
                )
            ...
            if hasattr(self.graph_execution_service, "execute_from_definition_async"):
                ...
                result = (
                    await self.graph_execution_service.execute_from_definition_async(
                        graph_def=graph_def, state=execution_state, graph_name=graph
                    )
                )
            else:
            ...
            self.logger.error(
                f"❌ Async execution failed: {execution_id} - {error_msg}"
            )

            # Add background task for error logging
            background_tasks.add_task(
                self._log_execution_error, execution_id, execution_time, error_msg
            )
    ...
    async def resume_workflow_async(
        ...
        response = await self.execute_workflow_async(
            workflow, graph, request, background_tasks
        )

        return response

    async def validate_csv_async(
        self, request: ValidationRequest, background_tasks: BackgroundTasks
    ) -> ValidationResponse:
        ...
        try:
            ...
            if request.csv_content:
                ...
                if hasattr(validation_service, "validate_csv_content_async"):
                    result = await validation_service.validate_csv_content_async(
                        request.csv_content, rules=request.validation_rules
                    )
                else:
                ...
                if hasattr(validation_service, "validate_csv_file_async"):
                    result = await validation_service.validate_csv_file_async(
                        request.file_path, rules=request.validation_rules
                    )
                else:
                    # Run sync validation in executor
                    loop = asyncio.get_event_loop()
            ...
            response = ValidationResponse(
                ...
                graphs_found=(
                    result.graphs_found if hasattr(result, "graphs_found") else []
                ),
            )
            ...
            self.logger.error(
                f"❌ Async validation failed: {validation_id} - {error_msg}"
            )
        ...
        try:
            ...
            if hasattr(workflow_service, "list_workflows_async"):
                ...
                workflows = await loop.run_in_executor(
                    None, workflow_service.list_workflows
                )
    ...
    async def _get_graph_definition_async(
        self, workflow: str, graph: str
        ...
        try:
            ...
            if hasattr(csv_service, "parse_workflow_async"):
                ...
                workflow_data = await loop.run_in_executor(
                    None, csv_service.parse_workflow, workflow
                )
    ...
    async def _log_execution_completion(
        self, execution_id: str, execution_time: float, success: bool
    ):
    ...
    async def _log_execution_error(
        self, execution_id: str, execution_time: float, error_msg: str
    ):
    ...
    async def _log_validation_completion(
        self, validation_id: str, validation_time: float, valid: bool
    ):
...
def create_async_execution_router(
    graph_execution_service: Any, dependency_adapter: Any
) -> APIRouter:
    """
...
def integrate_async_routes(app, container):
    ...
    async_router = create_async_execution_router(
        graph_execution_service, dependency_adapter
    )
