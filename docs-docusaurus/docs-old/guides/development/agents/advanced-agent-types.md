...
| default_target | Default node if no match found | next_on_success |
...
This means you do not need to use a routing function in the next_node column when using the OrchestratorAgent.
...
3. **type**: Directly from the node's agent_type column
...
| `default_target` | Fallback node | `next_on_success` | Node name |
...
The `graph_name` field groups nodes together into a single workflow. When running a graph with `run_graph(graph_name)`, only nodes with the matching `graph_name` are included.
...
- As an identifier in routing fields (`next_node`, `next_on_success`, `next_on_failure`)
...
### next_node vs next_on_success/next_on_failure

There are two routing methods in AgentMap:

#### Simple Routing with next_node

When `next_node` is specified, the node will always proceed to the target node:

```csv title="Simple next_node Routing"
GraphA,Start,Next,,...
GraphA,Next,End,...
GraphA,End,,...
```

#### Conditional Routing with next_on_success/next_on_failure

When `next_on_success` and/or `next_on_failure` are specified, routing depends on the `last_action_success` flag:
...
The `agent_type` field determines which agent implementation to use. Available types include:
