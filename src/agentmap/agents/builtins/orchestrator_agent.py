...
class OrchestratorAgent(BaseAgent, LLMCapableAgent):
    ...
    def __init__(
        ...
        if raw_strategy in valid_strategies:
            ...
            if self._logger:  # Only log if logger is available
                self.log_warning(f"Invalid matching strategy '{raw_strategy}', defaulting to 'tiered'")
    ...
    def _match_intent(self, input_text: str, available_nodes: Dict[str, Dict[str, Any]], inputs: Dict[str, Any]) -> str:
        ...
        if self.matching_strategy == "algorithm":
            self.log_info(f"Using algorithm-based orchestration for request: {input_text}")
            ...
            if confidence < 0.1:  # Very low confidence indicates no specific match
                self.log_warning(f"No specific match found for request '{input_text}', using fallback node '{node}'")
  ...
  "reasoning": "your reasoning"
    ...
    def _get_input_text(self, inputs: Dict[str, Any]) -> str:
        ...
        for field in self.input_fields:
            if field in inputs and field not in ["available_nodes", "nodes", "__node_registry"]:
                value = inputs[field]
    ...
    def _get_nodes_from_inputs(self, inputs: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for field in self.input_fields:
            if field in inputs and field not in ["request", "input", "query", "text", "message", "user_input"]:
                value = inputs[field]
