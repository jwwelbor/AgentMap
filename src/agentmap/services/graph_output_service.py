...
class GraphOutputService:
    ...
    def export_graph(self, 
        ...
        """
        self.logger.info(f"[GraphOutputService] Exporting graph '{graph_name}' to format '{format}'")
    ...
    def export_as_python(self, 
        ...
        """
        self.logger.debug(f"[GraphOutputService] Exporting '{graph_name}' as Python code")
    ...
    def export_as_source(self, 
        ...
        """
        self.logger.debug(f"[GraphOutputService] Exporting '{graph_name}' as source code")
    ...
    def export_as_debug(self, 
        ...
        """
        self.logger.debug(f"[GraphOutputService] Exporting '{graph_name}' with debug information")
        ...
        for node_name, node in graph_def.items():
            lines.extend([
                ...
                f"#   Prompt: {node.prompt[:100]}..." if len(node.prompt or "") > 100 else f"#   Prompt: {node.prompt}",
                f"#   Edges: {node.edges}",
            ])
        ...
        self.logger.info(f"[GraphOutputService] ✅ Exported {graph_name} debug info to {path}")
        return path

    def export_as_documentation(self, 
                               graph_name: str, 
                               output_path: Optional[str] = None,
        ...
        """
        self.logger.debug(f"[GraphOutputService] Generating documentation for '{graph_name}'")
        ...
        self.logger.info(f"[GraphOutputService] ✅ Generated {format} documentation for {graph_name}: {path}")
        return path
    ...
    def _get_graph_definition(self, graph_name: str):
        ...
        try:
            ...
            self.logger.error(f"[GraphOutputService] Failed to get graph definition for export '{graph_name}': {e}")
            raise

    def _get_output_path(self, graph_name: str, output_path: Optional[str], ext: str) -> Path:
        """
    ...
    def _resolve_state_schema_class(self, state_schema: str):
        ...
        if state_schema == "dict":
            ...
            try:
                ...
                self.logger.warning(f"[GraphOutputService] Failed to import '{model_name}', fallback to dict: {e}")
                return dict
        else:
            try:
                ...
                self.logger.warning(f"[GraphOutputService] Failed to import custom schema '{state_schema}': {e}")
                return dict

    def _generate_python_code(self, graph_name: str, graph_def: Dict, state_schema: str) -> List[str]:
        """
        ...
        entry = next(iter(graph_def))
        lines.extend([
            f'builder.set_entry_point("{entry}")',
