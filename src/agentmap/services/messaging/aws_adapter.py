        ...
        try:
            ...
            try:
                ...
                if self.service_type == "sns":
                    self._sns_client = session.client("sns", region_name=self.region_name)
                    self.logger.debug(f"AWS SNS client initialized for region: {self.region_name}")
                elif self.service_type == "sqs":
                    self._sqs_client = session.client("sqs", region_name=self.region_name)
                    self.logger.debug(f"AWS SQS client initialized for region: {self.region_name}")
