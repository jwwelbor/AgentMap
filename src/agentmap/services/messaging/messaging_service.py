...
from agentmap.exceptions import MessagingServiceUnavailableError, MessagingOperationError, MessagingConnectionError
...
class MessagingService:
            ...
            try:
                ...
                if self._check_provider_availability(provider):
                    ...
                    if adapter:
                        ...
                        self.logger.info(f"Initialized {provider.value} messaging adapter")
                    else:
                        self._available_providers[provider_name] = False
                        self.logger.warning(f"Failed to create adapter for {provider_name}")
                else:
                    self._available_providers[provider_name] = False
                    self.logger.debug(f"Provider {provider_name} not available (cached)")
    ...
    def _build_message(
        ...
        return {
            ...
            "source": {
                "system": "agentmap",
                "version": self._get_agentmap_version()
            }
        }
