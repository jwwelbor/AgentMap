...
class LocalMessageAdapter:
    ...
    def _initialize_storage(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Local message storage initialized at: {self.storage_path}")
    ...
    async def publish(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to write message file {message_file}: {str(e)}")
                return StorageResult(
                    success=False,
    ...
    def list_messages(self, topic: str, limit: Optional[int] = None) -> list[Dict[str, Any]]:
        """
        ...
        try:
            ...
            message_files = sorted(
                [f for f in topic_dir.iterdir() if f.is_file() and f.suffix == '.json' and not f.name.startswith('.')],
            ...
            for message_file in message_files:
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to read message file {message_file}: {str(e)}")
                    continue
    ...
    def read_message(self, topic: str, message_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to read message {message_id} from topic {topic}: {str(e)}")
            return None
                ...
                if message_file.is_file() and message_file.suffix == '.json':
                    if message_file.stat().st_mtime < cutoff_time:
                        try:
                            ...
                            self.logger.warning(f"Failed to delete old message file {message_file}: {str(e)}")
            
            self.logger.info(f"Cleaned up {deleted_count} old messages from topic {topic}")
