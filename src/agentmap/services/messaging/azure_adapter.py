...
class AzureMessageAdapter:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Azure Service Bus client initialized for {self.service_type}")
    ...
    async def _publish_topic(
        ...
        try:
            ...
            self.logger.debug(f"Published message to Azure Service Bus topic {topic}: {message_id}")
    ...
    async def _publish_queue(
        ...
        try:
            ...
            self.logger.debug(f"Published message to Azure Service Bus queue {queue}: {message_id}")
    ...
    async def _create_topic_entity(self, topic_name: str) -> StorageResult:
        ...
        try:
            ...
            self.logger.info(f"Azure Service Bus topic creation attempted: {topic_name}")
            self.logger.warning(
            ...
            self.logger.error(f"Failed to create Azure Service Bus topic {topic_name}: {str(e)}")
            return StorageResult(
                success=False,
    ...
    async def _create_queue_entity(self, queue_name: str) -> StorageResult:
        ...
        try:
            ...
            self.logger.info(f"Azure Service Bus queue creation attempted: {queue_name}")
            self.logger.warning(
            ...
            self.logger.error(f"Failed to create Azure Service Bus queue {queue_name}: {str(e)}")
            return StorageResult(
                success=False,
