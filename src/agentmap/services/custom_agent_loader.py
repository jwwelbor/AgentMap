...
class CustomAgentLoader:
    ...
    def get_available_agents(self) -> Dict[str, str]:
        ...
        if not self.custom_agents_path.exists():
            self.logger.warning(f"Custom agents directory not found: {self.custom_agents_path}")
            return agents
    ...
    def validate_agent_class(self, agent_class: Type) -> bool:
        ...
        try:
            ...
            if not hasattr(agent_class, "run"):
                self.logger.error(f"Agent class {agent_class.__name__} missing 'run' method")
                return False
            
            if not hasattr(agent_class, "process"):
                self.logger.error(f"Agent class {agent_class.__name__} missing 'process' method")
                return False
