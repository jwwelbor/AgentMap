"""
AgentBootstrapService for AgentMap.
...
from agentmap.services.config.agent_config_service import AgentConfigService #agent constants
...
class AgentBootstrapService:
    ...
    def __init__(
        ...
        self.logger.info(
            "[AgentBootstrapService] Initialized with all dependencies"
        )
    ...
    def bootstrap_application(self) -> None:
        ...
        """
        self.logger.info(
            "🚀 [AgentBootstrapService] Starting application bootstrap"
        )
        ...
        try:
            ...
            self.logger.info(
                "✅ [AgentBootstrapService] Application bootstrap completed successfully"
            )

        except Exception as e:
            self.logger.error(f"❌ [AgentBootstrapService] Bootstrap failed: {e}")
            # Don't re-raise - use graceful degradation following storage service patterns
            self.logger.warning(
                "[AgentBootstrapService] Continuing with partial bootstrap"
            )
    ...
    def register_core_agents(self) -> None:
        ...
        """
        self.logger.debug("[AgentBootstrapService] Registering core agents")
        ...
        for agent_type, class_path in core_agents:
            try:
                ...
                registered_count += 1
                self.logger.debug(
                    f"[AgentBootstrapService] ✅ Registered core agent: {agent_type}"
                )
            except Exception as e:
                self.logger.error(
                    f"[AgentBootstrapService] ❌ Failed to register core agent {agent_type}: {e}"
                )
                # Core agents failing is serious, but don't crash the entire bootstrap

        self.logger.info(
            f"[AgentBootstrapService] Registered {registered_count}/{len(core_agents)} core agents"
        )
    ...
    def register_custom_agents(self) -> None:
        ...
        """
        self.logger.debug(
            "[AgentBootstrapService] Dynamically discovering custom agents"
        )
        ...
        try:
            ...
            if not custom_agents_path.exists():
                self.logger.info(
                    f"[AgentBootstrapService] Custom agents directory not found: {custom_agents_path}"
                )
                self.logger.info(
                    "[AgentBootstrapService] Skipping custom agent registration"
                )
                return

            self.logger.debug(
                f"[AgentBootstrapService] Scanning custom agents directory: {custom_agents_path}"
            )
            ...
            if not discovered_agents:
                self.logger.info(
                    "[AgentBootstrapService] No custom agent classes found"
                )
                return
            ...
            for agent_type, agent_class in discovered_agents:
                try:
                    ...
                    registered_count += 1
                    self.logger.debug(
                        f"[AgentBootstrapService] ✅ Registered custom agent: {agent_type}"
                    )
                except Exception as e:
                    self.logger.error(
                        f"[AgentBootstrapService] ❌ Failed to register custom agent {agent_type}: {e}"
                    )

            self.logger.info(
                f"[AgentBootstrapService] Registered {registered_count}/{len(discovered_agents)} custom agents"
            )

        except Exception as e:
            self.logger.error(
                f"[AgentBootstrapService] Failed to register custom agents: {e}"
            )
            # Don't re-raise - use graceful degradation
            self.logger.warning(
                "[AgentBootstrapService] Continuing without custom agents"
            )
    ...
    def discover_and_register_llm_agents(self) -> None:
        ...
        """
        self.logger.debug(
            "[AgentBootstrapService] Discovering and registering LLM agents"
        )
        ...
        if registered_llm_agents:
            self.logger.info(
                f"[AgentBootstrapService] ✅ LLM agents registered for providers: {registered_llm_agents}"
            )
        else:
            self.logger.info(
                "[AgentBootstrapService] ⚠️ No LLM providers available - LLM agents not registered"
            )
            missing_deps = self.features_registry.get_missing_dependencies("llm")
            if missing_deps:
                self.logger.debug(
                    f"[AgentBootstrapService] Missing LLM dependencies: {missing_deps}"
                )
    ...
    def discover_and_register_storage_agents(self) -> None:
        ...
        """
        self.logger.debug(
            "[AgentBootstrapService] Discovering and registering storage agents"
        )
        ...
        if registered_storage_agents:
            self.logger.info(
                f"[AgentBootstrapService] ✅ Storage agents registered for types: {registered_storage_agents}"
            )
        else:
            self.logger.info(
                "[AgentBootstrapService] ⚠️ No storage types available - storage agents not registered"
            )
            missing_deps = self.features_registry.get_missing_dependencies("storage")
            if missing_deps:
                self.logger.debug(
                    f"[AgentBootstrapService] Missing storage dependencies: {missing_deps}"
                )
    ...
    def register_mixed_dependency_agents(self) -> None:
        ...
        """
        self.logger.debug(
            "[AgentBootstrapService] Registering mixed-dependency agents"
        )
        ...
        self.logger.debug(
            f"[AgentBootstrapService] Registered {registered_count}/{len(mixed_agents)} mixed-dependency agents"
        )
    ...
    def discover_and_register_host_protocols(self) -> None:
        ...
        """
        self.logger.debug(
            "[AgentBootstrapService] Discovering and registering host protocols"
        )
        ...
        try:
            ...
            if not self.app_config.is_host_application_enabled():
                self.logger.debug(
                    "[AgentBootstrapService] Host application support disabled, skipping protocol discovery"
                )
                return
            ...
            if not protocol_folders:
                self.logger.debug(
                    "[AgentBootstrapService] No host protocol folders configured"
                )
                return

            self.logger.debug(
                f"[AgentBootstrapService] Scanning {len(protocol_folders)} protocol folders"
            )
            ...
            if not discovered_protocols:
                self.logger.info(
                    "[AgentBootstrapService] No host protocol classes found"
                )
                return
            ...
            self.logger.info(
                f"[AgentBootstrapService] ✅ Registered {registered_count}/{len(discovered_protocols)} host protocols"
            )

        except Exception as e:
            self.logger.error(
                f"[AgentBootstrapService] Failed to discover host protocols: {e}"
            )
            # Don't re-raise - use graceful degradation following storage service patterns
            self.logger.warning(
                "[AgentBootstrapService] Continuing without host protocols"
            )
        ...
        try:
            for protocol_folder in protocol_folders:
                if not protocol_folder.exists():
                    self.logger.debug(
                        f"[AgentBootstrapService] Protocol folder not found: {protocol_folder}"
                    )
                    continue

                if not protocol_folder.is_dir():
                    self.logger.warning(
                        f"[AgentBootstrapService] Protocol path is not a directory: {protocol_folder}"
                    )
                    continue

                self.logger.debug(
                    f"[AgentBootstrapService] Scanning protocol folder: {protocol_folder}"
                )
                ...
                if not python_files:
                    self.logger.debug(
                        f"[AgentBootstrapService] No Python files found in: {protocol_folder}"
                    )
                    continue

                self.logger.debug(
                    f"[AgentBootstrapService] Found {len(python_files)} Python files in {protocol_folder}"
                )
                ...
                for py_file in python_files:
                    ...
                    try:
                        ...
                        if spec is None or spec.loader is None:
                            self.logger.debug(
                                f"[AgentBootstrapService] Could not load spec for {py_file}"
                            )
                            continue
                        ...
                        for name, obj in inspect.getmembers(module, inspect.isclass):
                            if self._is_protocol_class(obj, module):
                                ...
                                self.logger.debug(
                                    f"[AgentBootstrapService] Discovered protocol class: {name} -> {protocol_name}"
                                )

                    except Exception as e:
                        self.logger.warning(
                            f"[AgentBootstrapService] Failed to scan protocol file {py_file}: {e}"
                        )
                        continue

            self.logger.debug(
                f"[AgentBootstrapService] Discovered {len(discovered_protocols)} protocol classes total"
            )
            return discovered_protocols

        except Exception as e:
            self.logger.error(
                f"[AgentBootstrapService] Error during protocol discovery: {e}"
            )
            return []
    ...
    def _is_protocol_class(self, cls, module) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"[AgentBootstrapService] Error checking protocol class {cls}: {e}"
            )
            return False
    ...
    def _register_discovered_protocols(self, protocols: List[tuple]) -> int:
        ...
        if not self.host_service_registry:
            self.logger.warning(
                "[AgentBootstrapService] HostServiceRegistry not available, skipping protocol registration"
            )
            return 0
        ...
        try:
            for protocol_name, protocol_class in protocols:
                try:
                    ...
                    self.logger.debug(
                        f"[AgentBootstrapService] Registered discovered protocol: {protocol_name} -> {protocol_class.__name__}"
                    )
                    registered_count += 1

                except Exception as e:
                    self.logger.error(
                        f"[AgentBootstrapService] Failed to register protocol {protocol_name}: {e}"
                    )
                    continue

            if registered_count > 0:
                self.logger.info(
                    f"[AgentBootstrapService] Successfully registered {registered_count} discovered host protocols"
                )
                ...
                if discovered_protocols:
                    self.logger.info(
                        "[AgentBootstrapService] Discovered protocols awaiting implementation:"
                    )
            ...
            self.logger.error(
                f"[AgentBootstrapService] Error during protocol registration: {e}"
            )
            return 0
    ...
    def _get_discovered_protocols_summary(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error(
                f"[AgentBootstrapService] Error getting discovered protocols summary: {e}"
            )
    ...
    def _discover_agent_classes(self, custom_agents_path: Path) -> List[tuple]:
        ...
        try:
            ...
            if not python_files:
                self.logger.debug(
                    "[AgentBootstrapService] No Python files found in custom agents directory"
                )
                return discovered_agents

            self.logger.debug(
                f"[AgentBootstrapService] Found {len(python_files)} Python files to scan"
            )
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if self._is_agent_class(obj, module):
                            ...
                            self.logger.debug(
                                f"[AgentBootstrapService] Discovered agent class: {name} -> {agent_type}"
                            )

                except Exception as e:
                    self.logger.warning(
                        f"[AgentBootstrapService] Failed to scan {py_file}: {e}"
                    )
                    continue

            self.logger.debug(
                f"[AgentBootstrapService] Discovered {len(discovered_agents)} agent classes"
            )
            return discovered_agents

        except Exception as e:
            self.logger.error(
                f"[AgentBootstrapService] Error during agent discovery: {e}"
            )
            return []
    ...
    def _is_agent_class(self, cls, module) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"[AgentBootstrapService] Error checking class {cls}: {e}"
            )
            return False
    ...
    def _register_llm_provider_agents(self, provider: str) -> int:
        ...
        if provider not in provider_agents:
            self.logger.warning(
                f"[AgentBootstrapService] Unknown LLM provider: {provider}"
            )
            return 0
        ...
        self.logger.debug(
            f"[AgentBootstrapService] Registered {registered_count} agents for LLM provider: {provider}"
        )
        return registered_count
    ...
    def _register_storage_type_agents(self, storage_type: str) -> int:
        ...
        if storage_type not in storage_agents:
            self.logger.debug(
                f"[AgentBootstrapService] No predefined agents for storage type: {storage_type}"
            )
            return 0
        ...
        self.logger.debug(
            f"[AgentBootstrapService] Registered {registered_count} agents for storage type: {storage_type}"
        )
        return registered_count
    ...
    def _register_base_llm_agent(self) -> None:
        ...
        if self._register_agent_if_available(
            ...
            self.logger.debug(
                "[AgentBootstrapService] ✅ Registered base LLM agent"
            )
    ...
    def _register_agent_if_available(self, agent_type: str, class_path: str) -> bool:
        ...
        try:
            ...
            self.agent_registry.register_agent(agent_type, agent_class)
            self.logger.debug(
                f"[AgentBootstrapService] ✅ Registered agent: {agent_type}"
            )
            return True
        except ImportError as e:
            self.logger.debug(
                f"[AgentBootstrapService] ⚠️ Agent {agent_type} not available: {e}"
            )
            return False
        except Exception as e:
            self.logger.error(
                f"[AgentBootstrapService] ❌ Failed to register agent {agent_type}: {e}"
            )
            return False
    ...
    def get_bootstrap_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "agent_breakdown": {
                "core_agents": self._count_agents_by_type(
                    agent_types, AgentConfigService.get_core_agent_types()
                ),
                "custom_agents": self._count_custom_agents(agent_types),
                "llm_agents": self._count_agents_by_type(
                    agent_types, AgentConfigService.get_llm_agent_types()
                ),
                "storage_agents": self._count_agents_by_type(
                    agent_types, AgentConfigService.get_storage_agent_types()
                ),
                "mixed_agents": self._count_agents_by_type(
                    agent_types, AgentConfigService.get_mixed_dependency_agent_types()
                ),
            },
            "host_application": {
    ...
    def _count_agents_by_type(
        self, agent_types: List[str], types_set: set
    ) -> int:
        """Count agents that match any type in the given set."""
        count = 0
        for agent_type in agent_types:
            if agent_type in types_set:
                count += 1
        return count
    ...
    def _count_custom_agents(self, agent_types: List[str]) -> int:
        ...
        # Get all known built-in agent types from AgentConfigService
        builtin_types = set()
        builtin_types.update(AgentConfigService.get_core_agent_types())
        builtin_types.update(AgentConfigService.get_llm_agent_types())
        builtin_types.update(AgentConfigService.get_storage_agent_types())
        builtin_types.update(AgentConfigService.get_mixed_dependency_agent_types())

        count = 0
        for agent_type in agent_types:
            # If agent type is not in any builtin category, it's custom
            if agent_type not in builtin_types:
                count += 1
        return count
    ...
    def _log_startup_summary(self) -> None:
        ...
        self.logger.info(f"📊 [AgentBootstrapService] Bootstrap Summary:")
        self.logger.info(
    ...
    def _log_service_status(self) -> None:
        ...
        self.logger.debug(f"[AgentBootstrapService] Service status: {status}")
        ...
        if not status["all_dependencies_injected"]:
            ...
            self.logger.warning(
                f"[AgentBootstrapService] Missing dependencies: {missing_deps}"
            )
