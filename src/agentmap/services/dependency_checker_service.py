...
class DependencyCheckerService:
    ...
    def __init__(
        ...
        self.logger.debug("[DependencyCheckerService] Initialized with FeaturesRegistryService coordination")
    ...
    def check_dependency(self, pkg_name: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"[DependencyCheckerService] Dependency check passed for: {pkg_name}")
            return True
            
        except (ImportError, ModuleNotFoundError):
            self.logger.debug(f"[DependencyCheckerService] Dependency check failed for: {pkg_name}")
            return False
    ...
    def validate_imports(self, module_names: List[str]) -> Tuple[bool, List[str]]:
        ...
        for module_name in module_names:
            try:
                ...
                self.logger.debug(f"[DependencyCheckerService] Error validating import for {module_name}: {e}")
                invalid.append(module_name)
        
        success = len(invalid) == 0
        if success:
            self.logger.debug(f"[DependencyCheckerService] All {len(module_names)} imports validated successfully")
        else:
            self.logger.debug(f"[DependencyCheckerService] {len(invalid)} imports failed: {invalid}")
    ...
    def check_llm_dependencies(self, provider: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        if provider:
            ...
            self.logger.debug(f"[DependencyCheckerService] LLM provider '{provider}' validation: {result}")
            ...
            self.logger.debug(f"[DependencyCheckerService] LLM providers available: {available_providers}")
    ...
    def check_storage_dependencies(self, storage_type: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        if storage_type:
            ...
            self.logger.debug(f"[DependencyCheckerService] Storage type '{storage_type}' validation: {result}")
            return result, missing
        else:
            ...
            self.logger.debug(f"[DependencyCheckerService] Core storage dependencies validation: {result}")
            return result, missing
    ...
    def discover_and_validate_providers(self, category: str) -> Dict[str, bool]:
        ...
        """
        self.logger.debug(f"[DependencyCheckerService] Discovering providers for category: {category}")
        ...
        self.logger.debug(f"[DependencyCheckerService] Provider discovery results for {category}: {results}")
        return results
    ...
    def _validate_llm_provider(self, provider: str) -> Tuple[bool, List[str]]:
        ...
        if not dependencies:
            self.logger.warning(f"[DependencyCheckerService] Unknown LLM provider: {provider}")
    ...
    def _validate_storage_type(self, storage_type: str) -> Tuple[bool, List[str]]:
        ...
        if not dependencies:
            self.logger.warning(f"[DependencyCheckerService] Unknown storage type: {storage_type}")
    ...
    def _get_storage_installation_guide(self, storage_type: Optional[str] = None) -> str:
        ...
        if storage_type:
            ...
            if storage_lower == "csv":
                ...
                return "pip install 'agentmap[vector]' or pip install langchain chromadb"
    ...
    def get_available_llm_providers(self) -> List[str]:
        ...
        self.logger.debug(f"[DependencyCheckerService] Available LLM providers: {available}")
        return available
    ...
    def get_available_storage_types(self) -> List[str]:
        ...
        self.logger.debug(f"[DependencyCheckerService] Available storage types: {available}")
        return available
