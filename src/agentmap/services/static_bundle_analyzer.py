...
class StaticBundleAnalyzer:
    ...
    def create_static_bundle(
        ...
        self.logger.debug(f"Resolved requirements: {len(required_services)} services, "
    ...
    def _extract_agent_types(self, nodes: list[Node]) -> Set[str]:
        ...
        if not agent_types:
            ...
            self.logger.debug("No agent types found, defaulting to 'default' agent type")
        
        return agent_types

    def _validate_declarations(self, agent_types: Set[str]) -> Tuple[Set[str], Set[str]]:
        """
