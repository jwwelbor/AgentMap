...
class DeclarationRegistryService:
    ...
    def load_all(self) -> None:
        ...
        self.logger.info(f"Loaded {len(self._agents)} agents and {len(self._services)} services")
    ...
    def add_service_declaration(self, declaration: ServiceDeclaration) -> None:
        ...
        self.logger.debug(f"Added dynamic service declaration: {declaration.service_name}")
    ...
    def _load_from_source(self, source: DeclarationSource) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Loaded {len(agents)} agents from {type(source).__name__}")
            
            # Load services (later sources override)
            services = source.load_services()
            self._services.update(services)
            self.logger.debug(f"Loaded {len(services)} services from {type(source).__name__}")
            
        except Exception as e:
            self.logger.error(f"Failed to load from source {type(source).__name__}: {e}")

    def _resolve_service_dependencies(self, service_names: Set[str], visited: Set[str]) -> Set[str]:
        """
        ...
        for service_name in service_names:
            if service_name in visited:
                self.logger.warning(f"Circular dependency detected for service: {service_name}")
                continue
            ...
            if dependencies:
                resolved_deps = self._resolve_service_dependencies(dependencies, new_visited)
