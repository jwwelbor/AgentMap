...
class GraphExecutionService:
    ...
    def __init__(
        ...
        self.logger.info("[GraphExecutionService] Initialized with execution coordination services")
    ...
    def setup_execution_tracking(self, graph_name: str) -> Any:
        ...
        """
        self.logger.debug(f"[GraphExecutionService] Setting up execution tracking for: {graph_name}")
        
        # Use ExecutionTrackingService to create tracker
        execution_tracker = self.execution_tracking_service.create_tracker()
        
        self.logger.debug(f"[GraphExecutionService] Execution tracking setup complete for: {graph_name}")
        return execution_tracker
    
    def execute_compiled_graph(self, bundle_path: Path, state: Dict[str, Any]) -> ExecutionResult:
        """
        ...
        self.logger.info(f"[GraphExecutionService] Executing compiled graph: {graph_name}")
        ...
        try:
            ...
            graph_success = self.execution_policy_service.evaluate_success_policy(execution_summary)
    ...
    def execute_from_definition(self, graph_def: Dict[str, Any], state: Dict[str, Any], graph_name: Optional[str] = None) -> ExecutionResult:
        """
        ...
        self.logger.info(f"[GraphExecutionService] Executing from definition: {graph_name}")
        ...
        try:
            ...
            self.logger.debug(f"[GraphExecutionService] Setting up execution tracking for: {graph_name}")
            execution_tracker = self.setup_execution_tracking(graph_name)
            self.logger.debug(f"[GraphExecutionService] Execution tracker created: {type(execution_tracker)}")
            
            # Assemble the graph from definition and set tracker on agents
            self.logger.debug(f"[GraphExecutionService] Assembling graph from definition: {graph_name}")
            compiled_graph = self._assemble_graph_from_definition(
                graph_def, graph_name, execution_tracker
            )
            self.logger.debug(f"[GraphExecutionService] Graph assembly complete: {graph_name}")
            
            # Execute the graph with tracking (tracker already set on agents)
            self.logger.debug(f"[GraphExecutionService] Executing graph with tracking: {graph_name}")
            final_state, execution_summary = self._execute_graph_with_tracking(
                compiled_graph, state, graph_name, execution_tracker
            )
            self.logger.debug(f"[GraphExecutionService] Graph execution complete: {graph_name}")
            ...
            import traceback
            self.logger.error(f"[GraphExecutionService] Full traceback:\n{traceback.format_exc()}")
            
            # Try to create execution summary even in case of error
            try:
                if execution_tracker is not None:
                    self.logger.debug(f"[GraphExecutionService] Creating execution summary from tracker after error")
                    # Complete execution tracking with error state
                    self.execution_tracking_service.complete_execution(execution_tracker)
                    execution_summary = self.execution_tracking_service.to_summary(execution_tracker, graph_name, state)
                    self.logger.debug(f"[GraphExecutionService] Error execution summary created with {len(execution_summary.node_executions)} node executions")
                else:
                    self.logger.warning(f"[GraphExecutionService] No execution tracker available for error summary")
            except Exception as summary_error:
                self.logger.error(f"[GraphExecutionService] Failed to create execution summary after error: {summary_error}")
                execution_summary = None
        ...
        try:
            ...
            try:
                ...
                with open(bundle_path, "rb") as f:
                    ...
                    self.logger.debug("[GraphExecutionService] Loaded legacy pickle format")
    ...
    def _assemble_graph_from_definition(self, graph_def: Dict[str, Any], graph_name: str, execution_tracker: Any) -> Any:
        """
    ...
    def _execute_graph_with_tracking(
        ...
        self.logger.debug(f"[GraphExecutionService] Final state type: {type(final_state)}")
        self.logger.debug(
        ...
        self.execution_tracking_service.complete_execution(execution_tracker)
        execution_summary = self.execution_tracking_service.to_summary(execution_tracker, graph_name, final_state)
        
        self.logger.debug(f"[GraphExecutionService] Execution tracking complete: {graph_name}")
        
        return final_state, execution_summary
    
    def _set_tracker_on_agents(self, graph_def: Dict[str, Any], execution_tracker: Any) -> None:
        """
        ...
        """
        self.logger.debug("[GraphExecutionService] Setting execution tracker on agent instances")
        self.logger.debug(f"[GraphExecutionService] Graph definition contains {len(graph_def)} nodes")
        self.logger.debug(f"[GraphExecutionService] Execution tracker type: {type(execution_tracker)}")
        
        agent_count = 0
        for node_name, node in graph_def.items():
            try:
                self.logger.debug(f"[GraphExecutionService] Processing node: {node_name}, type: {type(node)}")
                
                # Get agent instance from node context
                agent_instance = None
                if hasattr(node, 'context'):
                    self.logger.debug(f"[GraphExecutionService] Node {node_name} has context: {node.context is not None}")
                    if node.context:
                        agent_instance = node.context.get('instance')
                        self.logger.debug(f"[GraphExecutionService] Agent instance found for {node_name}: {agent_instance is not None}")
                        if agent_instance:
                            self.logger.debug(f"[GraphExecutionService] Agent instance type: {type(agent_instance)}")
                            self.logger.debug(f"[GraphExecutionService] Agent has set_execution_tracker method: {hasattr(agent_instance, 'set_execution_tracker')}")
                    else:
                        self.logger.debug(f"[GraphExecutionService] Node {node_name} context is None")
                else:
                    self.logger.debug(f"[GraphExecutionService] Node {node_name} has no context attribute")
                
                if agent_instance and hasattr(agent_instance, 'set_execution_tracker'):
                    agent_instance.set_execution_tracker(execution_tracker)
                    agent_count += 1
                    self.logger.debug(f"[GraphExecutionService] ✅ Set tracker for agent: {node_name}")
                else:
                    if agent_instance is None:
                        self.logger.warning(f"[GraphExecutionService] ❌ No agent instance found for node: {node_name}")
                    else:
                        self.logger.warning(f"[GraphExecutionService] ❌ Agent {node_name} missing set_execution_tracker method")
                    
            except Exception as e:
                self.logger.error(f"[GraphExecutionService] ❌ Error setting tracker for node {node_name}: {e}")
                import traceback
                self.logger.error(f"[GraphExecutionService] Traceback: {traceback.format_exc()}")
        
        self.logger.info(f"[GraphExecutionService] Set tracker on {agent_count}/{len(graph_def)} agent instances")
        
        if agent_count == 0:
            self.logger.error("[GraphExecutionService] ❌ CRITICAL: No agent instances found to set tracker on - execution tracking will fail!")
            ...
            for node_name, node in graph_def.items():
                ...
                self.logger.error(f"[GraphExecutionService]   Node {node_name}: has_context={has_context}, has_instance={has_instance}")
        else:
            self.logger.debug(f"[GraphExecutionService] ✅ Successfully set tracker on {agent_count} agents")
