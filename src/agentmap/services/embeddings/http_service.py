...
class HttpEmbeddingService:
    ...
    def __init__(self, base_url: str, timeout_s: float = 60.0, headers: dict[str,str] | None = None):
    ...
    def embed_batch(
        ...
        with httpx.Client(timeout=self._timeout, headers=self._headers) as client:
            r = client.post(f"{self._base}/embed", json={
                "texts": texts,
                "model": model,
                "metric": metric,
                "normalize": normalize,
        ...
        for _id, _vec, _meta in zip(ids, vectors, metas):
            outs.append(EmbeddingOutput(id=_id, vector=list(map(float, _vec)), dim=dim, model=model_o, metric=metric, metadata=_meta))
        return outs
