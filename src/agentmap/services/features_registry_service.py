...
class FeaturesRegistryService:
    ...
    def set_provider_available(self, category: str, provider: str, available: bool = True) -> None:
        """
        ...
        current_available, current_validated = self.features_registry.get_provider_status(category, provider)
        
        # Update availability while preserving validation status
        self.features_registry.set_provider_status(category, provider, available, current_validated)
        
        self.logger.debug(f"[FeaturesRegistryService] Provider '{provider}' in category '{category}' set to: {available}")
    
    def set_provider_validated(self, category: str, provider: str, validated: bool = True) -> None:
        """
        ...
        self.features_registry.set_provider_status(category, provider, current_available, validated)
        
        self.logger.debug(f"[FeaturesRegistryService] Provider '{provider}' in category '{category}' validation set to: {validated}")
    ...
    def is_provider_available(self, category: str, provider: str) -> bool:
        ...
        available, validated = self.features_registry.get_provider_status(category, provider)
    ...
    def get_available_providers(self, category: str) -> List[str]:
        ...
        category_providers = all_missing.get(category, {})
    ...
    def record_missing_dependencies(self, category: str, missing: List[str]) -> None:
        ...
        if missing:
            self.logger.debug(f"[FeaturesRegistryService] Recorded missing dependencies for {category}: {missing}")
        else:
            self.logger.debug(f"[FeaturesRegistryService] No missing dependencies for {category}")
