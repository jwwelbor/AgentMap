...
class LLMMatcher:
    def __init__(self, logger: logging.Logger, prompt_manager: PromptManagerService, llm_service: LLMService, keyword_parser: Callable[[Dict[str, Any]], List[str]]):
        self.logger = logger
        self.prompt_manager = prompt_manager
        self.llm_service = llm_service
        self._parse_keywords = keyword_parser

    def llm_match(self, input_text: str, available_nodes: Dict[str, Dict[str, Any]], llm_config: Optional[Dict[str, Any]], context: Optional[Dict[str, Any]]) -> str:
        if not self.llm_service:
            raise ValueError("LLM service not configured")
        nodes_text = self._format_node_descriptions(available_nodes)
        additional_context = f"\n\nAdditional context: {context['routing_context']}" if context and context.get("routing_context") else ""
        formatted_prompt = self.prompt_manager.format_prompt("file:orchestrator/intent_matching_v1.txt", {"nodes_text": nodes_text, "input_text": input_text, "additional_context": additional_context})
        if additional_context:
            formatted_prompt += additional_context
        llm_config = llm_config or {}
        llm_response = self.llm_service.call_llm(provider=llm_config.get("provider", "openai"), messages=[{"role": "user", "content": formatted_prompt}], temperature=llm_config.get("temperature", 0.2))
    ...
    def _extract_node_from_response(self, llm_response: str, available_nodes: Dict[str, Dict[str, Any]]) -> str:
        try:
            if isinstance(llm_response, str) and llm_response.strip().startswith("{"):
                ...
                if "selectedNode" in parsed and parsed["selectedNode"] in available_nodes:
