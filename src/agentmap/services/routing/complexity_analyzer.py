...
class PromptComplexityAnalyzer:
    ...
    def _load_context_thresholds(self) -> Dict[str, int]:
        ...
        return {
            ...
            "input_field_count_threshold": context_config.get("input_field_count_threshold", 5)
        }
    
    def _compile_patterns(self):
        """Compile regex patterns for prompt structure analysis."""
        self.patterns = {
            "questions": re.compile(r'\?|\bwhat\b|\bhow\b|\bwhy\b|\bwhen\b|\bwhere\b|\bwho\b', re.IGNORECASE),
            "commands": re.compile(r'\banalyze\b|\bcreate\b|\bgenerate\b|\bexplain\b|\bcompare\b|\bwrite\b', re.IGNORECASE),
            "technical_terms": re.compile(r'\bAPI\b|\bJSON\b|\bSQL\b|\bHTTP\b|\balgorithm\b|\bfunction\b|\bclass\b', re.IGNORECASE),
            "complexity_indicators": re.compile(r'\bcomplex\b|\bdetailed\b|\bcomprehensive\b|\badvanced\b|\bin-depth\b', re.IGNORECASE),
            "urgency_indicators": re.compile(r'\burgent\b|\bcritical\b|\bemergency\b|\bimportant\b|\basap\b', re.IGNORECASE),
            "creative_indicators": re.compile(r'\bcreative\b|\bimagine\b|\bstory\b|\bnarrative\b|\bartistic\b|\binnovative\b', re.IGNORECASE)
        }
    ...
    def _analyze_prompt_keywords(self, prompt: str) -> ComplexitySignal:
        ...
        if not any(complexity_scores.values()):
            ...
            complexity = max(complexity_scores.keys(), key=lambda k: complexity_scores[k])
    ...
    def analyze_context_complexity(self, context: Dict[str, Any]) -> TaskComplexity:
        ...
        context_size = sum(len(str(v)) for v in context.values() if isinstance(v, (str, int, float)))
    ...
    def analyze_memory_complexity(self, memory_size: int, memory_content: List[Dict[str, str]]) -> TaskComplexity:
        ...
        if memory_content:
            total_content_length = sum(len(msg.get("content", "")) for msg in memory_content)
    ...
    def analyze_task_type_complexity(self, task_type: str, prompt: str) -> ComplexitySignal:
        ...
        if keyword_scores:
            ...
            max_complexity_level = max(keyword_scores.keys(), key=lambda k: keyword_scores[k])
            ...
            reasoning = f"Task-specific keywords for {task_type} ({max_complexity_level})"
        else:
    ...
    def combine_complexity_signals(self, signals: List[TaskComplexity]) -> TaskComplexity:
    ...
    def determine_overall_complexity(
        ...
        if routing_context.input_context:
            context_complexity = self.analyze_context_complexity(routing_context.input_context)
            signals.append(context_complexity)
        
        # Memory analysis
        if routing_context.memory_size > 0:
            memory_complexity = self.analyze_memory_complexity(routing_context.memory_size, [])
