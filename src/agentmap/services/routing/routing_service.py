...
class LLMRoutingService:
        ...
        try:
            ...
            if self.cache and not decision.fallback_used:
                self._cache_decision(
                    task_type, complexity, prompt, available_providers, 
    ...
    def select_optimal_model(
        ...
        if routing_context.cost_optimization and self.routing_config.is_cost_optimization_enabled():
    ...
    def _select_from_preferred_providers(
        ...
        for provider in preferred_providers:
            model = self.routing_config.get_model_for_complexity(provider, complexity_str)
            if model:
                return RoutingDecision(
                    provider=provider,
                    model=model,
    ...
    def _apply_fallback_strategy(
        ...
        fallback_provider = routing_context.fallback_provider or self.routing_config.get_fallback_provider()
        fallback_model = routing_context.fallback_model or self.routing_config.get_fallback_model()
        ...
        return self._create_emergency_fallback(available_providers, "All fallback strategies exhausted")
    
    def _get_lower_complexity(self, complexity: TaskComplexity) -> TaskComplexity:
        """Get the next lower complexity level."""
        complexity_order = [TaskComplexity.LOW, TaskComplexity.MEDIUM, TaskComplexity.HIGH, TaskComplexity.CRITICAL]
