...
from typing import Any, Dict, List, Optional, Protocol, Type, runtime_checkable, Union
...
class HostServiceRegistry:
    ...
    def register_service_provider(
        ...
        if provider is None and metadata and metadata.get("type") == "discovered_protocol":
            self.logger.debug(f"[HostServiceRegistry] Registering protocol placeholder for '{service_name}'")
        elif not provider:
            self.logger.warning(f"[HostServiceRegistry] Empty provider provided for service '{service_name}'")
            return
        
        try:
            # Check if service already registered
            if service_name in self._service_providers:
                self.logger.warning(f"[HostServiceRegistry] Service '{service_name}' already registered, overwriting")
            ...
            if protocols:
                for protocol in protocols:
                    if self._is_valid_protocol(protocol):
                        ...
                        self.logger.debug(f"[HostServiceRegistry] Registered protocol {protocol.__name__} -> {service_name}")
                    else:
                        self.logger.warning(f"[HostServiceRegistry] Invalid protocol provided: {protocol}")
            
            # Cache only valid protocols for this service
            if valid_protocols:
                self._protocol_cache[service_name] = valid_protocols
            
            self.logger.info(f"[HostServiceRegistry] ✅ Registered service provider: {service_name}")
            
            # Log protocol count for debugging
            if valid_protocols:
                self.logger.debug(f"[HostServiceRegistry] Service '{service_name}' implements {len(valid_protocols)} valid protocols")
            elif protocols:
                self.logger.debug(f"[HostServiceRegistry] Service '{service_name}' had {len(protocols)} protocols provided, but none were valid")
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Failed to register service '{service_name}': {e}")
    ...
    def register_protocol_implementation(
        ...
        if service_name not in self._service_providers:
            self.logger.warning(f"[HostServiceRegistry] Service '{service_name}' not registered")
            return
        ...
        try:
            ...
            self.logger.error(f"[HostServiceRegistry] Failed to register protocol {protocol.__name__}: {e}")
    ...
    def get_service_provider(self, service_name: str) -> Optional[Any]:
        ...
        """
        if service_name not in self._service_providers:
            self.logger.debug(f"[HostServiceRegistry] Service '{service_name}' not found")
            return None
        
        try:
            provider = self._service_providers[service_name]
            self.logger.debug(f"[HostServiceRegistry] Retrieved service provider: {service_name}")
            return provider
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error retrieving service '{service_name}': {e}")
            return None
    ...
    def get_protocol_implementation(self, protocol: Type) -> Optional[str]:
        ...
        if protocol not in self._protocol_implementations:
            self.logger.debug(f"[HostServiceRegistry] No implementation found for protocol: {protocol.__name__}")
            return None
        
        try:
            service_name = self._protocol_implementations[protocol]
            self.logger.debug(f"[HostServiceRegistry] Protocol {protocol.__name__} implemented by: {service_name}")
            return service_name
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error getting protocol implementation for {protocol.__name__}: {e}")
            return None
    ...
    def discover_services_by_protocol(self, protocol: Type) -> List[str]:
        ...
        try:
            ...
            if implementing_services:
                self.logger.debug(f"[HostServiceRegistry] Found {len(implementing_services)} services implementing {protocol.__name__}")
            else:
                self.logger.debug(f"[HostServiceRegistry] No services found implementing {protocol.__name__}")
            
            return implementing_services
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error discovering services for protocol {protocol.__name__}: {e}")
            return []
    ...
    def list_registered_services(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"[HostServiceRegistry] {len(service_names)} services registered")
    ...
    def get_service_metadata(self, service_name: str) -> Optional[Dict[str, Any]]:
        ...
        if service_name not in self._service_metadata:
            self.logger.debug(f"[HostServiceRegistry] No metadata found for service: {service_name}")
            return None
        
        try:
            metadata = self._service_metadata[service_name].copy()
            self.logger.debug(f"[HostServiceRegistry] Retrieved metadata for service: {service_name}")
            return metadata
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error getting metadata for service '{service_name}': {e}")
            return None
    ...
    def update_service_metadata(
        ...
        """
        if service_name not in self._service_providers:
            self.logger.warning(f"[HostServiceRegistry] Cannot update metadata for unregistered service: {service_name}")
            return False
        ...
        try:
            ...
            self.logger.debug(f"[HostServiceRegistry] ✅ Updated metadata for service: {service_name}")
            return True
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Failed to update metadata for service '{service_name}': {e}")
            return False
    ...
    def get_service_protocols(self, service_name: str) -> List[Type]:
        ...
        """
        if service_name not in self._protocol_cache:
            self.logger.debug(f"[HostServiceRegistry] No protocols cached for service: {service_name}")
            return []
        
        try:
            protocols = self._protocol_cache[service_name].copy()
            self.logger.debug(f"[HostServiceRegistry] Service '{service_name}' implements {len(protocols)} protocols")
            return protocols
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error getting protocols for service '{service_name}': {e}")
            return []
    ...
    def unregister_service(self, service_name: str) -> bool:
        ...
        """
        if service_name not in self._service_providers:
            self.logger.debug(f"[HostServiceRegistry] Service '{service_name}' not registered")
            return False
        ...
        try:
            ...
            self.logger.info(f"[HostServiceRegistry] ✅ Unregistered service: {service_name}")
            if protocols_to_remove:
                self.logger.debug(f"[HostServiceRegistry] Removed {len(protocols_to_remove)} protocol mappings")
            
            return True
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error unregistering service '{service_name}': {e}")
            return False
    ...
    def clear_registry(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"[HostServiceRegistry] ✅ Cleared registry: {service_count} services, {protocol_count} protocols")
    ...
    def get_registry_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[HostServiceRegistry] Error generating registry summary: {e}")
            return {
                "service": "HostServiceRegistry",
                "error": str(e),
    ...
    def validate_service_provider(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            checks = {
                ...
                "provider_is_valid": provider is not None and (callable(provider) or inspect.isclass(provider) or hasattr(provider, '__dict__')),
            ...
            if not validation_result["valid"]:
                ...
                self.logger.warning(f"[HostServiceRegistry] Service '{service_name}' validation failed: {failed_checks}")
            
            return validation_result
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error validating service '{service_name}': {e}")
            return {
                "valid": False,
                "error": str(e),
                "service_name": service_name,
    ...
    def _is_valid_protocol(self, protocol: Type) -> bool:
        ...
        try:
            ...
            if hasattr(protocol, '_is_protocol') and getattr(protocol, '_is_protocol', False):
                return True
            ...
            if hasattr(typing, 'Protocol'):
                ...
                for base in mro:
                    if (getattr(base, '__module__', '') == 'typing' and 
                        base.__name__ == 'Protocol'):
                        return True
            
            # Check if it's decorated with @runtime_checkable (has protocol-like attributes)
            if hasattr(protocol, '__class_getitem__') and hasattr(protocol, '__subclasshook__'):
            ...
            self.logger.debug(f"[HostServiceRegistry] Error validating protocol {protocol}: {e}")
            return False
    ...
    def _cleanup_partial_registration(self, service_name: str) -> None:
        ...
        try:
            ...
            self.logger.debug(f"[HostServiceRegistry] Cleaned up partial registration for: {service_name}")
            
        except Exception as e:
            self.logger.error(f"[HostServiceRegistry] Error during cleanup for service '{service_name}': {e}")
