...
from agentmap.services.protocols import LLMServiceProtocol, FeaturesRegistryServiceProtocol
...
class OrchestratorService:
    ...
    def __init__(
        ...
        if self.features_registry:
            ...
            self.logger.debug(f"[OrchestratorService] NLP capabilities: {self._nlp_capabilities}")
    ...
    def select_best_node(
        ...
        self.logger.debug(f"Strategy: {strategy}, Available nodes: {list(available_nodes.keys())}")
        ...
        self.logger.debug(f"Available nodes after filtering: {list(filtered_nodes.keys())}")

        if not filtered_nodes:
            default_target = context.get("default_target", "") if context else ""
            self.logger.warning(f"No nodes available after filtering. Using default: {default_target}")
            return default_target

        # Handle single node case
        if len(filtered_nodes) == 1:
            node_name = next(iter(filtered_nodes.keys()))
            self.logger.debug(f"Only one node available, selecting '{node_name}' without matching")
            return node_name

        # Select node based on matching strategy
        selected_node = self._match_intent(
            input_text, filtered_nodes, strategy, confidence_threshold, llm_config, context
        )
        ...
        try:
            ...
            for token in doc:
                if (
                    ...
                    and token.pos_ in ["NOUN", "VERB", "ADJ"]  # Meaningful parts of speech
                ):
            ...
            self.logger.debug(f"spaCy extracted {len(unique_keywords)} enhanced keywords")
    ...
    def _match_intent(
        ...
        if strategy == "algorithm":
            self.logger.info(f"Using algorithm-based orchestration for request: {input_text}")
            ...
            self.logger.debug(f"Algorithm matching selected '{node}' with confidence {confidence:.2f}")
            return node
    ...
    def _algorithm_match(
        ...
        if best_score > 0.3:  # Good basic match threshold
            ...
            self.logger.debug(f"Level 2 (basic keyword) match: {best_match} (score: {best_score:.2f})")
        ...
        if self._nlp_capabilities and self._nlp_capabilities.get("fuzzywuzzy_available", False):
            fuzzy_match, fuzzy_score = self._fuzzy_algorithm_match(input_text, available_nodes)
            if fuzzy_score > 0.2:  # Lower threshold for fuzzy matching
                matching_level = "fuzzy_keyword"
                self.logger.debug(f"Level 3 (fuzzy keyword) match: {fuzzy_match} (score: {fuzzy_score:.2f})")
        ...
        if self._nlp_capabilities and self._nlp_capabilities.get("spacy_available", False):
            spacy_match, spacy_score = self._spacy_algorithm_match(input_text, available_nodes)
            if spacy_score > 0.15:  # Even lower threshold for advanced NLP
                matching_level = "spacy_enhanced"
                self.logger.debug(f"Level 4 (spaCy enhanced) match: {spacy_match} (score: {spacy_score:.2f})")
    ...
    def _spacy_algorithm_match(
        ...
        try:
            ...
            input_keywords = [
                token.lemma_.lower() for token in input_doc
    ...
    def _llm_match(
        ...
        if not self.llm_service:
            raise ValueError("LLM service not configured but required for LLM matching strategy")
    ...
    def _extract_node_from_response(
        ...
        self.logger.warning("Couldn't extract node from LLM response. Using first available.")
