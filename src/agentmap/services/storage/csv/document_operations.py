...
class CSVDocumentOperations:
    ...
    def find_document_by_id(
        df: pd.DataFrame, document_id: str, id_field: Optional[str] = None
        ...
        if CSVDocumentOperations.BATCH_COLUMN in df.columns:
            ...
            if len(matching_rows) > 0:
                ...
                result_df = matching_rows.drop(
                    columns=[CSVDocumentOperations.BATCH_COLUMN]
                )
    ...
    def update_or_insert_document(
        ...
        """
        id_column = (
            id_field if id_field else CSVIdDetection.detect_id_column(existing_df)
        )
    ...
    def update_or_insert_batch(
        existing_df: pd.DataFrame, new_data_df: pd.DataFrame, document_id: str
    ) -> pd.DataFrame:
        """
