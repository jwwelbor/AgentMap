...
class GraphAssemblyService:
    ...
    def assemble_graph(
        ...
        if self.orchestrator_nodes:
            self.logger.debug(f"Adding dynamic routers for {len(self.orchestrator_nodes)} orchestrator nodes")
        ...
        if isinstance(agent_instance, NodeRegistryUser):
            ...
            if self.node_registry:
                try:
                    ...
                    self.logger.debug(f"✅ Injected registry into orchestrator '{name}'")
                except Exception as e:
    ...
    def process_node_edges(self, node_name: str, edges: Dict[str, str]) -> None:
        ...
        self.logger.debug(f"Processing edges for node '{node_name}': {list(edges.keys())}")
        ...
        if not has_func:
            if "success" in edges and "failure" in edges:
                self._add_success_failure_edge(node_name, edges["success"], edges["failure"])
    ...
    def _add_success_failure_edge(self, source: str, success: str, failure: str) -> None:
    ...
    def _add_function_edge(self, source: str, func_name: str, success: Optional[str], failure: Optional[str]) -> None:
