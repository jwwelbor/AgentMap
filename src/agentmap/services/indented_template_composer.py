...
class IndentedTemplateComposer:
    ...
    def __init__(self, app_config_service: AppConfigService, logging_service: LoggingService):
        """
        ...
        self.logger.info("[IndentedTemplateComposer] Initialized with internal template loading capabilities")
    ...
    def _load_template_internal(self, template_path: str) -> str:
        ...
        if normalized_path in self._template_cache:
            ...
            self.logger.debug(f"[IndentedTemplateComposer] Cache hit for template: {normalized_path}")
            return self._template_cache[normalized_path]
        
        # Cache miss - load template
        self._cache_stats["misses"] += 1
        self.logger.debug(f"[IndentedTemplateComposer] Loading template: {normalized_path}")
        ...
        try:
            ...
            self.logger.debug(f"[IndentedTemplateComposer] Successfully loaded and cached template: {normalized_path}")
            return content
            
        except Exception as e:
            self.logger.error(f"[IndentedTemplateComposer] Failed to load template {normalized_path}: {e}")
            raise
    ...
    def _discover_and_load_template(self, template_path: str) -> str:
        ...
        try:
            ...
            self.logger.debug(f"[IndentedTemplateComposer] Embedded resource loading failed for {template_path}: {e}")
        ...
        try:
            ...
            file_path = prompts_dir / f"scaffold/{template_path}" if not template_path.startswith("scaffold/") else prompts_dir / template_path
            
            if file_path.exists():
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read().strip()
                    self.logger.debug(f"[IndentedTemplateComposer] Loaded from prompts directory: {file_path}")
                    return content
        except Exception as e:
            self.logger.debug(f"[IndentedTemplateComposer] Prompts directory loading failed for {template_path}: {e}")
    ...
    def _load_from_embedded_resources(self, template_path: str) -> str:
        ...
        try:
            ...
            if sys.version_info >= (3, 7):
                try:
                    with importlib.resources.path(package, resource_name) as template_path_obj:
            ...
            raise FileNotFoundError(f"Template resource not found: {package}/{resource_name}")
            
        except Exception as e:
            self.logger.debug(f"[IndentedTemplateComposer] Embedded resource loading error: {e}")
            raise
    
    def _apply_variable_substitution(self, content: str, variables: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            self.logger.warning(f"[IndentedTemplateComposer] Missing template variable: {missing_var}")
            self.logger.debug(f"[IndentedTemplateComposer] Available variables: {list(variables.keys())}")
            # Return content unchanged when variables are missing
            return content
        except Exception as e:
            self.logger.error(f"[IndentedTemplateComposer] Variable substitution error: {e}")
    ...
    def compose_template(
        ...
        try:
            ...
            variables = self._prepare_comprehensive_template_variables(agent_type, info, service_reqs)
            ...
            self.logger.error(f"[IndentedTemplateComposer] Failed to compose template for {agent_type}: {e}")
            raise
    ...
    def _compose_with_master_template(
        ...
        processed_sections = {
            'header': self._process_section('header', variables, 0),  # No additional indentation
            'class_definition': self._process_section('class_definition', variables, 0),  # No additional indentation
            'init_method': self._process_section('init_method', variables, 0),  # No additional indentation
            'process_method': self._process_section('process_method', variables, 0),  # No additional indentation
            'helper_methods': self._process_section('helper_methods', variables, 0),  # No additional indentation
            'footer': self._process_section('footer', variables, 0)  # No additional indentation
        }
        
        # Add service usage examples section if services are configured
        if service_reqs.services:
            processed_sections['service_examples'] = self._load_service_examples_section(service_reqs)
        else:
    ...
    def _process_section(
        ...
        try:
            ...
            formatted_content = self._apply_variable_substitution(template_content, variables)
            
            # Apply indentation using textwrap.indent()
            return self._apply_indentation(formatted_content, indent_spaces)
            
        except Exception as e:
            self.logger.error(f"[IndentedTemplateComposer] Failed to process section '{section_name}': {e}")
        ...
        try:
            ...
            for service in service_reqs.services:
                ...
                try:
                    ...
                    self.logger.warning(f"[IndentedTemplateComposer] Could not load usage example for {service}: {e}")
            ...
            self.logger.error(f"[IndentedTemplateComposer] Failed to load service examples: {e}")
    ...
    def _generate_services_documentation(self, attributes: List[ServiceAttribute]) -> str:
        """
    ...
    def compose_function_template(
        ...
        """
        try:
            self.logger.debug(f"[IndentedTemplateComposer] Composing function template for: {func_name}")
            ...
            # Apply variable substitution
            formatted_template = self._apply_variable_substitution(template_content, template_vars)
            
            self.logger.debug(f"[IndentedTemplateComposer] Successfully composed function template for: {func_name}")
            return formatted_template
            
        except Exception as e:
            self.logger.error(f"[IndentedTemplateComposer] Failed to compose function template for {func_name}: {e}")
            # Return fallback template to ensure operation doesn't fail
            # return self._generate_fallback_function_template(func_name, info)
    
    def _prepare_function_template_variables(self, func_name: str, info: Dict[str, Any]) -> Dict[str, str]:
        """
        ...
        context_fields = self._generate_context_fields(
            info.get("input_fields", []), 
            info.get("output_field", "")
        )
    ...
    def get_function_template_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_variables": [
                "func_name", "context", "context_fields", "success_node", 
                "failure_node", "node_name", "description", "output_field"
            ],
            "cache_enabled": True,
