...
class CSVGraphParserService:
    ...
    def validate_csv_structure(self, csv_path: Path) -> ValidationResult:
        ...
        self.logger.debug(f"[CSVGraphParserService] Validating CSV structure: {csv_path}")
    ...
    def _validate_csv_structure(self, df: pd.DataFrame, csv_path: Path) -> None:
        ...
        for col in self.required_columns:
            if col in df.columns and df[col].isna().all():
                raise ValueError(f"Required column '{col}' is completely empty in {csv_path}")
        ...
        self.logger.debug(f"[CSVGraphParserService] CSV structure valid: {len(df)} rows, {len(df.columns)} columns")
    ...
    def _parse_row_to_node_spec(self, row: pd.Series, line_number: int) -> Optional[NodeSpec]:
        """
        ...
        if not graph_name:
            self.logger.warning(f"[Line {line_number}] Missing GraphName. Skipping row.")
            return None
        
        if not node_name:
            self.logger.warning(f"[Line {line_number}] Missing Node name. Skipping row.")
            return None
        ...
        input_fields = [
            field.strip() for field in input_fields_str.split("|") 
            if field.strip()
        ] if input_fields_str else []
    ...
    def _safe_get_field(self, row: pd.Series, field_name: str, default: str = "") -> str:
        """
