...
from agentmap.exceptions.base_exceptions import AgentMapException, InvalidPathError, PathTraversalError, SystemPathError
...
class FilePathService:
    ...
    def validate_safe_path(self, path: str, base_directory: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if base_directory:
                self._check_base_directory_constraint(normalized_path, path, base_directory)
    ...
    def _check_base_directory_constraint(self, normalized_path: Path, original_path: str, base_directory: str) -> None:
        """
        ...
        try:
            ...
            self.logger.warning(f"Path outside base directory: {original_path} not in {base_directory}")
            raise InvalidPathError(f"Path must be within base directory: {base_directory}")
    ...
    def sanitize_filename(self, filename: str) -> str:
        ...
        try:
            ...
            if not is_valid_filename(sanitized):
                raise InvalidPathError(f"Sanitized filename is still invalid: '{sanitized}'")
