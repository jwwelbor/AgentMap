# services/graph/bundle_persistence_service.py
...
class BundlePersistenceService:
    """
    Handles file I/O operations for GraphBundle persistence.

    This service handles:
    - Saving bundles to disk (system cache)
    - Loading bundles from disk
    - Deleting cached bundles
    ...
    def __init__(
        ...
        """Initialize BundlePersistenceService.

        Args:
            logging_service: LoggingService for logging
            bundle_serializer: BundleSerializer for serialization/deserialization
            system_storage_manager: System storage manager for system cache storage
            file_path_service: File path service for centralized path handling
            app_config_service: Application config service for cache path (optional)
        """
        self.logger = logging_service.get_class_logger(self)
        self.bundle_serializer = bundle_serializer
    ...
    def save_bundle(self, bundle: GraphBundle, path: Path) -> Optional[StorageResult]:
        ...
        data = self.bundle_serializer.serialize_metadata_bundle(bundle)
    ...
    def load_bundle(self, path: Path) -> Optional[GraphBundle]:
        ...
        try:
            ...
            return self.bundle_serializer.deserialize_metadata_bundle(data)
