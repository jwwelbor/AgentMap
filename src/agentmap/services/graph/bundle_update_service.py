...
class BundleUpdateService:
    ...
    def _update_missing_declarations(
        ...
        custom_agent_declarations = self.custom_agent_declaration_manager.load_declarations().get('agents', {}) or {}
        for agent_type in list(bundle.missing_declarations):
            decl = self.declaration_registry.get_agent_declaration(agent_type) or custom_agent_declarations.get(agent_type)
            
            if decl:
                class_path = isinstance(decl, dict) and decl.get('class_path') or decl.class_path
    ...
    def _verify_existing_mappings(
        ...
        for agent_type, class_path in list(bundle.agent_mappings.items()):
            ...
            if not decl:
                ...
                decl_class_path = isinstance(decl, dict) and decl.get('class_path') or decl.class_path
    ...
    def _update_service_requirements(
        ...
        if new_requirements['services']:
            ...
            bundle.service_load_order = (
                self.declaration_registry.calculate_load_order(
    ...
    def _persist_updated_bundle(self, bundle: GraphBundle) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"[BundleUpdateService] Error persisting bundle: {e}"
            )
    
    def _has_changes(self, summary: Dict[str, Any]) -> bool:
        """Check if any changes were made."""
        return bool(
            summary["newly_resolved"] or 
            summary["updated_mappings"] or 
            summary["removed_mappings"] or
    ...
    def get_update_summary(self, bundle: GraphBundle) -> Dict[str, Any]:
        ...
        preview = {
            ...
            "missing_declarations": list(bundle.missing_declarations) if bundle.missing_declarations else [],
            "would_resolve": [],
            "would_update": [],
            "would_remove": []
        }
