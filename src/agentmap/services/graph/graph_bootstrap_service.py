...
class GraphBootstrapService:
    ...
    def bootstrap_from_bundle(self, bundle: GraphBundle) -> Dict[str, Any]:
        ...
        self._register_required_llm_agents(bundle.required_agents, bundle.agent_mappings)
        
        # Step 3: Register required storage agents if needed
        self._register_required_storage_agents(bundle.required_agents, bundle.agent_mappings)
        
        # Step 4: Register any custom agents from mappings
        self._register_custom_agents_from_mappings(bundle.required_agents, bundle.agent_mappings)
    ...
    def _register_required_llm_agents(self, required_agents: Set[str], agent_mappings: Optional[Dict[str, str]]) -> None:
        """
        ...
        self.logger.debug(f"[GraphBootstrapService] Registering required LLM agents: {needed_llm_agents}")
    ...
    def _register_required_storage_agents(self, required_agents: Set[str], agent_mappings: Optional[Dict[str, str]]) -> None:
        """
        ...
        self.logger.debug(f"[GraphBootstrapService] Registering required storage agents: {needed_storage_agents}")
    ...
    def _register_custom_agents_from_mappings(self, required_agents: Set[str], agent_mappings: Optional[Dict[str, str]]) -> None:
        """
        ...
        self.logger.debug(f"[GraphBootstrapService] Registering custom agents: {unregistered}")
    ...
    def _log_bootstrap_summary(self, bundle: GraphBundle) -> Dict[str, Any]:
        ...
        self.logger.info(f"📊 [GraphBootstrapService] Bootstrap Summary for '{bundle.graph_name}':")
    ...
    def _get_bootstrap_summary(self, bundle: GraphBundle) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_breakdown": {
                "core": len([a for a in self._loaded_agents if a in {
                    "default", "echo", "branching", "failure", "success", "input", "graph", "human"
                }]),
                "llm": len([a for a in self._loaded_agents if a in {
                    "llm", "openai", "anthropic", "google", "claude", "gpt", "gemini", "chatgpt"
                }]),
                "storage": len([a for a in self._loaded_agents if any(
                    a.startswith(p) for p in ["csv_", "json_", "file_", "vector_", "blob_"]
                )]),
                "custom": len([a for a in self._loaded_agents if a not in {
                    "default", "echo", "branching", "failure", "success", "input", "graph", "human",
                    "llm", "openai", "anthropic", "google", "claude", "gpt", "gemini", "chatgpt"
                } and not any(a.startswith(p) for p in ["csv_", "json_", "file_", "vector_", "blob_"])]),
            }
        }
