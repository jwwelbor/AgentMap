...
class AppConfigService:
    ...
    def get_csv_repository_path(self) -> Path:
        ...
        try:
            ...
            self._logger.debug(f"[AppConfigService] CSV repository path ensured: {csv_repo_path}")
        except Exception as e:
            self._logger.warning(f"[AppConfigService] Could not create CSV repository directory {csv_repo_path}: {e}")
    ...
    def get_jwt_config(self) -> Dict[str, Any]:
        ...
        return self.get_value("authentication.jwt", {
            "secret": None,
            "algorithm": "HS256",
            "expiry_hours": 24
        })
    ...
    def get_supabase_auth_config(self) -> Dict[str, Any]:
        ...
        return self.get_value("authentication.supabase", {
            "url": None,
            "anon_key": None
        })
    ...
    def get_public_endpoints(self) -> List[str]:
        ...
        return self.get_value("authentication.public_endpoints", [
            "/",
            "/health",
            "/docs",
            "/openapi.json",
    ...
    def get_embedded_mode_config(self) -> Dict[str, Any]:
        ...
        return self.get_value("authentication.embedded_mode", {
            "enabled": True,
            "bypass_auth": True
        })
    ...
    def get_auth_permissions_config(self) -> Dict[str, Any]:
        ...
        return self.get_value("authentication.permissions", {
            "default_permissions": ["read"],
            "admin_permissions": ["read", "write", "execute", "admin"],
            "execution_permissions": ["read", "execute"]
        })
    ...
    def validate_auth_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if auth_config.get('enabled', True):
                ...
                has_auth_method = (
                    bool(api_keys) or 
                    bool(jwt_secret) or 
                    (bool(supabase_config.get('url')) and bool(supabase_config.get('anon_key')))
                )
            ...
            if jwt_config.get('secret') and len(jwt_config['secret']) < 32:
                warnings.append("JWT secret should be at least 32 characters long for security")
            ...
            if not isinstance(public_endpoints, list):
                ...
                warnings.append("No public endpoints configured - all endpoints will require authentication")
        ...
        summary = {
            ...
            "has_supabase_config": bool(
                self.get_supabase_auth_config().get('url') and 
                self.get_supabase_auth_config().get('anon_key')
            )
        }
