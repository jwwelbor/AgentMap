...
class AppConfigService:
    ...
    def get_host_application_config(self) -> Dict[str, Any]:
        ...
        if host_config:
            self._logger.debug(f"[AppConfigService] Host application config loaded with sections: {list(host_config.keys())}")
        else:
            self._logger.debug("[AppConfigService] No host application config found, using defaults")
    ...
    def get_host_protocol_folders(self) -> List[Path]:
        ...
        protocol_folders_config = self.get_value("host_application.protocol_folders", [])
        ...
        for folder in protocol_folders_config:
            try:
                ...
                self._logger.warning(f"[AppConfigService] Invalid protocol folder path '{folder}': {e}")
        ...
        if not protocol_folders:
            ...
            self._logger.debug(f"[AppConfigService] Using default protocol folders: {default_folders}")
        else:
            folder_paths = [str(folder) for folder in protocol_folders]
            self._logger.debug(f"[AppConfigService] Using configured protocol folders: {folder_paths}")
    ...
    def get_host_service_config(self, service_name: str) -> Dict[str, Any]:
        ...
        if service_config:
            self._logger.debug(f"[AppConfigService] Host service '{service_name}' config loaded")
        else:
            self._logger.debug(f"[AppConfigService] No config found for host service '{service_name}', using defaults")
    ...
    def validate_host_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(services_config, dict):
                errors.append("Host application services configuration must be a dictionary")
            else:
            ...
            if not isinstance(discovery_config, dict):
                warnings.append("Service discovery configuration should be a dictionary")
        ...
        summary = {
            ...
            "protocol_folders_count": len(self.get_host_protocol_folders()) if not errors else 0,
            "services_count": len(self.get_host_application_config().get("services", {})) if not errors else 0
        }
        
        # Log validation results
        if is_valid:
            if warnings:
                self._logger.info(f"[AppConfigService] Host config validation completed with {len(warnings)} warnings")
            else:
                self._logger.debug("[AppConfigService] Host config validation passed")
        else:
            self._logger.error(f"[AppConfigService] Host config validation failed with {len(errors)} errors")
        ...
        if "host_application" in self._config_data:
            try:
                ...
                summary["host_application"] = {
                    ...
                    "protocol_folders_configured": len(host_config.get("protocol_folders", [])),
                    "service_discovery_enabled": host_config.get("service_discovery", {}).get("enabled", False),
                    "features_enabled": list(k for k, v in host_config.get("features", {}).items() if v)
                }
            except Exception as e:
                summary["host_application"] = {"error": f"Failed to load host config: {str(e)}"}
        else:
