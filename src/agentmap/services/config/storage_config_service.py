...
class StorageConfigService:
    ...
    def _validate_and_load_config(self, storage_config_path: Optional[Union[str, Path]]):
        ...
        self._logger.info(f"[StorageConfigService] Loading storage configuration from: {storage_path}")
        ...
        try:
            ...
            self._logger.info("[StorageConfigService] Storage configuration loaded successfully")
            
            # Log available storage sections for visibility
            if self._config_data:
                sections = list(self._config_data.keys())
                self._logger.info(f"[StorageConfigService] Available storage sections: {sections}")
    ...
    def replace_logger(self, logger: logging.Logger):
        ...
        if logger and self._logger:
            ...
            self._logger = logger
            self._logger.debug("[StorageConfigService] Replaced bootstrap logger with real logger")
    ...
    def get_collection_config(self, storage_type: str, collection_name: str) -> Dict[str, Any]:
        """
    ...
    def get_default_directory(self, storage_type: str) -> str:
        ...
        """
        return self.get_value(f"{storage_type}.default_directory", f"data/{storage_type}")
    ...
    def validate_storage_config(self) -> Dict[str, list[str]]:
        ...
        for storage_type, config in self._config_data.items():
            if not isinstance(config, dict):
                errors.append(f"Storage type '{storage_type}' configuration must be a dictionary")
                continue
                
            # Check for collections section
            if "collections" not in config:
                warnings.append(f"Storage type '{storage_type}' has no collections configured")
            elif not isinstance(config["collections"], dict):
                errors.append(f"Storage type '{storage_type}' collections must be a dictionary")
        
        self._logger.debug(f"[StorageConfigService] Validation completed: {len(warnings)} warnings, {len(errors)} errors")
