...
class AuthService:
    ...
    def __init__(self, auth_config: Dict[str, any], logging_service: LoggingService):
        ...
        self.logger.info(f"[AuthService] Initialized with {len(self._api_keys)} API keys")
    ...
    def _load_api_keys(self) -> Dict[str, Dict[str, any]]:
        ...
        for env_var, env_value in os.environ.items():
            if env_var.startswith('AGENTMAP_API_KEY_'):
                ...
                api_keys[key_hash] = {
                    'name': key_name,
                    'permissions': ['read', 'write'],  # Default permissions for env keys
                    'user_id': key_name,
                    'expires_at': None,
                ...
                if key_info.get('expires_at'):
                    try:
                        ...
                        if datetime.now() > expires_at:
                            self.logger.warning(f"[AuthService] Expired API key used: {key_info['name']}")
                            return AuthContext(authenticated=False, auth_method='api_key')
                    except ValueError:
                        self.logger.error(f"[AuthService] Invalid expiration date for key: {key_info['name']}")
                
                self.logger.info(f"[AuthService] Successful API key authentication: {key_info['name']}")
                return AuthContext(
                    authenticated=True,
                    auth_method='api_key',
    ...
    def get_public_endpoints(self) -> List[str]:
        ...
        """
        return self.config.get('public_endpoints', [
            '/health',
            '/docs',
            '/openapi.json',
            '/redoc',
    ...
    def get_auth_stats(self) -> Dict[str, any]:
        ...
        """
        active_keys = sum(1 for key_info in self._api_keys.values()
                         if not key_info.get('expires_at') or 
