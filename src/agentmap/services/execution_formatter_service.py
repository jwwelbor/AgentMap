...
class ExecutionFormatterService:
    ...
    def format_execution_result(self, result: Dict[str, Any], verbose: bool = False) -> str:
        """
        ...
        if exec_summary:
            ...
            lines.append(f"Success: {'✅ Yes' if exec_summary.graph_success else '❌ No'}")
            
            # Duration calculation
            if hasattr(exec_summary, 'start_time') and hasattr(exec_summary, 'end_time'):
                duration = (exec_summary.end_time - exec_summary.start_time).total_seconds()
                lines.append(f"Total Duration: {duration:.2f} seconds")
                lines.append(f"Start Time: {exec_summary.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
                lines.append(f"End Time: {exec_summary.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            if hasattr(exec_summary, 'node_executions'):
                ...
                if verbose:
                    ...
                    for i, node in enumerate(exec_summary.node_executions, 1):
                        lines.append(f"\n{i}. {node.node_name}")
                        lines.append(f"   ├─ Status: {'✅ Success' if node.success else '❌ Failed'}")
                    ...
                    for i, node in enumerate(exec_summary.node_executions, 1):
                        ...
                        if node.node_name == "UserInput" and isinstance(node.output, str):
                            lines.append(f"{i:2}. {node.node_name:<25} {duration_str:>8} {status_icon}  → \"{node.output}\"")
                        elif node.node_name == "Orchestrator" and isinstance(node.output, str):
                            lines.append(f"{i:2}. {node.node_name:<25} {duration_str:>8} {status_icon}  → {node.output}")
                        else:
                            lines.append(f"{i:2}. {node.node_name:<25} {duration_str:>8} {status_icon}")
