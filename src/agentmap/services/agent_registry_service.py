...
class AgentRegistryService:
    ...
    def unregister_agent(self, agent_type: str) -> None:
        ...
        if self.agent_registry.has_agent(agent_type):
            ...
            self.logger.debug(f"[AgentRegistryService] Unregistered agent: {agent_type}")
        else:
            self.logger.warning(f"[AgentRegistryService] Attempted to unregister unknown agent: {agent_type}")
    
    def get_agent_class(self, agent_type: str, default: Optional[Type] = None) -> Optional[Type]:
        """
        ...
        if default:
            ...
            self.logger.debug(f"[AgentRegistryService] Agent '{agent_type}' not found, no default provided")
    ...
    def list_agents(self) -> Dict[str, Type]:
        ...
        self.logger.debug(f"[AgentRegistryService] Listed {len(agent_map)} registered agents")
        return agent_map
    ...
    def get_registered_agent_types(self) -> List[str]:
        ...
        self.logger.debug(f"[AgentRegistryService] Found {len(agent_types)} registered agent types: {agent_types}")
        return agent_types
    ...
    def clear_all_agents(self) -> None:
        ...
        self.logger.warning(f"[AgentRegistryService] Cleared all {agent_count} registered agents")
