...
class AgentFactoryService:
    ...
    def validate_agent_dependencies(self, agent_type: str) -> Tuple[bool, List[str]]:
        ...
        if dependencies_valid:
            self.logger.debug(f"[AgentFactoryService] All dependencies valid for agent type '{agent_type}'")
        else:
            self.logger.debug(f"[AgentFactoryService] Missing dependencies for '{agent_type}': {missing_deps}")
    ...
    def _is_llm_agent(self, agent_type: str) -> bool:
        ...
        llm_agent_types = {
            "openai", "anthropic", "google", "gpt", "claude", "gemini", "llm",
            "chat", "conversation", "text_generation"
        }
    ...
    def _get_dependency_error_message(self, agent_type: str, missing_deps: List[str]) -> str:
        ...
        if "llm" in missing_deps:
            if agent_type_lower in ("openai", "gpt"):
                ...
                return (f"LLM agent '{agent_type}' requires Anthropic dependencies. "
                       "Install with: pip install agentmap[anthropic]")
            elif agent_type_lower in ("google", "gemini"):
                return (f"LLM agent '{agent_type}' requires Google dependencies. "
