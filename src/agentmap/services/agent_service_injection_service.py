...
class AgentServiceInjectionService:
    ...
    def __init__(
        ...
        host_protocol_configuration_service: Optional[HostProtocolConfigurationService] = None,
        ...
        graph_checkpoint_service: Optional[Any] = None,  # GraphCheckpointService - optional
        ...
        self.logger.debug("[AgentServiceInjectionService] Initialized with core service dependencies")
    ...
    def configure_core_services(self, agent: Any) -> int:
        ...
        self.logger.debug(f"[AgentServiceInjectionService] Configuring core services for agent: {agent_name}")
        ...
        try:
            ...
            if isinstance(agent, StorageCapableAgent):
                try:
                    ...
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure storage service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, CheckpointCapableAgent):
                if self.graph_checkpoint_service is None:
                    error_msg = f"Checkpoint service not available for agent {agent_name}"
                    self.logger.error(f"[AgentServiceInjectionService] ❌ {error_msg}")
                    raise Exception(error_msg)
                
                try:
                    agent.configure_checkpoint_service(self.graph_checkpoint_service)
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured checkpoint service for {agent_name}")
                    core_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure checkpoint service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, OrchestrationCapableAgent):
                if self.orchestrator_service is None:
                    error_msg = f"Orchestrator service not available for agent {agent_name}"
                    self.logger.error(f"[AgentServiceInjectionService] ❌ {error_msg}")
                    raise Exception(error_msg)
                
                try:
                    agent.configure_orchestrator_service(self.orchestrator_service)
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured orchestrator service for {agent_name}")
                    core_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure orchestrator service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, BlobStorageCapableAgent):
                if self.blob_storage_service is None:
                    error_msg = f"Blob storage service not available for agent {agent_name}"
                    self.logger.error(f"[AgentServiceInjectionService] ❌ {error_msg}")
                    raise Exception(error_msg)
                
                try:
                    agent.configure_blob_storage_service(self.blob_storage_service)
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured blob storage service for {agent_name}")
                    core_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure blob storage service for {agent_name}: {e}")
                    raise
    ...
    def configure_storage_services(self, agent: Any) -> int:
        ...
        """
        agent_name = getattr(agent, "name", "unknown")
        self.logger.debug(f"[AgentServiceInjectionService] Configuring storage services for agent: {agent_name}")
        ...
        try:
            ...
            if isinstance(agent, CSVCapableAgent):
                try:
                    ...
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured CSV service for {agent_name}")
                    storage_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure CSV service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, JSONCapableAgent):
                try:
                    ...
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured JSON service for {agent_name}")
                    storage_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure JSON service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, FileCapableAgent):
                try:
                    ...
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured File service for {agent_name}")
                    storage_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure File service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, VectorCapableAgent):
                try:
                    ...
                    if vector_service is None:
                        error_msg = f"Vector service not available for agent {agent_name}"
                        self.logger.error(f"[AgentServiceInjectionService] ❌ {error_msg}")
                        raise Exception(error_msg)
                    
                    agent.configure_vector_service(vector_service)
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured Vector service for {agent_name}")
                    storage_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure Vector service for {agent_name}: {e}")
                    raise
            ...
            if isinstance(agent, MemoryCapableAgent):
                try:
                    ...
                    if memory_service is None:
                        error_msg = f"Memory service not available for agent {agent_name}"
                        self.logger.error(f"[AgentServiceInjectionService] ❌ {error_msg}")
                        raise Exception(error_msg)
                    
                    agent.configure_memory_service(memory_service)
                    self.logger.debug(f"[AgentServiceInjectionService] ✅ Configured Memory service for {agent_name}")
                    storage_services_configured += 1
                except Exception as e:
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure Memory service for {agent_name}: {e}")
                    raise
            ...
            if (
                storage_services_configured == 0
                and isinstance(agent, StorageCapableAgent)
            ):
                try:
                    ...
                    self.logger.error(f"[AgentServiceInjectionService] ❌ Failed to configure default storage service for {agent_name}: {e}")
                    raise
    ...
    def configure_host_services(self, agent: Any) -> int:
        ...
        if not self._host_services_available:
            self.logger.debug(f"[AgentServiceInjectionService] Host services not available for {agent_name}")
            return 0
    ...
    def configure_execution_tracker(self, agent: Any, tracker: Optional[Any] = None) -> bool:
        """
    ...
    def configure_all_services(self, agent: Any, tracker: Optional[Any] = None) -> dict:
        ...
        """
        agent_name = getattr(agent, "name", "unknown")
        self.logger.debug(f"[AgentServiceInjectionService] Configuring all services for agent: {agent_name}")
        ...
        total_configured = core_configured + storage_configured + host_configured + (1 if tracker_configured else 0)
        ...
        summary = {
            ...
            "configuration_status": "success" if total_configured > 0 else "no_services_configured",
            "service_details": {
                ...
                "host_services_success": host_configured > 0 if self._host_services_available else None,
    ...
    def get_service_injection_status(self, agent: Any) -> dict:
        ...
        try:
            ...
            core_protocols = [
                ...
                (CheckpointCapableAgent, "graph_checkpoint_service", "configure_checkpoint_service"),
            ...
            for protocol_class, service_name, configure_method in storage_protocols:
                storage_type = service_name.replace("_service", "")  # csv_service -> csv
                try:
