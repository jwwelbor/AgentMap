...
class GraphDefinitionService:
    ...
    def build_from_csv(self, csv_path: Path, graph_name: Optional[str] = None) -> Graph:
        ...
        """
        self.logger.info(f"[GraphDefinitionService] Building single graph from: {csv_path}")
        ...
        self.logger.info(f"[GraphDefinitionService] Returning first graph: {first_graph_name}")
    ...
    def build_all_from_csv(self, csv_path: Path) -> Dict[str, Graph]:
        ...
        self.logger.info(f"[GraphDefinitionService] Building all graphs from: {csv_path}")
    ...
    def build_graph_from_csv(self, csv_path: Path, graph_name: Optional[str] = None) -> Graph:
        """
    ...
    def _create_nodes_from_specs(self, node_specs: List[NodeSpec], graph_name: str) -> Dict[str, Node]:
        """
        ...
        for node_spec in node_specs:
            ...
            if node_spec.name not in nodes_dict:
                ...
                context_dict = {"context": node_spec.context} if node_spec.context else None
                ...
                self.logger.debug(f"  ⏩ Node {node_spec.name} already exists, skipping creation")
        
        return nodes_dict
    
    def _connect_nodes_from_specs(self, nodes_dict: Dict[str, Node], node_specs: List[NodeSpec], graph_name: str) -> None:
        """
        ...
        for node_spec in node_specs:
            ...
            if node_spec.edge:
                self._connect_direct_edge(nodes_dict, node_name, node_spec.edge, graph_name)
            
            # Connect with conditional edges
            elif node_spec.success_next or node_spec.failure_next:
                if node_spec.success_next:
                    self._connect_success_edge(nodes_dict, node_name, node_spec.success_next, graph_name)
                
                if node_spec.failure_next:
                    self._connect_failure_edge(nodes_dict, node_name, node_spec.failure_next, graph_name)
    
    def _connect_direct_edge(self, nodes_dict: Dict[str, Node], source_node: str, target_node: str, graph_name: str) -> None:
    ...
    def _connect_success_edge(self, nodes_dict: Dict[str, Node], source_node: str, target_node: str, graph_name: str) -> None:
    ...
    def _connect_failure_edge(self, nodes_dict: Dict[str, Node], source_node: str, target_node: str, graph_name: str) -> None:
    ...
    def _convert_to_graph_domain_model(self, graph_name: str, nodes_dict: Dict[str, Node]) -> Graph:
        """
