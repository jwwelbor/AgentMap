...
class MinimalContainer:
    ...
    def initialize_service(self, service_name: str) -> Optional[Any]:
        ...
        if service_name not in self.required_services:
            self.logger.debug(f"Service '{service_name}' not in required services, skipping")
            return None
            
        # Check if already initialized
        if service_name in self.initialized_services:
            self.logger.debug(f"Service '{service_name}' already initialized, returning cached instance")
        ...
        try:
            ...
            if provider is None:
                self.logger.warning(f"Provider for service '{service_name}' not found in parent container")
                return None
    ...
    def _initialize_dependencies(self, service_name: str) -> None:
        ...
        for dep_name in dependencies:
            if dep_name in self.required_services and dep_name not in self.initialized_services:
                self.logger.debug(f"Initializing dependency '{dep_name}' for service '{service_name}'")
    ...
    def get_service(self, service_name: str) -> Optional[Any]:
        ...
        """
        if service_name not in self.required_services:
            self.logger.debug(f"Service '{service_name}' not in required services, returning None")
            return None
    ...
    def get_initialization_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'initialization_rate': len(self.initialized_services) / len(self.required_services) if self.required_services else 0,
            'required_services': sorted(self.required_services),
            'initialized_services': sorted(self.initialized_services),
            'uninitialized_services': sorted(self.required_services - self.initialized_services)
        }
