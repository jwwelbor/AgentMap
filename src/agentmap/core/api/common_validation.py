...
class RequestValidator:
    ...
    @classmethod
    def validate_file_path(cls, file_path: Union[str, Path], max_size: Optional[int] = None) -> Path:
        """
    ...
    def validate_workflow_name(cls, name: str) -> str:
        ...
        if len(name) > 100:
            raise HTTPException(
                status_code=400,
                detail="Workflow name too long (max 100 characters)"
            )
    ...
    def validate_graph_name(cls, name: str) -> str:
        ...
        if not name:
            raise HTTPException(
                status_code=400,
                detail="Graph name cannot be empty"
            )
        
        if len(name) > 100:
            raise HTTPException(
                status_code=400,
                detail="Graph name too long (max 100 characters)"
            )
        ...
        if not cls.SAFE_FILENAME_PATTERN.match(name):
            raise HTTPException(
                status_code=400,
                detail="Graph name contains invalid characters"
            )
    ...
    def validate_thread_id(cls, thread_id: str) -> str:
        ...
        if len(thread_id) > 100:
            raise HTTPException(
                status_code=400,
                detail="Thread ID too long (max 100 characters)"
            )
    ...
    def validate_response_action(cls, action: str) -> str:
        ...
        """
        valid_actions = {
            'approve', 'reject', 'choose', 'respond', 'edit', 
        ...
        if not action:
            raise HTTPException(
                status_code=400,
                detail="Response action cannot be empty"
            )
...
class ValidatedStateExecutionRequest(BaseModel):
    ...
    autocompile: bool = Field(default=False, description="Whether to autocompile if missing")
    execution_id: Optional[str] = Field(default=None, max_length=100, description="Optional execution tracking ID")
...
class ValidatedResumeWorkflowRequest(BaseModel):
    ...
    response_data: Optional[Dict[str, Any]] = Field(default=None, description="Additional response data")
...
class ErrorHandler:
    ...
    @staticmethod
    def handle_validation_error(field: str, message: str, value: Any = None) -> HTTPException:
