...
def refresh_cmd(
    ...
    try:
        ...
        if not storage_only:
            ...
            llm_results = dependency_checker.discover_and_validate_providers("llm", True)
            ...
            for provider, is_available in llm_results.items():
                ...
                if not is_available:
                    ...
                    if missing:
                        ...
                        guide = dependency_checker.get_installation_guide(provider, "llm")
                        typer.echo(f"    Install: {guide}")
        
        # Discover and validate storage providers
        if not llm_only:
            typer.echo("\n💾 Discovering Storage Providers...")
            storage_results = dependency_checker.discover_and_validate_providers("storage", True)
            ...
            for storage_type, is_available in storage_results.items():
                ...
                if not is_available:
                    # Get missing dependencies
                    _, missing = dependency_checker.check_storage_dependencies(storage_type)
                    if missing:
                        typer.echo(f"    Missing: {', '.join(missing)}")
                        guide = dependency_checker.get_installation_guide(storage_type, "storage")
                        typer.echo(f"    Install: {guide}")
        ...
        if llm_count > 0:
            typer.echo(f"    Providers: {', '.join(status_summary['llm']['available_providers'])}")
        
        typer.echo(f"  Storage Types Available: {storage_count}")
        if storage_count > 0:
            typer.echo(f"    Types: {', '.join(status_summary['storage']['available_types'])}")
        
        typer.secho("\n✅ Provider availability cache refreshed successfully!", fg=typer.colors.GREEN)
