...
def generate_api_key_config(name: str, permissions: list, expires_days: int = None) -> dict:
...
def create_auth_configuration() -> dict:
    ...
    keys = {
        ...
        'developer': generate_api_key_config('developer', ['read', 'write'], expires_days=90)
    }
...
def auth_init(
    config: str = typer.Option(
        ...
        "--config", 
        "-c", 
        help="Path to configuration file to create/update"
    ),
    ...
    typer.secho(f"✅ Authentication configuration {'updated' if force else 'created'} in: {config}", fg=colors.GREEN)
...
def auth_update(
    ...
    if 'authentication' not in existing_config:
        typer.secho(f"❌ No authentication section found in {config}. Use 'agentmap auth init' instead.", fg=colors.RED)
        raise typer.Exit(code=1)
...
def auth_view(
    config: str = typer.Option(
        ...
        "--config", 
        "-c", 
        help="Path to configuration file to view"
    ),
    ...
    typer.secho(f"\nAuthentication Enabled: {auth_config.get('enabled', False)}", fg=colors.GREEN if auth_config.get('enabled') else colors.RED)
    ...
    if 'public_endpoints' in auth_config:
        typer.secho(f"\n📂 Public Endpoints ({len(auth_config['public_endpoints'])}):", fg=colors.GREEN)
