    ...
    try:
        ...
        if dry_run:
            ...
            if preview['missing_declarations']:
                typer.secho(f"   Missing declarations: {len(preview['missing_declarations'])}", fg=typer.colors.YELLOW)
                for agent_type in preview['missing_declarations']:
                    typer.echo(f"      • {agent_type}")
            
            if preview['would_resolve']:
                typer.secho(f"   Would resolve: {len(preview['would_resolve'])} agents", fg=typer.colors.GREEN)
                for agent_type in preview['would_resolve']:
                    typer.echo(f"      • {agent_type}")
            
            if preview['would_update']:
                typer.secho(f"   Would update: {len(preview['would_update'])} mappings", fg=typer.colors.CYAN)
                for agent_type in preview['would_update']:
                    typer.echo(f"      • {agent_type}")
            
            if preview['would_remove']:
                typer.secho(f"   Would remove: {len(preview['would_remove'])} obsolete mappings", fg=typer.colors.RED)
            ...
            if not any([
                ...
                typer.secho("   ✅ No changes needed - bundle is up to date", fg=typer.colors.GREEN)
        ...
        if missing_count > 0:
            typer.secho(f"   Still missing: {missing_count} declarations", fg=typer.colors.YELLOW)
