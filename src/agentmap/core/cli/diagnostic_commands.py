...
def inspect_graph_cmd(
    ...
    node: Optional[str] = typer.Option(None, "--node", "-n", help="Inspect specific node only"),
    show_services: bool = typer.Option(True, "--services/--no-services", help="Show service availability"),
    show_protocols: bool = typer.Option(True, "--protocols/--no-protocols", help="Show protocol implementations"),
    show_config: bool = typer.Option(False, "--config-details", help="Show detailed configuration"),
    show_resolution: bool = typer.Option(False, "--resolution", help="Show agent resolution details")
):
    ...
    try:
        ...
        csv_path = Path(csv_file) if csv_file else container.app_config_service().get_csv_path()
        graph_def, resolved_name = graph_runner._load_graph_definition_for_execution(csv_path, graph_name)
        ...
        typer.echo(f"   Unique Agent Types: {agent_status['overall_status']['unique_agent_types']}")
        typer.echo(f"   All Resolvable: {'✅' if agent_status['overall_status']['all_resolvable'] else '❌'}")
        typer.echo(f"   Resolution Rate: {agent_status['overall_status']['resolution_rate']:.1%}")
        ...
        for node_name in nodes_to_inspect:
            if node_name not in graph_def:
                typer.secho(f"❌ Node '{node_name}' not found in graph", fg=typer.colors.RED)
                continue
            ...
            if show_resolution:
                ...
                if agent_type in agent_status['agent_types']:
                    ...
                    typer.echo(f"      Resolvable: {'✅' if type_info['resolvable'] else '❌'}")
                    typer.echo(f"      Source: {type_info.get('source', 'Unknown')}")
                    if not type_info['resolvable']:
                        typer.echo(f"      Issue: {type_info.get('resolution_error', 'Unknown error')}")
            
            # Try to create the agent to get service info
            try:
                # Get node registry for this graph
                node_registry = graph_runner.node_registry.prepare_for_assembly(graph_def, graph_name)
                
                # Create agent instance
                agent_instance = graph_runner._create_agent_instance(node_def, graph_name, node_registry)
                ...
                if show_config:
                    ...
                    for key, value in service_info.items():
                        if key not in ["agent_name", "agent_type", "services", "protocols", "configuration"]:
                ...
                typer.secho(f"   ❌ Failed to create agent: {str(e)}", fg=typer.colors.RED)
                # Show what we can from the agent status
                agent_type = node_def.agent_type or "Default"
                if agent_type in agent_status['agent_types']:
                    type_info = agent_status['agent_types'][agent_type]['info']
                    if not type_info['resolvable']:
                        typer.echo(f"   💡 Resolution error: {type_info.get('resolution_error', 'Unknown')}")
                        if type_info.get('missing_dependencies'):
                            typer.echo(f"   📦 Missing dependencies: {', '.join(type_info['missing_dependencies'])}")
        ...
        if agent_status["issues"]:
            ...
            typer.secho(f"\n✅ No issues found - all agents properly configured!", fg=typer.colors.GREEN)
            
        # Helpful suggestions
        typer.echo(f"\n💡 Helpful Commands:")
        typer.echo(f"   agentmap diagnose                    # Check system dependencies")
        typer.echo(f"   agentmap inspect-graph {graph_name} --config-details  # Show detailed config")
        if node:
            typer.echo(f"   agentmap inspect-graph {graph_name}             # Inspect all nodes")
        else:
            typer.echo(f"   agentmap inspect-graph {graph_name} --node NODE_NAME  # Inspect specific node")
