...
def init_config_command(
    ...
    for template_name, target_name in files_to_copy.items():
        ...
        if not template_path.exists():
            typer.secho(f"❌ Template file not found: {template_path}", fg=typer.colors.RED)
            raise typer.Exit(1)
    ...
    typer.secho(f"✅ Successfully copied {len(copied_files)} config files:", fg=typer.colors.GREEN)
