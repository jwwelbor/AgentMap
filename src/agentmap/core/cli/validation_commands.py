...
def validate_csv_cmd(
    csv_path: Optional[str] = typer.Option(None, "--csv", "-c", help="Path to CSV file to validate"),
...
def validate_all_cmd(
    csv_path: Optional[str] = typer.Option(None, "--csv", help="Path to CSV file to validate"),
    config_path: str = typer.Option("agentmap_config.yaml", "--config", "-c", help="Path to config file to validate"),
    no_cache: bool = typer.Option(False, "--no-cache", help="Skip cache and force re-validation"),
    fail_on_warnings: bool = typer.Option(False, "--fail-on-warnings", help="Treat warnings as errors"),
    config_file: Optional[str] = typer.Option(None, "--config-file", help="Path to initialization config file")
):
